...
In essence, "formulating the solution mathematically" means writing all of the equations that will be need to complete the assignment and briefly describing how to use them. The formulation is written in anticipation of implementing the solution numerically. The mathematical formulation always begins by generating the reactor design equations need to model the reactor. @sec-3_design_eqns describes how to do this. At the same time, it should identify the quantities that will be found by solving the design equations.
