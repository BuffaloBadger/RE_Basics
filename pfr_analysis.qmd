...
I need to generate the reactor design equations for modeling this particular PFR as discussed in [Chapter -@sec-ch5_ideal_reactor_analysis]. The minimum number of mole balances is equal to the number of mathematically independent reactions plus one mole balance per inert species. Since I'm going to solve them numerically, it's just as easy to write a mole balance on every reagent in the system (A, B, and Z in this assignment). The PFR operates at steady-state, so I'll start with the general form of the steady-state PFR mole balance given in @eq-pfr_ss_mol_balance. There is only one reaction taking place, so the summation reduces to a single term. 
...
**Given and Known Constants**: $D$ = 10 cm, $L$ = 5 m, $C_{A,in}$ = 1.0 M, $C_{B,in}$ = 1.2 M, $\dot{V}_{in}$ = 75 L min^-1^, $k_{0.1}$ = 8.72 x 10^5^ L mol^-1^ min^-1^, $E_1$ = 7200 cal mol^-1^, $\Delta H_1$ = –10,700 cal mol^-1^, $\tilde{C}_p$ = 1.0 cal g^-1^ K^-1^, $\rho$ = 1.0 g cm^-3^, $f_A$ = 0.95, and $R$ = 1.987 cal mol^-1^ K^-1^.
...
: Initial values and stopping criterion for solving  equations, (3) - (7). {#tbl-example_13_7_2_initial_values}
...
The feed temperature, $T_{in}$, is not specified. Indeed, it is one of the quatntities of interest. Instead, however, a second outlet value, namely the conversion, is provided. The inlet temperature must result in a conversion, $f_A \big \vert_{z=L}$, equal to the specified conversion, $f_A$, listed among the given and known quantities above. As described in @sec-apxd_ivodes, this can be written as an implicit equation for calculating $T_{in}: f_A \big \vert_{z=L} = f_{A,out}$. That implicit equation can be written as a residual expression (see @sec-apxd_ates) and solved numerically.
...
The PFR is not isothermal, so I must add an energy balance on the reacting gas. @eq-pfr_ss_energy_balance presents the general, steady-state form of the energy balance. None of the terms in the equation can be eliminated, but the two summations can be expanded, and the equation can be rearranged into the form of a derivative expression (see @sec-apxd_ivodes).
...
**Given and Known Constants**: $\Delta H$ = –9120 cal mol^-1^, $K_{0,1}$ = 0.132, $\hat{C}_{p,CO}$ = 29.3 J mol^-1^ K^-1^, $\hat{C}_{p,H_2O}$ = 34.3 J mol^-1^ K^-1^, $\hat{C}_{p,CO_2}$ = 41.3 J mol^-1^ K^-1^, $\hat{C}_{p,H_2}$ = 29.2 J mol^-1^ K^-1^, $\hat{C}_{p,I}$ = 40.5 J mol^-1^ K^-1^, $k_{0.1}$ = 0.0354 mol cm^-3^ min^-1^ atm^-2^, $E_1$ = 9740 cal mol^-1^, $P$ = 26 atm, $T_{in}$ = 320 °C, $\dot{n}_{CO,in}$ = 1 mol h^-1^, $\dot{n}_{CO_2,in}$ = 0.359 mol h^-1^, $\dot{n}_{H_2,in}$ = 4.44 mol h^-1^, $\dot{n}_{I,in}$ = 0.180 mol h^-1^, $f_{CO}$ = 0.55, and $R$ = 1.987 cal mol^-1^ K^-1^.
...
![Reactor volume as a function of H~2~O feed rate.](){#fig-example_13_7_4_V_vs_H2O_in width="80%"}

![Outlet Temperature as a function of H~2~O feed rate.](){#fig-example_13_7_4_T_vs_H2O_in width="80%"}
...
**Given and Known Constants**:  $D$ = 10 cm, $L$ = 5 m, $\dot{V}_{in}$ = 75 L min^-1^, $T_{in,b}$ = 25 °C, $C_{A,in,b}$ = 0.0 M, $C_{B,in,b}$ = 0.0 M, $C_{A,in,a}$ = 1.0 M, $C_{B,in,a}$ = 1.2 M, $T_{in,a}$ = 30 °C, $k_{0.1}$ = 8.72 x 10^5^ L mol^-1^ min^-1^, $E_1$ = 7200 cal mol^-1^, $\Delta H_1$ = –10,700 cal mol^-1^, $\tilde{C}_p$ = 1.0 cal g^-1^ K^-1^, $\rho$ = 1.0 g cm^-3^, $\tau$ = 0.131, 0.262, 0.393, and 0.524 min, and $R$ = 1.987 cal mol^-1^ K^-1^.
...
The catalyst particles are spherical, so I can calculate the effectiveness factor using @eq-effectiveness_factor. In order to do that, I will first need to calculate the Thiele modulus for a spherical particle using @eq-Thiele_modulus and noting that the particle radius is half of its diameter.
...
## Symbols Used in [Chapter -@sec-ch9]
