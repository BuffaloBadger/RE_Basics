...
The same effect is seen with temperature, but for an adiabatic, exothermic reaction, this gives an advantage to the CSTR. The temperature will rise over time in the BSTR, but in the CSTR the temperature is at its final, highest value the entire time the reaction is taking place. Assuming Arrhenius temperature dependence of the rate coefficient, that means that the rate coefficient will be larger, and hence the rate will be greater in the CSTR than at the start of batch processing.

## CSTR Design Equations

The reactor design equations for CSTRs are derived in @sec-apndx_ideal_reactor_models and discussed in @sec-3_design_eqns. The general CSTR mole balance, @eq-cstr_mol_balance, and reacting fluid energy balance, @eq-cstr_energy_balance, are reproduced below along with the energy balances for the heat exchange fluid for the two situations where it exchanges only sensible heat, @eq-exchange_energy_bal_sensible, and where it exchanges only latent heat, @eq-exchange_energy_bal_latent. The choice of which equations to use when modeling a particular reactor is also discussed in @sec-3_design_eqns.
...
It is noted in @sec-3_design_eqns that when a CSTR operates at steady-state, nothing changes over time. consequently the time derivatives in the design equations all equal zero. The corresponding reactor design equations are reproduced below.
...
It is important to note that the $V\frac{dP}{dt}$ and $P\frac{dV}{dt}$ terms in the reacting fluid energy balance will have different units than the other terms, as was the case for the BSTR and SBSTR reacting fluid energy balances. As described in @sec-3_design_eqns, the CSTR design equations can be modified and simplified, depending upon the particular CSTR being modeled. For liquid phase reacting fluids, the gravimetric or volumetric heat capacity may be known instead of the molar heat capacities of the individual reagents, in which case the sensible heat terms can be modified.
...
Suppose the temperature of a CSTR was operating at the low steady state was increased very slightly. That is, on the graph, suppose the operating point shifted slightly to the right of the intersection labeled low. At that new point the heat absorption (blue) would be greater than the heat generation (red), causing the operating temperature to decrease (i. e. to shift back toward the low point). 

Similarly, if the temperature of the CSTR operating at the low steady state was decreased (shifted left), heat generation (red) would be greater than heat absorption (blue), causing the temperature to increase (i. e. to again shift back toward the low point). This argument suggests that the low steady state is stable because following a small perturbation away from it, they system returns back to it. The exact same argument can be applied to the high steady state. Following any small perturbation from it, the system returns to it.

Now consider the middle steady state. If the operating temperature is increased very slightly (i. e. shifted to the right), the heat generation will be greater than the heat absorption. This will cause the temperature to rise. That is, the point will move even farther to the right. In fact, it will continue to move to the right until it reaches the high steady state.

If the operating temperature of a CSTR operating at the middle steady state is decreased very slightly (i. e. shifted slightly to the left), the heat absorbed will exceed the heat generated and the temperature will decrease. That is, the point will move far the to the left. This will continue until the point reaches the low steady state.
...
The is another possible result of perturbing a steady-state reactor. The reactor could go into a state of sustained, periodic oscillations. This possibility is not considered in *Reaction Engineering Basics*. The topic of steady state stability is often considered in detail in more advanced reaction engineering books and courses.

### Parametric Sensitivity of CSTRs{#sec-ignition_and_extinction}

For reasons just described, real CSTRs do not operate at unstable steady states. This leads to one type of parametric sensitivity known as hysteresis. @fig-hysteresis shows only the stable steady states from @fig-T_vs_Tin_unstable. Consider a CSTR operating at steady state with an inlet temperature of -20 °C. If the inlet temperature is very slowly increased in a way that the reactor remains at steady-state, the outlet temperature will slowly rise along the lower black curve in the figure until it reaches the solid red line.
...
If the CSTR was operating at an inlet temperature of 100 °C (outlet temperature ~320 °C) and the feed temperature was very slowly decreased so that the reactor remained at steady state, it would follow the uppre black curve in the figure until it reached the dashed red line. The point where the temperature just reaches the dashed red line is called the **extinction point**. Lowering the temperature just slightly below the extinction point would cause the reactor temperature to drop to the lower black curve. That is, the reaction would shut down.
...
Ignition and extinction are one type of **parametric sensitivity**. More generally, when a small change in a reactor operating parameter results in an extreme change in the reactor output, the reactor is said to be parametrically sensitive to the parameter being changed. Identifying and avoiding operation at conditions near a point of parametric sensitivity is essential for the safe operation of CSTRs.
...
The first four examples in this section involve steady-state CSTRs. @sec-example_12_7_1 and @sec-example_12_7_2 illustrate steady-state CSTR response tasks. The former involves two liquid-phase reactions while the second involves a gas phase reaction. @sec-example_12_7_3 presents the solution of an optimization task involving a steady-state CSTR. @sec-example_12_7_4 features a steady-state CSTR where three steady states are possible. Start-up of a CSTR, a transient process, is analyzed in @sec-example_12_7_5. Finally, @sec-example_12_7_6 follows three transients that result when an operating parameter of a steady-state CSTR is perturbed. In the first analysis the CSTR is operating at an unstable steady state, and when it is perturbed, the transient ends when it reaches a different, stable steady state. The other two analyses show what happens when a steady-state CSTR operating close to an ignition or extinction point is perturbed beyond the ignition or extinction point.
...
The outlet concentrations can be calculated using their definitions. The reacting fluid here is an ideal liquid, so the inlet and outlet volumetric flow rates will be equal.
...
Solving the reactor design equations yields $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_D$, $\dot{n}_U$, and $T$. The conversion of A and the selectivity for D over U can be calculated using equations (17) and (18).
...
**Given and Known Constants**: $y_{A,in}$ = 0.1, $y_{Bin}$ = 0.65, $y_{I,in}$ = 0.25, $T_{in}$ = 165 °C, $P$ = 5 atm, $y_{A,max}$ = 0.001, $\tau$ = 0.5 min, $k_{0,1}$ = 1.37 x 10^5^ m^3^ mol^-1^ min^-1^, $E_1$ = 11,100 cal mol^-1^, $\Delta H_1$ = -7200 cal mol^-1^, $\hat{C}_{p,A}$ = 7.6 cal mol^-1^ K^-1^, $\hat{C}_{p,B}$ = 8.2 cal mol^-1^ K^-1^, $\hat{C}_{p,I}$ = 4.3 cal mol^-1^ K^-1^, and $\hat{C}_{p,Z}$ = 13.8 cal mol^-1^ K^-1^, and $R$ = 1.987 cal mol^-1^ K^-1^ = 8.206 x 10^-5^ m^3^ atm mol^-1^ K^-1^.
...
I will solve reactor design equations for the unknown outlet molar flow rates and the outlet temperature. In order to solve the ATEs numerically I'll need to do two things: calculate the values of the residuals for each guess and provide an initial guess.

Starting with evaluating the residuals, I'll need to evaluate them each time a new guess for the solution is generated. At that point, guesses for each of the unknowns will be available along with the given and known constants. I will need to calculate any other quantities that appear in the residual expressions. Looking at equations (3) through (7) I see that for this assignment I'll need to calculate the volume, $V$, inlet molar flow rates, $\dot{n}_{i,in}$, and rate, $r_1$.

Looking back at the assignment summary, I see that all of the given and known quantities are intensive. That is, they do not establish the size of the system. In that situation I am free to choose a basis for my calculations. I'll use a reactor fluid volume of 1 m^3^ as my basis. Having chosen a reactor volume, I can calculate the inlet volumetric flow rate using the given space time, @eq-space_time.

$$
\tau = \frac{V}{\dot V_{in}} \qquad \Rightarrow \qquad \dot{V}_{in} = \frac{V}{\tau}
$$

The assignment provides inlet mole fractions along with the pressure and inlet temperature. Having calculated the inlet volumetric flow rate, I can use the ideal gas law to calculate the inlet molar flow rates. The inlet mole fractions of A, B, and I sum to 1, so the inlet mole fraction of Z must equal zero.

$$
\dot{n}_{i,in} = y_{i,in}\frac{P\dot{V}_{in}}{RT}
$$

The rate can be calculated using equaion (2), but to do that I must calculate the rate coefficient and the outlet concentrations of A and B. The former can be accomplished using the Arrhenius expression, @eq-arrhenius, and for the outlet concentrations I can again use the ideal gas law.
...
$$
V = 1 \text{ m}^3 \left(\text{basis}\right)
$$
...
$$
\dot{n}_{B,in} = y_{B,in}\frac{P\dot{V}_{in}}{RT} \tag{10}
$$

$$
\dot{n}_{I,in} = y_{I,in}\frac{P\dot{V}_{in}}{RT} \tag{11}
$$
...
$$
C_A = \frac{\dot{n}_A}{\dot{n}_A + \dot{n}_B + \dot{n}_I + \dot{n}_Z}\frac{P}{RT} \tag{13}
$$

$$
C_B = \frac{\dot{n}_B}{\dot{n}_A + \dot{n}_B + \dot{n}_I + \dot{n}_Z}\frac{P}{RT} \tag{14}
$$

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

When I solve the ATEs numerically, I'll also need to provide an initial guess for the solution. I know that the molar flow rates of A and B will decrease and that of Z will increase, but I don't know by how much. The reaction is exothermic and the reactor is adiabatic, so I know the temperature will increase, but again I don't know by how much. I'll start with small guesses for the changes. If the ATE solver doesn't converge (and I can't find any typos or other errors in my computer code), I'll try changing the guesses until it does.

When it does converge, the outlet molar flow rates and temperature, $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_I$, $\dot{n}_Z$, and $T$ will be known. $T$ is one of the quantities of interest. The others are the outlet mole fractions of the reagents. They can be calculated using the definition of a mole fraction.

:::

**Ancillary Equations for Calculating the Quantities of Interest**

Solving the reactor design equations yields $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_I$, $\dot{n}_Z$, and $T$. The mole fractions of A, B, I, and Z can be calculated using equations (15) - (18).

$$
y_A = \frac{\dot{n}_A}{\dot{n}_A + \dot{n}_B + \dot{n}_I + \dot{n}_Z} \tag{15}
$$

$$
y_B = \frac{\dot{n}_B}{\dot{n}_A + \dot{n}_B + \dot{n}_I + \dot{n}_Z} \tag{16}
$$

$$
y_I = \frac{\dot{n}_I}{\dot{n}_A + \dot{n}_B + \dot{n}_I + \dot{n}_Z} \tag{17}
$$

$$
y_Z = \frac{\dot{n}_Z}{\dot{n}_A + \dot{n}_B + \dot{n}_I + \dot{n}_Z} \tag{18}
$$
...
1. Substitute given and known constants into all equations.
2. When it is necessary to evaluate the residuals
	a. $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_I$, $\dot{n}_Z$, and $T$ will be available, and the basis ,$V$, will have been chosen and made available.
	b. calculate $\dot{V}_{in}$ using equation (8)
	c. calculate $\dot{n}_{A,in}$, $\dot{n}_{B,in}$, $\dot{n}_{I,in}$, $k_1$, $C_A$ and $C_B$ using equations (9) through (14)
	d. calculate $r_!$ using equation (2)
	e. evaluate the residuals, $\epsilon_1$ through $\epsilon_5$ using equations (3) through (7)
3. When it is necessary to calculate the quantities of interest, $y_A$, $y_B$, $y_I$, and $y_Z$,
	a. $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_I$, $\dot{n}_Z$, and $T$ will be available.
	b. calculate $y_A$, $y_B$, $y_I$, and $y_Z$ using equations (15) through (18).

#### Numerical implementation of the Solution

1. Make the given and known constants and the chosen basis available for use in all functions.
2. Write a residuals function that
	a. receives guess values for $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_I$, $\dot{n}_Z$, and $T$,
	b. evaluates the residuals as described in step 2 of the calculations summary, and
	c. returns the values of the residuals.
3. Write a reactor model function that
	a. receives an initial guess for the unknowns, $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_I$, $\dot{n}_Z$, and $T$,
	c. gets a solution of the ATEs by calling an ATE solver and passing the following information to it
		i. the initial guess and
		ii. the name of the residuals function from step 2 above,
	d, checks that the solver converged, and
	e. returns the values returned by the ATE solver.
...
The calculations were performed as described above. The outlet gas temperature is `r df$value[5]` `r df$units[5]` and the outlet composition is `r df$value[1]``r df$units[1]` A, `r df$value[2]``r df$units[2]` B, `r df$value[3]``r df$units[3]` I, and `r df$value[4]``r df$units[4]` Z. This analysis was quite straightforward, and the results are as would be expected on the basis of a qualitative analysis.

:::{.callout-note collapse="false"}
## Note

Even though the analysis in this assignment was very straightforward, there is an important point to recognize. Very often, CSTRs are used to process liquid feeds. Assuming the liquids to be ideal solutions makes the volumetric flow rate constant; there is no expansion or contraction of the fluid. The concentrations that are substituted into the rate expressions are simply the outlet molar flow rate divided by the outlet volumetric flow rate, and since the volumetric flow rate is constant, the inlet and outlet volumetric flow rates are equal.
...
The results shown in @fig-example_12_7_3_figure are reasonable. For any feed temperature heat will be released by reaction leading to an outlet temperature that is greater than the inlet temperature. At lower feed temperatures, the reaction does not approach equilibrium. Increasing the feed temperature results in a greater outlet temperature. That, in turn results in a larger reaction rate, and consequently an increase in the conversion.

The reaction is exothermic, so as the temperature increases, the equilibrium constant decreases. At intermediate feed temperatures, the reaction begins to approach thermodynamic equilibrium. This reduces the effect of temperature upon the reaction rate and the conversion rises less steeply as the inlet temperature increases. Eventually, as the inlet temperature is increased, the reaction reaches thermodynamic equilibrium.

Further increase in the inlet temperature results in decreasing conversion. The rate is sufficient to reach equilibrium, but because the reaction is exothermic, the equilibrium conversion desreases as the temperature rises.
...
The reactor is not isothermal, so the mole balances cannot be solved independently. An energy balance on the reacting fluid is needed. @eq-cstr_ss_energy_balance gives the general form of the reacting fluid energy balance for a steady-state CSTR. The reactor is adiabatic and there are no shafts or moving boundary that are performing work, so $\dot{Q}$ and $\dot{W}$ are equal to zero. The constant heat capacities can be taken outside of the integrals, and the integral can be evaluated. There is only one reaction, so the final summation reduces to a single term.

$$
0 = \cancelto{0}{\dot Q} - \cancelto{0}{\dot W} - \cancelto{\left(\sum_i\dot n_{i,in}\hat C_{p,i}\right) \left(T - T_{in}\right)}{\sum_i\dot n_{i,in} \int_{T_{in}}^T \hat C_{p,i}dT} - V\cancelto{r_1 \Delta H_1}{\sum_j r_j \Delta H_j}
$$
...
Starting with evaluating the residuals, I'll need to evaluate them each time a new guess for the solution is generated. At that point, guesses for $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_Y$, $\dot{n}_Z$, and $T$ will be available along with the given and known constants. I will need to calculate any other quantities that appear in the residuals expressions.
...
This assignment states that three steady states are possible. In most cases, however, an engineer won't know whether multiple steady states are possible or not. It is critically important to know when multiple steady states exist so that the desired steady state can be identified and appropriate start-up procedures can be developed to reach it.
...
The reactor model was used to calculate the steady-state outlet temperature and the steady-state conversion as functions of the feed temperature for this reactor. The results are presented in @fig-example_12_7_4_T_vs_Tin and @fig-example_12_7_4_f_vs_Tin. In both cases the curves exhibit the "S" shape described in @sec-multiplicity. The dashed sections of the (black) curves are unstable steady states (see @sec-sec-stability). The ignition point occurs at an inlet temperature of 90 °C and is shown as a solid red line. The extinction point occurs at an inlet temperature of 4 °C and is indicated by a dashed red line.
...
When these IVODEs are solved numerically, it will be necessary to evaluate the derivatives at the start of each integration step. That will be straightforward if they are written in the form of derivative expressions (see @sec-apndx_solve_ivodes.)
...
4. When it is necessary to calculate the quantities of interest, [list]
