...
By definition, every reaction in a reaction mechanism is an elementary reaction. Consequently, the rate expression for each mechanistic step will have the functional form shown in @eq-net_rate_elem_rxn, or for gases, in @eq-net_rate_elem_rxn_part_press. The rate coefficients for the absolute rate in the reverse direction are not independent. They are related to the rate coefficients for the absolute rate in the forward direction and the equilibrium constant, @eq-k_relation_to_K. However, in the case of mechanistic steps, thermodynamic data  for the reactive intermediates are often not available. In those cases, the equilibrium constant for the mechanistic step won’t be known. For this reason, in this chapter the rate expressions for mechanistic steps will be written using forward and reverse rate coefficients and not equilibrium constants. @sec-ch3 showed that for gas phase reaction mechanisms, either concentration or partial pressures can be used as the composition variables and the rate coefficients will still exhibit Arrhenius temperature dependence. As such, it is convenient to use square brackets to represent either concentrations or partial pressures in rate expressions for mechanistic steps, as in @eq-rate_mech_step.
...
$$
\begin{align}
0 &= \sum_{j^\prime}\nu_{i_{RI},j^\prime}r_{j^\prime}\\
& j^\prime \text{ indexes the steps in the reaction mechanism}
\end{align}
...
$$
\begin{align}
\Delta G_{j_{non}} &= \sum_{j^\prime} \sigma_{j^\prime} \Delta G_{j^\prime} \\
& j^\prime \text{ indexes all steps in the mechanism}
\end{align}
...
1. Identify the reactive intermediates and the macroscopically observed reagents that are easily quantifiable.
...
3. Eliminate the concentrations (partial pressures) of reagents that are not easily quantifiable.
    ...
    d. Solve the equations resulting from steps 3a through 3c to obtain expressions for the concentrations (partial pressures) of the reactive intermediates.
    e. Substitute the expressions for the concentrations (partial pressures) of the reactive intermediates into the rate expression from step 2.
4. Simplify the resulting rate expression as appropriate.
    a. Define the minimum number of apparent rate coefficients.
        i. Determine whether each apparent rate coefficient will exhibit Arrhenius temperature dependence.
...
## Learning Objectives and Examples {#sec-ch4_learning_objectives}
...
* be able to
    * write an expression for the rate of generation of a reactant or product of a non-elementary reaction in terms of its rates of generation in the steps of the reaction’s mechanism (-@sec-ch4_ex1, -@sec-ch4_ex3, -@sec-ch4_ex4)
    * simplify rate expressions for kinetically insignificant and effectively irreversible steps (-@sec-ch4_ex1, -@sec-ch4_ex3, -@sec-ch4_ex4)
    * identify reactive intermediates (-@sec-ch4_ex1, -@sec-ch4_ex2, -@sec-ch4_ex3, -@sec-ch4_ex4, -@sec-ch4_ex5, -@sec-ch4_ex6)
    * write Bodenstein steady-state approximation for a reactive intermediate (-@sec-ch4_ex1, -@sec-ch4_ex3, -@sec-ch4_ex4)
    * eliminate concentrations of reactive intermediates from mechanistic rate expressions using the Bodenstein steady-state approximation (-@sec-ch4_ex1, -@sec-ch4_ex3, -@sec-ch4_ex4)
    * define the minimum number of apparent rate coefficients in a mechanistic rate expression (-@sec-ch4_ex1, -@sec-ch4_ex2, -@sec-ch4_ex3, -@sec-ch4_ex4, -@sec-ch4_ex5, -@sec-ch4_ex6)
    * write an expression for the rate of a non-elementary reaction in terms of the rate of the rate-determining step in the reaction’s mechanism, when appropriate (-@sec-ch4_ex2, -@sec-ch4_ex5, -@sec-ch4_ex6)
    * eliminate concentrations of reactive intermediates from mechanistic rate expressions using quasi-equilibrium expressions, when appropriate (-@sec-ch4_ex2, -@sec-ch4_ex5, -@sec-ch4_ex6)
    * write an expression for the conservation of catalyst, enzyme, catalytic sites or charge (-@sec-ch4_ex3, -@sec-ch4_ex4, -@sec-ch4_ex5, -@sec-ch4_ex6)
    * eliminate concentrations of reagent-catalyst complexes from mechanistic rate expressions when catalysts, enzymes, or charged species participate in the mechanism (-@sec-ch4_ex3, -@sec-ch4_ex4, -@sec-ch4_ex5, -@sec-ch4_ex6)
    * identify terms in a mechanistic rate expression that can be eliminated when there is a most abundant intermediate and simplify the mechanistic rate expression accordingly (-@sec-ch4_ex6)
