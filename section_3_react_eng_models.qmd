...
The four types of ideal reactors were briefly considered in @sec-1_intro_and_defs where schematic diagrams of stirred tanks, @fig-closed_stirred_tank_reactor, and tubular reactors, @fig-pfr, were presented. Specifically, batch stirred-tank reactors (BSTRs), semi-batch stirred-tank reactors (SBSTRs), continuous stirred-tank reactors (CSTRs), and plug flow reactors (PFRs) were introduced.  The modeling of each of these will be discussed in detail later in this chapter. To begin, however, a generic reactor (one that could be any one of those four ideal reactor types) is considered. Doing so helps identify the different kinds of variables needed to model a reactor as well as the equations that are used to describe the relationships between those variables.
...
As discussed in [Appendix -@sec-apndx_heat_exch], there are two forms of the energy balance on the exchange fluid: one is used when the temperature of exchange fluid changes, and the other is used when the exchange fluid changes phase. Examples [-@sec-example_6_8_3] and [-@sec-example_6_8_5] illustrate sensible heat is exchanged, i. e. exchange fluid temperature changes, and [Example -@sec-example_6_8_4] illustrates a system where latent heat is exchanged, i. e. the exchange fluid undergoes a phase change.
...
$$
C_i = \frac{\dot{n}_i}{\displaystyle \sum_i \dot n_i}\frac{P}{RT}
...
{{< include ../RE_Basics_Examples/reb_6_8_1/problem_statement.qmd >}}
...
The reactor operates isothermally, and its temperature is known. Consequently the mole balance equations above can be solved independently of any other reactor design equations. Here the equations are needed to model the only the composition of the system, so equations (2) and (3) are the only reactor design equations needed.
...
{{< include ../RE_Basics_Examples/reb_6_8_2/problem_statement.qmd >}}
...
{{< include ../RE_Basics_Examples/reb_6_8_3/problem_statement.qmd >}}
...
$$
\breve C_pV \frac{dT}{dt} - P\frac{dV}{dt} = \dot Q - \dot V_{in} \breve C_p \left( T - T_{in} \right)  - r_1  \Delta H_1 \tag{6}
$$
...
$$
\rho_{ex} V_{ex} \tilde C_{p,ex}\frac{dT_{ex}}{dt} = -\dot Q - \dot m_{ex} \tilde C_{p,ex} \left( T_{ex} - T_{ex,in} \right) \tag{7}
$$
...
{{< include ../RE_Basics_Examples/reb_6_8_4/problem_statement.qmd >}}
...
$$
0 = \dot n_{A,in} - \dot n_A - r_1 V \tag{2}
$$

$$
0 = \dot n_{B,in} - \dot n_B - r_1 V \tag{3}
$$

$$
0 = - \dot n_Z = \dot{n}_Z + r_1 V \tag{4}
$$
...
$$
0 = \dot Q - \rho \dot V_{in} \tilde C_p  \left( T - T_{in} \right) - r_1 V \Delta H_1 \tag{5}
$$
...
$$
0 = - \dot Q - \gamma \dot m_{ex} \frac{\Delta H_{\text{latent},ex}^0}{M_{ex}} \tag{6}
$$
...
{{< include ../RE_Basics_Examples/reb_6_8_5/problem_statement.qmd >}}
...
$$
\frac{d \dot n_A}{dz} = \frac{\pi D^2}{4}\left(-2 r_1 - r_2 \right) \tag{3}
$$
...
$$
\frac{d \dot n_Z}{dz} = \frac{\pi D^2}{4}r_2 \tag{5}
$$
...
$$
\frac{\partial \dot n_i}{\partial z} + \frac{\pi D^2}{4\dot V} \frac{\partial\dot n_i}{\partial t} - \frac{\pi D^2\dot n_i}{4\dot V^2} \frac{\partial \dot V}{\partial t}  =\frac{\pi D^2}{4}\left(\nu_{i,1}r_1 + \nu_{i,2}r_2\right)
$$
...
$$
\left(\sum_i \dot n_i \hat C_{p,i} \right) \frac{dT}{dz} = \pi D U\left( T_{ex} - T  \right) - \frac{\pi D^2}{4}\left( r_1 \Delta H_1 + r_2 \Delta H_2 \right)
$$
...
$$
\left( \dot n_A \hat C_{p,A} + \dot n_{A_2} \hat C_{p,A_2} + \dot n_I \hat C_{p,I} \right) \frac{d T}{dz} = \pi D U\left( T_{ex} - T  \right) - \frac{\pi D^2}{4}\left( r_1 \Delta H_1 + r_2 \Delta H_2 \right)
$$
