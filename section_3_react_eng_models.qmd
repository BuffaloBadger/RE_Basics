# Reaction Engineering Models {#sec-3_react_eng_models}

Reactor models are essential for reaction engineering analyses. A reactor model is a set of equations that relates what goes into a reactor to what comes out. Other parameters that affect the relationship between what goes in and what come out appear within the model. This chapter describes and discusses the variables needed to model a reactor and the equations that constitute a reactor model.

## A Generic Reactor

The four types of ideal reactors were briefly considered in @sec-1_1_b_chem_reactors where schematic diagrams of stirred tanks, @fig-closed_stirred_tank_reactor, and tubular reactors, @fig-pfr, were presented. Specifically, batch stirred tank reactors (BSTRs), semi-batch stirred tank reactors (SBSTRs), continuous stirred tank reactors (CSTRs), and plug flow reactors (PFRs) were introduced.  Models for each of these will be discussed in detail later in this chapter. To begin, however, a generic reactor (one that could be any one of those four ideal reactor types) is considered. Doing so helps identify the different kinds of variables needed to model a reactor as well as the equations that are used to describe the relationships between those variables.

### Variables Used When Modeling a Reactor

Briefly, every ideal reactor has a compartment that contains reacting fluid. The reacting fluid in most reactors needs to be heated or cooled. There are many ways to do that, but as discussed in [Appendix -@sec-apndx_heat_exch], only one form of heat exchange is considered in *Reaction Engineering Basics*. If the reacting fluid needs to be heated or cooled, the reactor will have a second compartment. Depending on the specific physical structure of the reactor, the second compartment is sometimes referred to as the reactor shell or jacket, and a fluid will be flowing through it. That fluid is called the exchange fluid because its purpose is to exchange heat with the reacting fluid. The exchange fluid is physically separated from the reacting fluid by a solid wall that prevents the two fluids from mixing. Heat is transferred through the wall separating the two reactor compartments. 
...
| Reactor/Process Parameters | Reactor dimensions (typically volume or diameter and length), shell/jacket volume, heat transfer area, heat transfer coefficient, packed bed porosity, and some measure of reaction time. |
...
It is not always necessary to use all of the reactor modeling equations listed in @tbl-reactor_equations. For example, in closed and steady-state systems, mole balances do not need to be written for every reagent in the system because the amounts of the reactants and products in the reactions are related through stoichiometry. The amount of an inert reagent is not related to the amount of any other reagent through stoichiometry. If $N_{ind}$ is the number of mathematically independent reactions taking place in the system, then $N_{ind}$ mole balances are needed to relate the steady-state input amounts of reactants and products to the output amounts. Consequently, the minimum number of mole balances is equal to $N_{ind}$ plus one mole balance for each inert reagent. [Example -@sec-example_6_8_1] provides an example of this.

liquid phase systems often consist of a solvent that contains the reagents as solutes. In systems of this type, sensible heat terms are usually written in terms of the mass-specific or volume-specific heat capacity of the entire reacting fluid. Generally, a mole balance on the liquid phase solvent is not needed for such systems. If, for some reason, the sensible heat terms are written in terms of molar heat capacities of the individual reagents, then the solvent must be included among those reagents and a mole balance on the solvent would need to be included among the reactor design equations. [Example -@sec-example_6_8_4] illustrates a system involving a liquid phase solution.

A momentum balance is only needed when the reactor is a PFR. Even then, some PFR assignments indicate that pressure drop is negligible, in which case the momentum balance is not needed.

If a reactor operates adiabatically there will not be an exchange fluid, and consequently an energy balance on the exchange fluid cannot be included among the design equations. [Example -@sec-example_6_8_2] illustrates the formulation of a reactor model for an adiabatic reactor. For historical reasons it is worth noting that for steady-state, adiabatic systems, the energy balance on the reacting fluid can be replaced by an equation for the adiabatic temperature (see [Appendix -@sec-apndx_reaction_thermo] and particularly @eq-adiabatic_temperature). Before computers became commonplace, doing this sometimes made it easier to solve the steady-state design equations analytically. It isn't as important now that powerful personal computers that can solve multiple ODEs are ubiquitous.

If a reactor operates isothermally, the mole balance equations and the energy balance equations are not coupled. The mole balance equations can be solved separately from the two energy balance equations. True isothermal operation of an industrial reactor is not very common, but laboratory reactors that are used to generate kinetics data normally operate isothermally. The energy balances may be needed when modeling isothermal reactors, but if the reacting fluid temperature is known, the mole balances can be solved separately from the energy balances, and *vice versa*. [Example -@sec-example_6_8_1] illustrates writing reactor design equations for an isothermal reactor.

If the outlet temperature of the exchange fluid is known and constant, the other reactor design equations can be solved independently from the energy balance on the exchange fluid. There are two situations where this can happen. The first is when the exchange fluid undergoes a phase change and exchanges the associated latent heat. Phase changes occur at constant temperature, so, for example, if condensation of saturated steam at 1 atm is being used to supply heat to the reacting fluid, the temperature of the exchange fluid will be constant and equal to 100 °C. Knowing this, all the other reactor design equations can be solved separately from the energy balance on the exchange fluid. In this case, the energy balance on the exchange fluid can be solved separately, for example, to calculate the minimum exchange fluid flow rate needed to provide the required amount of energy to the reacting fluid. [Example -@sec-example_6_8_4] illustrates writing, simplifying and solving reactor design equations for a system where the exchange fluid phase changes.
...
The differential form of the ideal gas law is included among the reactor design equations in @tbl-reactor_equations. In some cases where the mole and energy balances for a stirred tank reactor are ODEs, there can be one more dependent variables than the number of ODEs. In this situation, the differential form of the ideal gas law is included among the reactor design equations, as indicated in the sections on each of the ideal stirred tank reactors that follow.

## Exchange Fluid Energy Balance {#sec-heat_exchange_eqns}

An energy balance on the exchange fluid is one of the design equations for every one of the nonadiabatic ideal reactors, so it is considered first here. *Reaction Engineering Basics* assumes that the exchange fluid continually flows through the shell/jacket, and that within the shell/jacket, it is perfectly mixed. The exchange fluid is further assumed to be a pure fluid that either undergoes phase change or temperature change, but not both. The associated latent or sensible heat is transferred to or from the reacting fluid through a common wall that separates them.

As discussed in [Appendix -@sec-apndx_heat_exch], there are two forms of the energy balance on the exchange fluid: one is used when the temperature of exchange fluid changes, and the other is used when the exchange fluid changes phase. Examples [-@sec-example_6_8_3] and [-@sec-example_6_8_5] illustrate sensible heat exchange, i. e. exchange fluid temperature changes, and [Example -@sec-example_6_8_4] illustrates a system where latent heat is exchanged, i. e. the exchange fluid undergoes a phase change.
...
## Ideal BSTR Design Equations {#sec-ideal_bstr_eqns}
...
$$
\left(\sum_i n_i \hat{C}_{p,i} \right) \frac{dT}{dt}\ \Leftrightarrow\ \rho V \tilde{C}_p \frac{dT}{dt}\ \Leftrightarrow\ V \breve{C}_p \frac{dT}{dt}
$$ {#eq-bstr_equivalent_sens_heat_terms}

The second and third terms on the left side of @eq-bstr_energy_balance represent accumulation of energy associated with work done during expansion or contraction of the reacting fluid.

:::{.callout-note collapse="false"}
## Note

It is likely that the second and third terms in @eq-bstr_energy_balance will have units of pressure times volume per time while the other terms will have units of energy per time. It is critical to remember to convert all terms to consistent units. An easy way to do this is to multiply the second and third terms by the ratio of the ideal gas constant in energy units, $R_e$, to the ideal gas constant in pressure-volume units, $R_w$.

$$
- V\frac{dP}{dt}  - P  \frac{dV}{dt} \Rightarrow -\frac{R_e}{R_w} \left( V\frac{dP}{dt}  + P  \frac{dV}{dt} \right)
$$

:::

On the right side of @eq-bstr_energy_balance, $\dot Q$ represents the rate of heat input into the reacting fluid. This term is the rate at which heat passes from the yellow exchange fluid in @fig-bstr_ch6_schematic through the red internal wall and into the blue reacting fluid. If the exchange fluid is being used to heat the reacting fluid, $\dot Q$ is positive; if the exchange fluid is being used to cool the reacting fluid, $\dot Q$ is negative.
...
$$
R_w \left( T \left( \sum_i \frac{dn_i}{dt} \right) + \left( \sum_i n_i \right) \frac{dT}{dt} \right) -  P \frac{dV}{dt} - V \frac{dP}{dt} = 0
...
It has already been noted that if the reactor operates adiabatically, there will not be an exchange fluid in the reactor, so it is not possible to write an energy balance on it. By definition, in the energy balance for the reacting fluid, the rate of heat input is equal to zero for an adiabatic process, @eq-bstr_adiabatic.
...
As previously noted, the design equations for a BSTR are initial-value, ordinary differential equations (IVODEs). As detailed in [Appendix -@sec-apndx_solve_ivodes], in order to solve a set of IVODEs, every quantity appearing in the IVODEs must be expressed in terms of known constants, the independent variable and the dependent variables. The independent variable in the BSTR balance equations is time. Depending upon the specific reactor system, the dependent variables may include the molar amounts of the reagents, the temperature of the reacting fluid, the pressure of the reacting fluid, the volume of the reacting fluid, and the exchange fluid temperature.
...
## Ideal SBSTR Design Equations {#sec-ideal_sbstr_eqns}
...
$$
\frac{dn_i}{dt} = \dot{n}_{i,0} + V \sum_j \nu_{i,j}r_j
...
$$
\begin{align}
\sum_i \left( n_i \hat{C}_{p,i}\right) & \frac{dT}{dt} -V\frac{dP}{dt} - P\frac{dV}{dt} = \dot{Q} - \dot{W} \\ &- \sum_i \dot n_{i,0} \int_{T_{0}}^T \hat{C}_{p,i}dT  - V\sum_j r_j \Delta H_j
\end{align}
...
$$
\sum_i \dot{n}_{i,0} \int_{T_{0}}^T \hat{C}_{p,i}dT\ \Leftrightarrow\ \rho \dot{V}_{0} \int_{T_{0}}^T \tilde{C}_pdT\ \Leftrightarrow\ \dot{V}_{0} \int_{T_{0}}^T \breve{C}_pdT
...
## Ideal CSTR Design Equations {#sec-ideal_cstr_eqns}

An ideal CSTR is similar to an ideal BSTR and an ideal SBSTR, with the primary difference being that fluid continuously flows in and out of a CSTR. This is represented schematically in @fig-cstr_ch6_schematic. In that figure a subscripted 0 denotes inlet variables and a subscripted 1 denotes outlet variables. Notice that because the reacting fluid within the reactor is perfectly mixed, the outlet temperature equals the temperature of the reacting fluid within the reactor. Similarly, the concentrations of the reagents leaving the reactor are the same as they are within the reactor. 

![Schematic Representation of a Continuous Stirred Tank Reactor](./Graphics/cstr_non_adiabatic.png){#fig-cstr_ch6_schematic fig-align="center" width="40%"}

The most important assumptions of the ideal CSTR model are (i) the reactor is an open system with fluid continually flowing in and out, (ii) within the reactor the reacting fluid is perfectly mixed, (iii) the reaction rates are normalized per unit volume of reacting fluid, (iv) no phase changes occur upon entering or within the reactor, and (v) the reacting fluid is an ideal mixture. A complete listing of the assumptions and a detailed derivation of the mole and energy balances on the reacting fluid are presented in [Appendix -@sec-apndx_CSTR_balances]. [Example -@sec-example_6_8_4] illustrates writing the reactor design equations for an ideal CSTR.

A mole balance on reagent $i$ in a CSTR is presented in @eq-cstr_mol_balance. In contrast to the BSTR and SBSTR balances, the CSTR mole and energy balances are written in terms of the molar flow rates of the reagents, $\dot n_i$, not the molar amounts, $n_i$. The three terms on the left side of @eq-cstr_mol_balance represent the accumulation of reagent $i$ in the reacting fluid. The first term is accumulation due to a changing outlet molar flow rate of reagent $i$. The second term represents accumulation due to the volume of the reacting fluid changing and the third represents accumulation due to changes in the outlet volumetric flow rate.  The first term on the right side of the equation is the molar flow rate of reagent $i$ into the reactor (denoted by a subscripted 0) and the second term on the right side is its molar flow out of the reactor (denoted by a subscripted 1). The last term represents the moles of reagent $i$ that are generated by the reactions taking place within the reactor.

$$
\frac{V}{\dot{V}_1}\frac{d \dot{n}_{i,1}}{dt} + \frac{\dot{n}_{i,1}}{\dot{V}_1}\frac{dV}{dt} - \frac{\dot{n}_{i,1}V}{\dot{V_1}^2}\frac{d \dot{V_1}}{dt} = \dot{n}_{i,0} - \dot{n}_{i,1} + V \sum_j \nu_{i,j}r_j
$$ {#eq-cstr_mol_balance}

The energy balance on the reacting fluid within an ideal CSTR is presented in @eq-cstr_energy_balance. The terms in that equation have a one-to-one correspondence with the terms in the SBSTR energy balance, @eq-sbstr_energy_balance, representing sensible heat accumulation, two components of the accumulation of energy associated with mechanical work done of expanding or contracting reacting fluid, the rate of heat input into the reacting fluid, the rate at which the reacting fluid does work on the surroundings, the heat required to raise or lower the entering fluid from its feed temperature to the temperature of the reacting fluid, and the energy associated with the reactions taking place within the reactor, respectively. The first term looks different because the ideal CSTR balances are written in terms of molar flow rates and not molar amounts.

$$
\begin{split}
\frac{V}{\dot{V}_1}\sum_i \left( \dot{n}_{i,1} \hat{C}_{p,i}  \right) &\frac{dT_1}{dt} - V \frac{dP}{dt} - P\frac{dV}{dt} = \dot Q - \dot W \\&- \sum_i\dot{n}_{i,0} \int_{T_{0}}^{T_1} \hat{C}_{p,i}dT - V\sum_j r_j \Delta H_j
\end{split}
...
$$
\frac{V}{\dot{V}_1}\sum_i \left( \dot{n}_{i,1} \hat{C}_{p,i}  \right) \frac{dT_1}{dt}\ \Leftarrow\ \rho V \tilde{C}_p \frac{dT_1}{dt}\ \Leftarrow\ V \breve{C}_p \frac{dT_1}{dt}
$$ {#eq-cstr_equivalent_sens_heat_terms}

As noted below, when the reacting fluid in a CSTR is a gas, both the volume and the pressure are constant, so a differential form of the ideal gas law is not needed.

### Common Simplifications of the CSTR Design Equations

When the CSTR walls are rigid a gas phase reacting fluid will always occupy the complete physical volume of the reactor. That means that the volume in the mole and energy balances will be constant and the time-derivative of the volume will equal zero, @eq-bstr_const_volume. Very often this is also true for a liquid phase reacting fluid. However, during transient operation, it is possible for the reacting liquid volume to increase or decrease, along with an equal but opposite change in the headspace. When the liquid is assumed to be an incompressibe, ideal mixture, this only happens if the inlet and outlet volumetric flow rates are not equal.
...
$$
0 = \dot{n}_{i,0} - \dot{n}_{i,1} + V \sum_j \nu_{i,j}r_j
$$ {#eq-cstr_ss_mole_balance}

$$
0 = \dot{Q} - \dot{W} - \sum_i\dot{n}_{i,0} \int_{T_{0}}^{T_1} \hat{C}_{p,i}dT - V\sum_j r_j \Delta H_j
...
## Ideal PFR Design Equations {#sec-ideal_pfr_eqns}
...
$$
\frac{\partial \dot{n}_i}{\partial z} + \frac{\pi D^2}{4\dot{V}} \frac{\partial\dot{n}_i}{\partial t} - \frac{\pi D^2\dot n_i}{4\dot{V}^2} \frac{\partial \dot{V}}{\partial t}  =\frac{\pi D^2}{4}\sum_j \nu_{i,j}r_j
$$ {#eq-pfr_mol_balance}
...
Transient PFRs are not analyzed in *Reaction Engineering Basics*, and for steady-state PFRs, a differential form of the ideal gas law is not needed.
...
$$
\left(\sum_i \dot{n}_i \hat{C}_{p,i} \right) \frac{d T}{d z} = \pi D U\left( T_e - T  \right) - \frac{\pi D^2}{4}\sum_j r_j \Delta H_j
...
In Equations [-@eq-pfr_momentum_balance] and [-@eq-pfr_ergun_eqn] the variable, $G$, is sometimes called the mass velocity. It has units of a mass flux and is equal to the total mass flow rate divided by the cross-sectional area of the PFR. The friction factor, $f$, in @eq-pfr_momentum_balance is typically found using correlations found in handbooks. The friction factor depends upon the Reynolds number and the pipe roughness, among other factors.

## Examples

The examples in this chapter illustrate choosing those design equations that are needed to model the reactors described in the example. In doing so, all summations and continuous products are expanded and any terms that are negligible or equal to zero are eliminated. Solving the design equations presented in these examples would require additional ancillary equations. When the design equations are IVODEs, initial values and a stopping criterion would also be needed. The specification of initial values and a stopping criterion, the generation of the necessary ancillary equations, and the solution of the design equations will be described and discussed in subsequent chapters.

### Modeling an Isothermal BSTR using the Minimum Number of Mole Balances {#sec-example_6_8_1}
...
Mole balances are always needed when modeling a reactor. Here the minimum number of design equations are to be used. There is one reaction taking place and there is an inert present in the system, so the minimum number of mole balances is two. A mole balance on the inert is required. The other mole balance can be for A, B, Y, or Z. Choosing to write the second mole balance on reagent A results in equations (2) and (3).
...
The reactor operates isothermally, and its temperature is known. Consequently the mole balance equations above can be solved independently of any other reactor design equations. Here assignment narrative asks for equations needed to model the composition of the system, so equations (2) and (3) are the only reactor design equations needed.

:::{.callout-note collapse="false"}
## Notes

1. Solving the design equations above will yield the molar amounts of A and I over time. The amounts of the other reagents can be calculated using the stoichiometry of the reaction and the initial amounts of the reagents, @eq-moles_apparent_extent. (It also may be necessary to use this relationship to solve the design equations.)

2. It should also be noted that energy balances were not needed here because the problem specifically stated that the composition of the reactor over time was to be modeled. For an isothermal system where the temperature is known, the mole balances can be solved independently of the energy balances. An alternative assignment might have provided additional information and asked for the coolant flow rate as a function of time that would be necessary to operate the reactor isothermally. For that assignment energy balances on the reacting and exchange fluids would be needed, even though the mole balances still could be solved independently of them.

:::

### Modeling an Adiabatic BSTR {#sec-example_6_8_2}
...
Unlike the previous example, this example does not stipulate that the minimum number of design equations be used. Using the minimum number of design equations is important if they are going to be solved analytically. Nonetheless, even if the minimum number of design equations is used, they often cannot be solved analytically, making a numerical solution necessary. I prefer to include a mole balance for every reagent present in the system and solve the equations numerically. Here the only reagents are A and Z, so @eq-bstr_mole_balance will be written for each of them. There is only one reaction taking place, so the summation in @eq-bstr_mole_balance becomes a single term, and the stoichiometric coefficients of A and Z are -1 and 1, respectively.

:::

**Reactor Design Equations**

Mole balances on reagents A and Z are shown in equations (2) and (3), respectively. An energy balance on the reacting fluid is presented in equation (4). The reactor is adiabatic, so there isn't an exchange fluid to write an energy balance for, and momentum balances are not used with stirred tank reactors.

$$
\frac{dn_A}{dt} = - r_1 V \tag{2}
$$

$$
\frac{dn_Z}{dt} =  r_1 V \tag{3}
$$

$$
\left( n_A \hat C_{p,A} + n_Z \hat C_{p,Z} \right) \frac{dT}{dt} - V\frac{dP}{dt} = - V r_1 \Delta H_1 \tag{4}
$$
...
At present there are 3 reactor design equations, but they contain 4 dependent variables ($n_A$, $n_B$, $T$, and $P$). As noted in [Appendix -@sec-apndx_solve_ivodes], the number of IVODEs must equal the number of dependent variables, so as described in @sec-ideal_bstr_eqns, the differential version of the ideal gas law, @eq-differential_ideal_gas_law, is added to the design equations, setting the time derivative of the volume equal to zero.

$$
R_w \left( T \left( \sum_i \frac{dn_i}{dt} \right) + \left( \sum_i n_i \right) \frac{dT}{dt} \right) - V \frac{dP}{dt} = 0 \tag{5}
$$

:::{.callout-note collapse="false"}
## Note

In this example, the volume was assumed to be constant when writing both the energy balance and the differential form of the ideal gas law. In chemical processing equipment there are almost never moving reactor walls. As a consequence, when the reacting fluid is an ideal gas or an ideal liquid solution, the reactor volume of a BSTR can always be assumed to be fixed, meaning its time derivative may always be assumed to be zero. In *Reaction Engineering Basics*, the assignment would explicitly say so if this was not true.

:::

### Modeling a Cooled SBSTR {#sec-example_6_8_3}
...
The reactor design equations always include mole balances. I will write a mole balance on each of the reagents, A, B, S, and W. The reactor is neither isothermal nor adiabatic, so energy balances on the reacting fluid and on the exchange fluid must also be included among the design equations. A momentum balance is not included because momentum balances are not used with stirred tank reactors.

:::

**Reactor Design Equations**

Mole balances on A, B, S, and W take the form shown in equations (2) through (5).

$$
\frac{dn_A}{dt} = \dot n_{A,0} - r_1V \tag{2}
$$

$$
\frac{dn_B}{dt} = - r_1V \tag{3}
$$

$$
\frac{dn_S}{dt} = r_1V \tag{4}
$$

$$
\frac{dn_W}{dt} = r_1V \tag{5}
$$
...
$$
\frac{dn_i}{dt} = \dot{n}_{i,0} + r_1V
$$

The stoichiometric coefficient of A is -1, leading to equation (2). The inlet stream contains only A, so the inlet molar flow rates of B, S, and W are equal to zero. The stoichiometric coefficients of B, S, and W are -1, 1 and 1, respectively. Substitution of those values yields equations (3), (4), and (5).

:::

Equation (6) is an energy balance on the reacting fluid.

$$
\breve{C}_pV \frac{dT}{dt} - P\frac{dV}{dt} = \dot{Q} - \dot{V}_{in} \breve{C}_p \left( T - T_{0} \right)  - r_1  \Delta H_1 \tag{6}
$$
...
$$
\begin{align}
\breve{C}_pV & \frac{dT}{dt} - V\frac{dP}{dt} - P\frac{dV}{dt} = \dot{Q} - \dot{W} \\ &- \dot{V}_{0} \int_{T_{0}}^T \breve{C}_pdT  - V\sum_j r_j \Delta H_j
\end{align}
$$

The pressure is constant, so its derivative with respect to time is equal to zero. The only work being done is that associated with agitating the liquid, and that may be assumed to be negligible. The heat capacity is constant, so it can be taken outside of the integral and the integral can be evaluated. Again, there is only one reaction occurring, so the summation in the last term reduces to a single term for that reaction.

$$
\begin{align}
\breve{C}_pV & \frac{dT}{dt} - V\cancelto{0}{\frac{dP}{dt}} - P\frac{dV}{dt} = \dot{Q} - \cancelto{0}{\dot{W}} \\ &- \dot{V}_{0} \breve{C}_p \left( T - T_{0} \right)  - r_1 V \Delta H_1
\end{align}
$$

Applying those simplifications results in equation (6).

:::

The exchange fluid is removing energy in the form of sensible heat, so an energy balance on that fluid is given by equation (7).

$$
\rho_{ex} V_{ex} \tilde{C}_{p,ex}\frac{dT_{ex}}{dt} = -\dot{Q} - \dot{m}_{ex} \tilde{C}_{p,ex} \left( T_{ex} - T_{ex,in} \right) \tag{7}
$$
...
At present there are 6 reactor design equations, but they contain 7 dependent variables ($n_A$, $n_B$, $n_S$, $n_W$, $T$, $V$, and $T_{ex}$). The number of IVODEs must equal the number of dependent variables, so another IVODE is needed. This is a liquid phase system, so the differential form of the ideal gas law does not apply. However, if the liquid is assumed to be an ideal mixture, the rate of change of the reacting fluid volume will equal the inlet volumetric flow rate of the feed stream, @eq-sbstr_expansion_term. With the inclusion of equation (8) there are 7 IVODEs and 7 dependent variables.

$$
\frac{dV}{dt} = \dot{V}_{0} \tag{8}
$$
...
This problem indicates that the reactor initially contained a solution containing B and that the feed was a solution containing A. That implies that there is a solvent in the system, but a mole balance for the solvent was not written. This is quite common for liquid phase reacting fluids. It is possible because reaction does not change the amount of solvent and because the heat capacity of the reacting fluid as a whole was used in the energy balance.

:::

### Modeling a Steady-State CSTR where the Exchange Fluid Phase Changes {#sec-example_6_8_4}
...
Mole balances are always included among the design equations, so mole balances for each of the three reagents, A, B, and Z, will be written. The reactor is being heated by the condensation of steam, so energy balances on the reacting fluid and the exchange fluid are also necessary. 

:::

**Reactor Design Equations**

Steady-state mole balances on A, B, and Z are shown in equations (2), (3), and (4).

$$
0 = \dot{n}_{A,0} - \dot{n}_{A,1} - r_1 V \tag{2}
$$

$$
0 = \dot{n}_{B,0} - \dot{n}_{B,1} - r_1 V \tag{3}
$$

$$
0 = \dot{n}_{Z,0} - \dot{n}_{Z,1} + r_1 V \tag{4}
$$
...
$$
\frac{V}{\dot{V}}\frac{d \dot{n}_{i,1}}{dt} + \frac{\dot{n}_{i,1}}{\dot{V}}\frac{dV}{dt} - \frac{\dot{n}_{i,1}V}{\dot{V}^2}\frac{d \dot{V}}{dt} = \dot{n}_{i,0} - \dot{n}_{i,1} + V \sum_j \nu_{i,j}r_j
$$
...
The stoichiometric coefficients of A and B are -1 while that of Z is 1.Making these substitutions leads to equations (2) through (4).

:::

An energy balance on the reacting fluid takes the form shown in equation (5).

$$
0 = \dot{Q} - \rho \dot{V}_{0} \tilde{C}_p  \left( T_1 - T_{0} \right) - r_1 V \Delta H_1 \tag{5}
$$
...
$$
\begin{split}
\frac{V}{\dot{V}}\sum_i \left( \dot{n}_{i,1} \hat{C}_{p,i}  \right) &\frac{dT_1}{dt} - V \frac{dP}{dt} - P\frac{dV}{dt} = \dot{Q} - \dot{W} \\&- \sum_i\dot{n}_{i,0} \int_{T_{0}}^{T_1} \hat{C}_{p,i}dT - V\sum_j r_j \Delta H_j
\end{split}
$$
...
$$
0 = \dot{Q} - \dot{W} - \sum_i\dot{n}_{i,0} \int_{T_{in}}^T \hat{C}_{p,i}dT - V\sum_j r_j \Delta H_j
$$
...
$$
0 = \dot{Q} - \rho \dot{V}_{0} \int_{T_{0}}^{T_1} \hat{C}_{p,i}dT - r_1 V \Delta H_1
$$

Noting that the heat capacity is constant, it can be taken outside the integral and the integral can be evaluated, leading to equation (5).

:::

An energy balance on the steam in the jacket is given by @eq-exchange_energy_bal_latent. Here the reactor operates at steady-state, so the time derivative is equal to zero, resulting in equation (6).

$$
0 = - \dot{Q} - \gamma \dot{m}_{ex} \frac{\Delta H_{\text{latent},ex}^0}{M_{ex}} \tag{6}
$$

:::{.callout-note collapse="false"}
## Note

Assuming that the temperature of the saturated steam is known, the mole and energy balances on the reacting fluid, equations (2) through (5), can be solved independently of the energy balance on the exchange fluid, equation (6). In that situation, equation (6) can be used to calculate the mass flow rate of the steam, given the fraction of the steam that condenses, or the fraction of the steam that condenses, given the mass flow rate of the steam.

:::

### Modeling a Steady-State PFR where the Exchange Fluid Temperature Changes {#sec-example_6_8_5}
...
The reactor design equations always include mole balances, so mole balances will be written for A, A~2~, and Z. The reacting fluid is being heated or cooled using sensible heat from the exchange fluid, so energy balances are needed for the reacting fluid and the exchange fluid. (I know that senisble heat of the exchange fluid is being used because the exchange fluid enters and leaves as a liquid.) The assignment narrative says that the reactor operates with negligible pressure drop, so a momentum balance is not needed.

:::

**Reactor Design Equations**

Mole balances on A, A~2~, and Z take the forms shown in equations (3), (4), and (5).

$$
\frac{d \dot{n}_A}{dz} = \frac{\pi D^2}{4}\left(-2 r_1 - r_2 \right) \tag{3}
$$

$$
\frac{d \dot{n}_{A_2}}{dz} = \frac{\pi D^2}{4}r_1 \tag{4}
$$

$$
\frac{d \dot{n}_Z}{dz} = \frac{\pi D^2}{4}r_2 \tag{5}
$$

:::{.callout-tip collapse="true"}
## Click Here to See Where That Came From

Start with @eq-pfr_mol_balance, expanding the summation to two terms, one for each reaction.
$$
\frac{\partial \dot{n}_i}{\partial z} + \frac{\pi D^2}{4\dot{V}} \frac{\partial\dot{n}_i}{\partial t} - \frac{\pi D^2\dot{n}_i}{4\dot{V}^2} \frac{\partial \dot{V}}{\partial t}  =\frac{\pi D^2}{4}\left(\nu_{i,1}r_1 + \nu_{i,2}r_2\right)
$$

The system operates at steady state, so the time derivatives are equal to zero and the derivative with respect to $z$ becomes an ordinary derivative.

$$
\frac{d \dot{n}_i}{dz} = \frac{\pi D^2}{4} \nu_{i,1}r_1
$$

The stoichiometric coefficients of A, A~2~, and I are -2, 1 and 0 in reaction (1) and -1, 0, and 1 in reaction (2). Substitution of those values leads to equations (3) through (5).
...
$$
\left( \dot{n}_A \hat{C}_{p,A} + \dot{n}_{A_2} \hat{C}_{p,A_2} + \dot{n}_I \hat{C}_{p,I} \right) \frac{d T}{dz} = \pi D U\left( T_{ex} - T  \right) - \frac{\pi D^2}{4} r_1 \Delta H_1 \tag{6}
$$
...
As with the mole balances, the system operates at steady state, so the time derivatives are equal to zero and the derivative with respect to $z$ becomes an ordinary derivative. The summation expands to two terms, one for each reaction.

$$
\left(\sum_i \dot{n}_i \hat{C}_{p,i} \right) \frac{dT}{dz} = \pi D U\left( T_{ex} - T  \right) - \frac{\pi D^2}{4}\left( r_1 \Delta H_1 + r_2 \Delta H_2 \right)
$$

The summation in the sensible heat term must include all reagents in the system, so it expands to three terms, one for A, one for A~2~, and one for Z.

$$
\left( \dot{n}_A \hat{C}_{p,A} + \dot{n}_{A_2} \hat{C}_{p,A_2} + \dot{n}_I \hat{C}_{p,I} \right) \frac{d T}{dz} = \pi D U\left( T_{ex} - T  \right) - \frac{\pi D^2}{4}\left( r_1 \Delta H_1 + r_2 \Delta H_2 \right)
$$
...
$$
0 = -\dot{Q} - \dot{m}_{ex} \tilde{C}_{p,ex} \left( T_{ex} - T_{ex,in} \right) \tag{7}
$$
...
## Symbols Used in @sec-3_react_eng_models
...
| $n_i$ | Moles of reagent $i$; an additional subscripted "0" denotes the initial molar amount and "1" the outlet. |
...
| $R$ | Ideal gas constant; an additional subscripted $e$ denotes energy units and an additional subscripted $w$ denotes pressure-volume units. |
...
| $\dot V$ | Volumetric flow rate of the reacting fluid; an additional subscripted "0" denotes the value at the inlet, and "1," the outlet. |
