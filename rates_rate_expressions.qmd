...
According to either theory, the net rate of elementary reaction $j$ can be written as shown in @eq-net_rate_elem_rxn, where $k_{f,j}$ denotes the rate coefficient for reaction in the forward direction, $k_{r,j}$ denotes the rate coefficient for reaction in the reverse direction, $i_r$ indexes the reactants, and $i_p$ indexes the products. In both theories the forward and reverse rate coefficients are each proportional to the exponential of the negative of an activation energy divided by the product of the gas constant and the absolute temperature, @eq-arrhenius. They differ, however, in the temperature dependence of the pre-exponential factor, $k_0$.
...
Linear least squares parameter estimation (see [Appendix -@sec-apxd_par_est]) is used to determine whether a quantity ($k$ in this case) exhibits Arrhenius temperature dependence. Taking the logarithm of both sides of the Arrhenius expression results in @eq-arrhenius_linearized. Substituting $y = \ln{k}$ and $x = 1/T$ then gives the straight line shown in @eq-arrhenius_straight_line with , $m = -E/R$ and $b=\ln{k_0}$ .
...
$$
\begin{matrix} y = \ln{\left(k \right)} \\ x = \displaystyle \frac{1}{T} \end{matrix} \quad \Rightarrow \quad y = m x + b \quad \Rightarrow \quad \begin{matrix} m = \frac{-E}{R} \\ b = \ln{\left(k_0 \right)} \end{matrix}
$$ {#eq-arrhenius_straight_line}
...
## Learning Objectives and Examples {#sec-ch3_learning_objectives}
