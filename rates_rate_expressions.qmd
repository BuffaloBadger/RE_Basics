...
* be able to
    * renormalize a reaction rate (-@sec-ch3_ex1)
    * convert reaction rates to species generation rates and vice versa (-@sec-ch3_ex1)
    * write a power-law rate expression (-@sec-ch3_ex2)
    * modify a rate expression so that the rate it predicts goes to zero as equilibrium is approached (-@sec-ch3_ex2)
    * estimate Arrhenius expression parameters using experimental data (-@sec-ch3_ex3)
    * test whether a quantity displays Arrhenius temperature dependence given either data or an expression for its dependence on temperature (-@sec-ch3_ex4)

As was the case with @sec-ch2, the information presented in this chapter will be used as one small part of reactor analyses that are performed in later chapters.

### Renormalizing and Rewriting a Species Generation Rate as a Reaction Rate{#sec-ch3_ex1}
...
### Ensuring a Rate Expression Predicts a Net Rate of Zero at Equilibrium{#sec-ch3_ex2}
...
### Estimating the Pre-exponential Factor and Activation Energy in the Arrhenius Expression {#sec-ch3_ex3}
...
The calculations were performed as described, yielding a coefficient of determination, $R^2$, equal to 0.992. The best estimate for $k_0$ is 6.97 $\times$ 10^6^ L mol^–1^ min^–1^, 95% CI [1.01 $\times$ 10^6^, 4.79 $\times$ 10^7^], and that for $E$ is 57.3 kJ mol^-1^, 95% CI [52, 62.5].
...
### Representing a Collision Theory Rate Coefficient Using the Arrhenius Expression {#sec-ch3_ex4}
