...
The first example here considers a liquid-phase reaction. Assuming liquids to be incompressible, ideal mixturs means that the inlet and outlet volumetric flow rates are equal. In contrast, for a gas-phase reacting fluid the inlet and outlet volumetric flow rates will not be equal if the reaction causes a change in the total number of moles. That situation is illustrated in [Example -@sec-example_20_5_2]. The final example in this chapter illustrates the assessment of a rate expression that contains both a rate coefficient and an equilbrium constant.
...
Eventually I will need to calculate the model-predicted response using the results from solving the CSTR design equation. The response is related to the outlet composition, so I'll solve the CSTR design equatiions for the outlet molar flow rates of A, B, Y, anc Z. The design equations are ATEs, so to solve them numerically, I'll need to write a residuals function. The residuals function will be given values for those outlet molar flow rates, and it will have access to the given and known constants in the assignment summary. It must evaluate the residuals in equations (3) through (6) and return their values. Before that can be done, any other unknown quantities appearing in equations (3) through (6) must be calculated.

Examining equations (3) through (6), I can see that there are five additional unknown quantities: the four inlet molar flow rates and the reaction rate. I cannot calculate the inlet molar flow rates using information available to the residuals function. In addition, the inlet molar flow rates will vary from one experiment to the next. However, if I am given the volumetric flow rate and the inlet concentrations for any one experiment (i. e. the adjusted experimental inputs), I can calculate the inlet molar flow rates using the definition of concentration for a flow system, @eq-concentration_open.

$$
C_i = \frac{\dot{n}_i}{\dot{V}} \qquad \Rightarrow \qquad \dot{n}_i = C_i \dot{V}
$$

The rate can be calculated using equation (2) wherein the outlet concentrations are used. To calculate the concentrations I can again use the definition of concentration, @eq-concentration_open. However, it isn't possible to calculate the rate coefficient using the information provided to the residuals function, so the rate coefficient will need to be provided to the residuals function.

:::

[Required Input:]{.underline} $\dot{V}$, $C_{A,0}$, $C_{B,0}$, $C_{Y,0}$, $C_{Z,0}$, $k$, $C{Y,1}$ (for generating a guess).
...
$$
\dot{n}_{A,0} = C_{A,i0}\dot{V} \tag{7}
$$
...
$$
C_{A,1} = \frac{\dot{n}_{A,1}}{\dot{V}} \tag{11}
$$

$$
C_{B,1} = \frac{\dot{n}_{B,1}}{\dot{V}} \tag{12}
$$
...
I'll solve the CSTR design equations using an ATE solver. I'll need to provide it with the name of the residuals function I write, above, and a guess for the ATE unknowns. I'll be solving the design equations for the outlet molar flow rates of the reagents for each of the experiments. In those experiments, the outlet concentration of Y was measured as the response. Hence, if that response is provided, I can use it to generate a guess.

The outlet molar flow rate of Y can be calculated from its outlet concentration, and the inlet molar flow rates of A, B, Y, and Z, can be calculated as above. The outlet molar flow rate of Y can then be used to calculate the extent of reaction, and the extent of reaction can be used to calculate the other outlet molar flow rates.

The results returned by the solver should be checked to make sure that it converged to the desired steady state. (Multiple steady states are possible, but when the temperature is being controlled at a constant value, they will only be encountered if the rate expression is non-linear.)

:::

[Solving the Steady-State CSTR Design equations]{.underline}

Given the experimental response from an experiment, included in the required input above, a guess for the ATE unknowns can be generated using the following sequence of calculations.
...
To estimate $k$, the response function described above was passed to parameter estimation software along with the adjusted experimental inputs, the experimental responses and a guess for $k$. The results returned by that software are shown in @tbl-example_20_5_1_results.
