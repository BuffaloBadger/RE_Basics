...
This section of *Reaction Engineering Basics* focuses upon the analysis of kinetics data for the purpose of assessing the accuracy of a proposed rate expression. @sec-6_kin_data_gen presented a general introduction to kinetics data analysis and @sec-6_bstr_data_analysis examined the analysis of kinetics data generated using a BSTR. This chapter is like @sec-6_bstr_data_analysis except it is focused on the analysis of kinetics data generated using a CSTR.
...
The start of the Examples section of @sec-6_bstr_data_analysis noted that before a reaction engineer can solve a problem, they must identify what type of problem it is. The following callout was provided with the suggestion that the reader mentally insert it at the start of every example in that chapter *and* in @sec-6_cstr_data_analysis and @sec-6_pfr_data_analysis.
...
It should also be noted that each of the examples in this chapter present only a single constant-temperature block of data and ask that those data be used to assess the accuracy of a rate expression with respect to the concentration dependence of a rate expression. This is intentional. Two of the four examples in @sec-6_bstr_data_analysis illustrated the second phase of the analysis where the Arrhenius expression was fit to the rate coefficients found by fitting the reactor model to the experimental data. By now, the reader should know how to fit the Arrhenius expression to $k$ *vs*. $T$ data. The examples here focus exclusively on fitting a CSTR reactor model to CSTR kinetics data with the assumption that the reader already knows how to process the resulting rate coefficients and estimate their pre-exponential factor and activation energy.

### Analysis of Data for Liquid-Phase Reaction {#sec-example_20_1}

{{< include ../RE_Basics_Examples/reb_20_1/problem_statement.qmd >}}

```{r}
#| echo: false
#| output: false
path_to_data <- './Data/'
path_to_results <- '../RE_Basics_Examples/reb_20_1/Results/'
path_to_figures <- './Graphics/'
d <- read.csv(paste0(path_to_data,"reb_20_1_data.csv"))
 # VFR, CAin, CBin, CYin, CZin, CYout
```

The first few data points are shown in @tbl-example_20_1_data. The full data set is available in the file [reb_20_1_data.csv](`r paste0(path_to_data,'reb_20_1_data.csv')`).
...
#| echo: false
#| output: false
df <- read.csv(paste0(path_to_results,"reb_20_1_results.csv"))
...
To estimate $k$, the response function described above was passed to parameter estimation software along with the adjusted experimental inputs, the experimental responses and a guess for $k$. The results returned by that software are shown in @tbl-example_20-1_results.

```{r}
#| echo: false
#| warning: false
#| label: tbl-example_20-1_results
#| tbl-cap: Parameter estimation results
kbl(d, format = "markdown", align = "cc", col.names = NULL)
```

The estimated rate coefficient was used to calculate the model-predicted response for each experiment, and that was used to calculate the residual for each experiment. The results were then used to generate the parity plot and residuals plots shown in @fig-example_20_1_plots.

::: {#fig-example_20_1_plots layout-ncol=2}

![](`r paste0(path_to_figures,'reb_20_1_parity.png')`)

![](`r paste0(path_to_figures,'reb_20_1_VFR_residuals.png')`)

![](`r paste0(path_to_figures,'reb_20_1_CA_residuals.png')`)

![](`r paste0(path_to_figures,'reb_20_1_CB_residuals.png')`)

![](`r paste0(path_to_figures,'reb_20_1_CY_residuals.png')`)

![](`r paste0(path_to_figures,'reb_20_1_CZ_residuals.png')`)
...
### Analysis of Data for a Gas-Phase Reaction with Change of Total Moles {#sec-example_20_2}

{{< include ../RE_Basics_Examples/reb_20_2/problem_statement.qmd >}}

```{r}
#| echo: false
#| output: false
path_to_data <- './Data/'
path_to_results <- '../RE_Basics_Examples/reb_20_2/Results/'
path_to_figures <- './Graphics/'
df <- read.csv(paste0(path_to_data,"reb_20_2_data.csv"))
 # tau, yAin, CZout
```

The first few data points are shown in @tbl-example_20_2_data. The full data set is available in the file [reb_20_2_data.csv](`r paste0(path_to_data,'reb_20_2_data.csv')`).

```{r}
#| echo: false
#| warning: false
#| label: tbl-example_20_2_data
...
#| echo: false
#| output: false
df <- read.csv(paste0(path_to_results,"reb_20_2_results.csv"))
...
The reactor model described above was fit to the experimental data presented in @tbl-example_20_2_data using parameter estimation software. The results are presented in @tbl-example_20_2_parameters. A parity plot is shown in @fig-example_20_2_parity and residuals plots are shown in @fig-example_20_2_residuals_plots.

```{r}
#| echo: false
#| warning: false
#| label: tbl-example_20_2_parameters
#| tbl-cap: Parameter estimation results
kbl(d, format = "markdown", align = "cc", col.names = NULL)
```

![Parity plot.](`r paste0(path_to_figures,'reb_20_2_parity.png')`){#fig-example_20_2_parity}

::: {#fig-example_20_2_residuals_plots layout-ncol=2}

![](`r paste0(path_to_figures,'reb_20_2_tau_residuals.png')`)

![](`r paste0(path_to_figures,'reb_20_2_yA_residuals.png')`)
...
When used with the rate expression parameters shown in @tbl-example_20_2_parameters, the accuracy of the proposed rate expression in describing the concentration dependence of the rate at 450 °C is acceptable.


### Analysis of Data for a Reversible Reaction {#sec-example_20_3}

{{< include ../RE_Basics_Examples/reb_20_3/problem_statement.qmd >}}

```{r}
#| echo: false
#| output: false
path_to_data <- './Data/'
path_to_results <- '../RE_Basics_Examples/reb_20_3/Results/'
path_to_figures <- './Graphics/'
df <- read.csv(paste0(path_to_data,"reb_20_3_data.csv"))
 # VFR, CAin, CYin, CZin, CA
```

The first few data points are shown in @tbl-example_20_3_data. The full data set is available in the file [reb_20_3_data.csv](`r paste0(path_to_data,'reb_20_3_data.csv')`).

```{r}
#| echo: false
#| warning: false
#| label: tbl-example_20_3_data
#| tbl-cap: First 6 experimental data points.
...
#| echo: false
#| output: false
df <- read.csv(paste0(path_to_results,"reb_20_3_results.csv"))
df <- fmt_tibble_col(df, 2, 3, 2, 2)
a <- c('k~f~','k~r~','R^2^')
b1 <- paste0(df$value[1],' ',df$units[1],', 95% CI [',df$value[2],', ',df$value[3],']')
b2 <- paste0(df$value[4],' ',df$units[4],', 95% CI [',df$value[5],', ',df$value[6],']')
...
To estimate the kinetics parameters, the response function described above was passed to parameter estimation software along with the adjusted experimental inputs, the experimental responses and a guess for the kinetics parameters. The results returned by that software are shown in @tbl-example_20_3_results.

```{r}
#| echo: false
#| warning: false
#| label: tbl-example_20_3_results
#| tbl-cap: Parameter estimation results
kbl(d, format = "markdown", align = "cc", col.names = NULL)
```

The results from the parameter estimation were then used to create the parity plot shown in @fig-example_20_3_parity and residuals plots shown in @fig-example_20_3_residuals_plots as described in @sec-6_kin_data_gen.

![Parity plot.](`r paste0(path_to_figures,'reb_20_3_parity.png')`){#fig-example_20_3_parity}

::: {#fig-example_20_3_residuals_plots layout-ncol=2}

![](`r paste0(path_to_figures,'reb_20_3_VFR_residuals.png')`)

![](`r paste0(path_to_figures,'reb_20_3_CA_residuals.png')`)

![](`r paste0(path_to_figures,'reb_20_3_CY_residuals.png')`)

![](`r paste0(path_to_figures,'reb_20_3_CZ_residuals.png')`)
...
When the rate coefficients in @tbl-example_20_3_results are used in the rate expression, equation (2), it provides an acceptably accurate representation of the experimental results.
