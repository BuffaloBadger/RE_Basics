...
Because elementary reactions are exact descriptions of a single molecular event, it is possible to develop theories for how they occur. In this appendix, two theories for the rates of elementary reactions are presented. One is known as collition theory and the other, as transition state theory.

## Collision Theory {#sec-apndx_collision_theory}

Collision theory is based upon the kinetic theory of gases, so clearly, it applies to elementary gas phase reactions. In essence, the rate is assumed to equal the number of sufficiently energetic collisions between the reactants per unit time. The rate at which reactants collide is calculated using the kinetic theory of gases.
...
Transition state theory is based upon the concept of an *activated complex*. The activated complex can be thought of as a molecular configuration that is intermediate between that of the reactants and that of the products. The theory assumes that the activated complex is in a kind of equilibrium with the reactants, and uses molecular thermodynamics to describe that equilibrium.
...
## Summary: Rate Expressions for Elementary Reactions

This appendix has shown that both kinetic theories predict rate expressions for elementary reactions that have the common form shown in @eq-elem_rate_expr_common_form_C. The theories differ in the temperature dependence of the rate coefficient. Both, however, feature an Arrhenius-type term within the rate coefficient. In many situations, it is safe to assume that term to predominate so that the temperature dependence of the rate coefficient is given by the Arrhenius expression.

$$
r_j = k_{j} \prod_{i_r} C_{i_r}^{-\nu_{{i_r},j}} - \frac{k_{j}}{K_{j,eq_c}} \left(\prod_{i_p} C_{i_p}^{\nu_{{i_p},j}}\right)
$${#eq-elem_rate_expr_common_form_C}

For gas-phase reactions, the concentration and pressure are related through the ideal gas law, $C=\frac{P}{RT}$. Substituting in @eq-elem_rate_expr_common_form_C yields @eq-elem_rate_expr_common_form_P, where the $RT$ has been incorporated in the rate coefficient and the pressure form of the equilibrium constant appears. Again, in many situations, Arrhenius temperature dependence of the rate coefficient can be used.

$$
r_j = k_{j} \prod_{i_r} P_{i_r}^{-\nu_{{i_r},j}} - \frac{k_{j}}{K_{j,eq_p}} \left(\prod_{i_p} P_{i_p}{\nu_{{i_p},j}}\right)
$${#eq-elem_rate_expr_common_form_P}
