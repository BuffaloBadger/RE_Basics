...
There are two heat exchanger design equations (9) and (10). That means I can solve them to find two unknown quantities. Examining those equations I see that they contain three unknown quantities: $T_1$, $T_2$, and $T_3$. As described in @sec-apndx_solve_cvodes,  I will solve the heat exchanger design equations, (9) and (10), for $T_1$ and $T_3$. To do so numerically, I will have to (a) provide an initial guess for those unknowns and (b) evaluate the residuals. At the point where I need to evaluate the residuals, I will have a guess for $T_1$ and $T_3$. So, to evaluate the residuals I'll need to calculate any other unknown quantities that appear in them. In this case, the only other unknown is $T_2$. $T_1$ is one of the initial values I need for solving the PFR design equations, so using the guess for $T_1$, I can solve the PFR design equations to find the outlet temperature, $T_2$.
...
There are two heat exchanger design equations (10) and (11). That means I can solve them to find two unknown quantities. Examining those equations I see that they contain three unknown quantities: $T_1$, $T_2$, and $T_3$. As described in @sec-apndx_solve_cvodes,  I will solve the heat exchanger design equations, (9) and (10), for $T_1$ and $T_3$. To do so numerically, I will have to (a) provide an initial guess for those unknowns and (b) evaluate the residuals. At the point where I need to evaluate the residuals, I will have a guess for $T_1$ and $T_3$. So, to evaluate the residuals I'll need to calculate any other unknown quantities that appear in them. In this case, the only other unknown is $T_2$. $T_1$ is one of the initial values I need for solving the PFR design equations, so using the guess for $T_1$, I can solve the PFR design equations to find the outlet temperature, $T_2$.
