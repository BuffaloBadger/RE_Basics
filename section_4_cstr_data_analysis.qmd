...
The first few data points are shown in @tbl-example_10_1_data. The full data set is available in the file [reb_10_1_data.csv](Data/reb_10_1_data.csv).
...
library(tidyverse)
source("~/Libraries/R/fmt_tibble_col.R")
d <- read.csv("../RE_Basics_Examples/reb_10_1/Results/reb_10_1_results.csv")
d <- fmt_tibble_col(d, 2, 3, 2, 2)
k <- paste0(d$value[1],' ',d$units[1],', 95% CI [',d$value[2],', ',d$value[3],']')
```

The reactor model was fit to the experimental data in @tbl-example_10_1_data. The resulting estimate for the rate coefficient is `r k`, and the coefficient of determination is `r d$value[4]`. The estimated rate coefficient was used to calculate the model-predicted response for each experiment, and that was used to calculate the residual for each experiment. The results were then used to generate the parity plot and residuals plots shown in @fig-example_10_1_plots.

::: {#fig-example_10_1_plots layout-ncol=2}

![](Graphics/reb_10_1_parity.png)

![](Graphics/reb_10_1_VFR_residuals.png)

![](Graphics/reb_10_1_CA_residuals.png)

![](Graphics/reb_10_1_CB_residuals.png)

![](Graphics/reb_10_1_CY_residuals.png)

![](Graphics/reb_10_1_CZ_residuals.png)
...
The first few data points are shown in @tbl-example_10_2_data. The full data set is available in the file [reb_10_2_data.csv](Data/reb_10_2_data.csv).
...
As detailed in @sec-4_kin_data_gen, having estimated the values of the parameters, I can calculate the model-predicted response for every experiment. Then I can calculate the residual (difference between the experimental response and the model-predicted response) for each experiment. Then a parity plot and residuals plots can be generated. The following criteria will be satisifed if the model is accurate.
...
library(tidyverse)
source("~/Libraries/R/fmt_tibble_col.R")
d <- read.csv("../RE_Basics_Examples/reb_10_2/Results/reb_10_2_results.csv")
d <- fmt_tibble_col(d, 2, 3, 2, 2)
...
The reactor model described above was fit to the experimental data presented in @tbl-example_10_2_data. The results are presented in @tbl-example_10_2_parameters. A parity plot is shown in @fig-example_10_2_parity and residuals plots are shown in @fig-example_10_2_residuals_plots.
...
![Parity plot.](Graphics/reb_10_2_parity.png){#fig-example_10_2_parity}

::: {#fig-example_10_2_residuals_plots layout-ncol=2}

![](Graphics/reb_10_2_tau_residuals.png)

![](Graphics/reb_10_2_yA_residuals.png)
...
library(tidyverse)
source("~/Libraries/R/fmt_tibble_col.R")
d <- read.csv("Data/reb_10_3_data.csv")
 # VFR, CAin, CYin, CZin, CA
```

The first few data points are shown in @tbl-example_10_3_data. The full data set is available in the file [reb_10_3_data.csv](Data/reb_10_3_data.csv).
...
A response function can be written to solve the reactor design equations as described above. The resulting value of $\dot{n}_A$ can then be used to predict the outlet concentration of A, the response, using equation (9).
...
library(tidyverse)
source("~/Libraries/R/fmt_tibble_col.R")
d <- read.csv("../RE_Basics_Examples/reb_10_3/Results/reb_10_3_results.csv")
d <- fmt_tibble_col(d, 2, 3, 2, 2)
a <- c('k~f~','k~r~','R^2^')
b1 <- paste0(d$value[1],' ',d$units[1],', 95% CI [',d$value[2],', ',d$value[3],']')
b2 <- paste0(d$value[4],' ',d$units[4],', 95% CI [',d$value[5],', ',d$value[6],']')
...
![Parity plot.](Graphics/reb_10_3_parity.png){#fig-example_10_3_parity}

::: {#fig-example_10_3_residuals_plots layout-ncol=2}

![](Graphics/reb_10_3_VFR_residuals.png)

![](Graphics/reb_10_3_CA_residuals.png)

![](Graphics/reb_10_3_CY_residuals.png)

![](Graphics/reb_10_3_CZ_residuals.png)
...
When the rate coefficients in @tbl-example_10_3_results are used in the rate expression, equation (2), it provides an accurate representation of the experimental results. It should be accepted.
