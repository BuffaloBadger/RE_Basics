# Transition State Theory  {#sec-transition-state-theory}
...
![Constrained three atom system where all atoms lie on a common straight line.](./Graphics/TST_linear_molecule.pdf){#fig-linear-molecule}
...
![A schematic representation of a potential energy surface for the reaction given in @eq-tst-reaction.](./Graphics/TST_hypersurface.pdf){#fig-hypersurface}
...
$$
q_{vib,i}=q_{v_1,i}q_{v_2,i}q_{v_3,i}q_{v_4,i}
$$ {#eq-vib-part-func-components}

$$
q_{v_n,i} = \frac{1}{1 - \exp{\left( \frac{-h\nu_n}{k_BT} \right)}}
$$ {#eq-vib-part-func}
...
$$
q_{\text{ABC}^\ddagger} = q_{tr,\text{ABC}^\ddagger}q_{rot,\text{ABC}^\ddagger}\left(\prod_{n=1}^{N_{vib}}  q_{v_n,\text{ABC}^\ddagger}\right)
...
$$
q_{\text{ABC}^\ddagger} = q_{tr,\text{ABC}^\ddagger}q_{rot,\text{ABC}^\ddagger}\left(\prod_{\substack{n=1\\n\ne c}}^{N_{vib}}  q_{v_n,\text{ABC}^\ddagger}\right)\frac{k_BT}{h\nu_c}
...
$$
q_{\ddagger} = \frac{q_{\text{ABC}^\ddagger}}{\left(\frac{-h\nu_c}{k_BT}\right)}=q_{tr,\text{ABC}^\ddagger}q_{rot,\text{ABC}^\ddagger}\left(\prod_{\substack{n=1\\n\ne c}}^{N_{vib}}  q_{v_n,\text{ABC}^\ddagger}\right)
$$ {#eq-part-minus-crit}
...
$$
r_j = k_{j,f}\prod_{i=1}^{N_{\text{Reactants}}} C_i^{-\nu_{i,j}} - k_{j,r}\prod_{i=1}^{N_{\text{Reactants}}} C_i^{\nu_{i,j}}
$$ {#eq-tst-gen-net-rate}
...
| $q_{v_j,i}$ | Partition function for vibrational degree of freedom $j$ for reagent $i$ per unit volume. |
