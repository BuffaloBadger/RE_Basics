...
Assignments that entail isolated ideal reactor analysis can be identified by the following distinguishing characteristics.

* The system consists of one ideal reactor and no other equipment.
* The assignment describes the reactor and provides information about its operation.
* The rate expressions are known for all reactions taking place in the reactor.
* The quantities of interest in the assignment are reactor parameters, process parameters, reactor inputs and/or reactor outputs.
...
3. Draw sets of axes for each graph with the quantity of interest on the y-axis and reaction time or space time on the x-axis.
...
In **optimization tasks** the number of reactor variables that are specified or known is *not* sufficient to allow the reaction engineer to solve the reactor design equations and calculate the quantities of interest. The simplest example would task the engineer to maximize a quantity of interest (the target) with respect to some other reactor variable (the adjusted variable). Neither the value of the targe nor the value of the adjusted variable are known. In lieu of knowing their actual values, the reaction engineer instead knows that it is the maximum value that the target can be attained by varying the adjusted variable. Typically the engineer is tasked to find both the maximum value of the target and the corresponding value of the adjusted variable. Again, generally there is a single correct answer to a reactor optimization assignment.
...
The second second differs in that instead of tabulating the results from solving the reactor design equations, they are plotted. The maximum in the graph is identified and the maximum value of the target and the corresponding value of the adjusted variable are read from the graph. In both of these approaches, it is possible that a maximum will not be present. In that case, the range of values of the adjusted variable must be increased until a maximum is observed. Once a maximum is observed, the precision of the results can be improved by adding many additional values of the adjusted variable between the two points that bracket the maximum.
...
The next two Chapters, [-@sec-4_bstr_analysis] and [-@sec-4_sbstr_analysis], examine the analysis of non-continuous reactors and present examples involving response and optimization tasks. That affords the reader an opportunity to gain some understanding of the qualitative and quantitative differences between BSTRs and SBSTRs, which is necessary for the design of non-continuous reactors in the following Chapter, [-@sec-4_non_continuous_design]. Following that, analysis of continuous reactors is considered in Chapters [-@sec-4_cstr_analysis] and [-@sec-4_pfr_analysis]. Similarly, they afford the reader an opportunity to gain some understanding of the qualitative and quantitative differences between CSTRs and PFRs, which is necessary for the design of continuous reactors in @sec-4_continuous_design, which also considers the choice between continuous and non-continuous reactors.