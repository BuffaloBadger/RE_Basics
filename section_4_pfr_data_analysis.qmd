...
This chapter completes the Kinetics Data Analysis section of *Reaction Engineering Basics*. It focuses on the generation and analysis of kinetics data for a PFR, as defined in @sec-3_design_eqns and [Appendix -@sec-apndx_ideal_reactor_models]. @sec-4_kin_data_gen presented an overview of the data generation and analysis processes and Chapters [-@sec-4_bstr_data_analysis] and [-@sec-4_cstr_data_analysis] examined BSTRs and CSTRs, respectively. Much of the information presented in those chapters also applies to PFRs and won't be duplicated here.
...
Finally, **if the reactor was operated differentially when generating the data**, an approximate mole balance can be used, @eq-differential_pfr_mole_bal_one_rxn. This is completely analogous to the approximate mole balance used for analysis of BSTR data. It converts the mole balance from an IVODE to an ATE. The derivative can be approximated using backward, forward or central differences (see [Appendix -@sec-apndx_pre_knowledge]). The approximation of the derivative as the ratio, $\frac{\Delta \dot n_i}{\Delta z}$, is only valid when the changes in $\dot n_i$ and $z$ are small. That is why it is necessary to operate the reactor differentially for this type of analysis.
...
The start of the Examples sections of Chapters [-@sec-4_bstr_data_analysis] and [-@sec-4_cstr_data_analysis] noted that before one can solve a problem, they must identify what type of problem it is. The following callout was provided. It should be mentally insert it at the start of every example this chapter.
...
It was noted earlier in this chapter that each of the examples here illustrate the analysis of a single constant-temperature data block. Doing that yields values of the kinetics parameters at that temperature. The second phase of the analysis where the Arrhenius expression is fit to the rate coefficients was illustrated in @sec-4_bstr_data_analysis and is not repeated here. However see the notes following Examples [-@sec-example_11_2] and [-@sec-example_11_3]. As also noted previously in this chapter, analysis using a linearized reactor model and an approximate reactor model were illustrated in @sec-4_bstr_data_analysis and are not repeated here.

### Kinetics of Gas-Phase Decomposition {#sec-example_11_1}
...
d <- read.csv("./Data/reb_11_1_data.csv")
 # A_in, Y_in, Z_in, fA
```
...
library(tidyverse)
source("./Code/fmt_tibble_col.R")
d <- read.csv("../RE_Basics_Examples/reb_11_1/Results/reb_11_1_results.csv")
...
The reactor model described above was fit to the experimental data. The results are presented in @tbl-example_11_1_results.
...
The estimated rate coefficient was used in the reactor model to calculate the model-predicted response and the residual for every experiment. The results were used to generate the parity plot and the residuals plots shown in @fig-example_11_1_plots.

::: {#fig-example_11_1_plots layout-ncol=2}

![](./Graphics/reb_11_1_parity.png)

![](./Graphics/reb_11_1_residuals_vs_VFR_A.png)

![](./Graphics/reb_11_1_residuals_vs_VFR_Y.png)

![](./Graphics/reb_11_1_residuals_vs_VFR_Z.png)
...
library(tidyverse)
source("./Code/fmt_tibble_col.R")
d <- read.csv("./Data/reb_11_2_data.csv")
 # yA, yB, yY, yZ, PA
```
...
library(tidyverse)
source("./Code/fmt_tibble_col.R")
d <- read.csv("../RE_Basics_Examples/reb_11_2/Results/reb_11_2_results.csv")
d <- fmt_tibble_col(d, 2, 3, 2, 2)
...
The reactor model described above was fit to the experimental data. The results are presented in @tbl-example_11_2_results.
...
The estimated rate coefficient was used in the reactor model to calculate the model-predicted response and the residual for every experiment. The results were used to generate the parity plot shown in @fig-example_11_2_parity and the residuals plots shown in @fig-example_11_2_residuals_plots.

![Parity Plot.](./Graphics/reb_11_2_parity.png){#fig-example_11_2_parity width=80%}

::: {#fig-example_11_2_residuals_plots layout-ncol=2}

![](./Graphics/reb_11_2_residuals_vs_yAin.png)

![](./Graphics/reb_11_2_residuals_vs_yBin.png)

![](./Graphics/reb_11_2_residuals_vs_yYin.png)

![](./Graphics/reb_11_2_residuals_vs_yZin.png)
...
The first few data points are shown in @tbl-example_11_3_data; the full data set are available in the file, [reb_11_2_data.csv](./Data/reb_11_2_data.csv). This is the same data set as that used in [Example -@sec-example_11_2].
...
The formulation and solution of the reactor model and the calculation of the model-predicted response for this problem are exactly the same as [Example -@sec-example_11_2] except that the rate expression is different (but still depends upon the same composition variables) and the kinetics parameters are $k$, $K_A$, $K_B$, $K_Y$, and $K_Z$. 
