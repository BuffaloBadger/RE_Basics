# Analysis of Kinetics Data from a PFR {#sec-4_pfr_data_analysis}

This chapter completes the Kinetics Data Analysis section of *Reaction Engineering Basics*. It focuses on the generation and analysis of kinetics data from a PFR. @sec-4_kin_data_gen presented an overview of kinetics data generation and analysis, and Chapters [-@sec-4_bstr_data_analysis] and [-@sec-4_cstr_data_analysis] examined analysis of data from BSTRs and CSTRs, respectively. Much of the information presented in those chapters also applies to PFRs and won't be duplicated here.
...
The start of the Examples sections of Chapters [-@sec-4_bstr_data_analysis] and [-@sec-4_cstr_data_analysis] noted that before a reaction engineer can solve a problem, they must identify what type of problem it is. The following callout was provided. It should be mentally insert it at the start of every example this chapter.
...
#| output: false
path_to_data <- './Data/'
path_to_results <- '../RE_Basics_Examples/reb_10_1/Results/'
...
Mole balances on A, Y, and Z are shown in equations (3), (4), and (5). The initial values and stopping criterion needed to solve them are given in @tbl-example_10_1_initial_values
...
: Initial values and stopping criterion for solving the design equations, (3), (4), (5). {#tbl-example_10_1_initial_values tbl-colwidths="[20, 40, 40]"}
...
* For each experiment, the response function 
    ...
    * Solves the design equations using the initial values and stopping criterion in @tbl-example_10_1_initial_values to find $\dot{n}_A$, $\dot{n}_Y$, and $\dot{n}_Z$ at $z=L$.
...
#| echo: false
#| output: false
d <- read.csv(paste0(path_to_results, 'reb_10_1_results.csv'))
...
To estimate $k$, the response function as described above was passed to parameter estimation software along with the adjusted experimental inputs, the measured experimental responses, and a guess for $k$. The results returned by that software are shown in @tbl-example_10_1_results.
...
A parity plot and residuals plots were generated as described above, and are shown in @fig-example_10_1_plots

::: {#fig-example_10_1_plots layout-ncol=2}

![](`r paste0(path_to_figures, 'reb_10_1_parity.png')`)

![](`r paste0(path_to_figures, 'reb_10_1_residuals_vs_VFR_A.png')`)

![](`r paste0(path_to_figures, 'reb_10_1_residuals_vs_VFR_Y.png')`)

![](`r paste0(path_to_figures, 'reb_10_1_residuals_vs_VFR_Z.png')`)
...
When the rate coefficient in @tbl-example_10_1_results is used in the proposed rate expression, it is acceptably accurate. The data do not indicate any functional dependence of the rate upon the paritial pressures of Y or Z.
...
#| output: false
path_to_data <- './Data/'
path_to_results <- '../RE_Basics_Examples/reb_10_2/Results/'
path_to_figures <- './Graphics/'
d <- read.csv(paste0(path_to_data,"reb_10_2_data.csv"))
 # A_in, Y_in, Z_in, fA
```

The first few data points are shown in @tbl-example_10_2_data; the full data set are available in the file, [reb_10_2_data.csv](`r paste0(path_to_data,'reb_10_2_data.csv')`).
...
The design equations are IVODEs, so initial values and a stopping criterion are needed in order to solve them. At the inlet to the reactor, the catalyst mass is equal to zero, $m=0$, and at the end of the reactor it is equal to $m_{\text{cat}}$. Letting $\dot{n}_{A,in}$, $\dot{n}_{B,in}$, $\dot{n}_{Y,in}$, and $\dot{n}_{Z,in}$ represent the molar flow rates at the reactor inlet then leads to the initial values and stopping criterion shown in @tbl-example_10_2_initial_values.

:::

**Reactor Model**

Mole balances on A, B, Y, and Z, are presented in equations (3) through (6), and the corresponding initial values and stopping criterion are shown in @tbl-example_10_2_initial_values.
...
: Initial values and stopping criterion for solving the design equations, equations (3) through (6). {#tbl-example_10_2_initial_values tbl-colwidths="[20, 40, 40]"}
...
* For each experiment, the response function 
    ...
    * Solves the design equations using the initial values and stopping criterion in @tbl-example_10_2_initial_values to find $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_Y$, and $\dot{n}_Z$ at the reactor outlet where $m = m_{\text{cat}}$.
...
#| echo: false
#| output: false
d <- read.csv(paste0(path_to_results,'reb_10_2_results.csv'))
...
![Parity Plot.](`r paste0(path_to_figures,'reb_10_2_parity.png')`){#fig-example_10_2_parity width=80%}

::: {#fig-example_10_2_residuals_plots layout-ncol=2}

![](`r paste0(path_to_figures,'reb_10_2_residuals_vs_yAin.png')`)

![](`r paste0(path_to_figures,'reb_10_2_residuals_vs_yBin.png')`)

![](`r paste0(path_to_figures,'reb_10_2_residuals_vs_yYin.png')`)

![](`r paste0(path_to_figures,'reb_10_2_residuals_vs_yZin.png')`)
...
### Mechanistic Kinetics of a Heterogeneous Catalytic Reaction {#sec-example_10_3}

{{< include ../RE_Basics_Examples/reb_10_3/problem_statement.qmd >}}

```{r}
#| echo: false
#| output: false
path_to_data <- './Data/'
path_to_results <- '../RE_Basics_Examples/reb_10_3/Results/'
path_to_figures <- './Graphics/'
d <- read.csv(paste0(path_to_data,"reb_10_2_data.csv"))
 # A_in, Y_in, Z_in, fA
```

The first few data points are shown in @tbl-example_10_3_data; the full data set are available in the file, [reb_10_2_data.csv](`r paste0(path_to_data,'reb_10_2_data.csv')`).
...
The only difference in this assignment from [Example -@sec-example_10_2] is the rate expression to be assessed. As a consequence, the rate expression parameters to be estimated in this assignment are different.
...
The reactor model, initial values, stopping criterion, ancillary equations, response function, and calculations are all the same as in @sec-example_10_2.

:::

**Reactor Model**

Mole balances on A, B, Y, and Z, are presented in equations (3) through (6), and the corresponding initial values and stopping criterion are shown in @tbl-example_10_3_initial_values.
...
: Initial values and stopping criterion for solving the design equations, equations (3) through (6). {#tbl-example_10_3_initial_values tbl-colwidths="[20, 40, 40]"}
...
#| echo: false
#| output: false
d <- read.csv(paste0(path_to_results,'reb_10_3_results.csv'))
d <- fmt_tibble_col(d, 2, 3, 2, 2)
...
The calculations were performed as described above. The resulting values of the rate expression parameters, their 95% confidence intervals, and the coefficient of determination, $R^2$, are shown in @tbl-example_10_3_results. The parity plot is shown in @fig-example_10_3_parity, and the residuals plots are shown in @fig-example_10_3_residuals_plots.
...
![Parity Plot.](`r paste0(path_to_figures,'reb_10_3_parity.png')`){#fig-example_10_3_parity width=80%}

::: {#fig-example_10_3_residuals_plots layout-ncol=2}

![](`r paste0(path_to_figures,'reb_10_3_residuals_vs_yAin.png')`)

![](`r paste0(path_to_figures,'reb_10_3_residuals_vs_yBin.png')`)

![](`r paste0(path_to_figures,'reb_10_3_residuals_vs_yYin.png')`)

![](`r paste0(path_to_figures,'reb_10_3_residuals_vs_yZin.png')`)
...
#| echo: false
#| output: false
d <- read.csv(paste0(path_to_results,'reb_10_3_two_par_results.csv'))
d <- fmt_tibble_col(d, 2, 3, 2, 2)
a <- c("k'","K~B~'",'R^2^')
b1 <- paste0(d$value[1],' ',d$units[1],', 95% CI [',d$value[2],', ',d$value[3],']')
b2 <- paste0(d$value[4],' ',d$units[4],', 95% CI [',d$value[5],', ',d$value[6],']')
...
The analysis was repeated a third time using equation (15) as the rate expression. The parameter estimation results are shown in @tbl-example_10_3_results_2_par. The parity plot is shown in @fig-example_10_3_parity_two_par and the residuals plots are shown in @fig-example_10_3_residuals_plots_two_par.

```{r}
#| echo: false
#| warning: false
#| label: tbl-example_10_3_results_2_par
#| tbl-cap: Parameter estimation results for equation (15)
kbl(d, format = "markdown", align = "cc", col.names = NULL)
```

![Parity Plot for Rate Expression (3).](`r paste0(path_to_figures,'reb_10_3_2_par_parity.png')`){#fig-example_10_3_parity_two_par width=80%}

::: {#fig-example_10_3_residuals_plots_two_par layout-ncol=2}

![](`r paste0(path_to_figures,'reb_10_3_2_par_residuals_vs_yAin.png')`)

![](`r paste0(path_to_figures,'reb_10_3_2_par_residuals_vs_yBin.png')`)

![](`r paste0(path_to_figures,'reb_10_3_2_par_residuals_vs_yYin.png')`)

![](`r paste0(path_to_figures,'reb_10_3_2_par_residuals_vs_yZin.png')`)
...
## Symbols Used in @sec-4_pfr_data_analysis
