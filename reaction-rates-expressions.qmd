...
Recall from @sec-react-eng-def that a reaction like @eq-unimolecular_rxn is called a unimolecular reaction. Unimolecular reactions are *not* elementary reactions. Nonetheless, the rate expression resulting from application of @eq-net_rate_elem_rxn or @eq-net_rate_elem_rxn_part_press is generally found to be valid unless the pressure is very low. In *Reaction Engineering Basics*, rate expressions for unimolecular reactions will be assumed to have the mathematical form predicted by @eq-net_rate_elem_rxn or @eq-net_rate_elem_rxn_part_press. As such, the mathematical form of the rate expression for @eq-unimolecular_rxn is as shown in @eq-unimolecular_rate_expr or @eq-unimolecular_rate_expr_part_press.
...
The uncertainty in $k_0$ is relatively high; the estimated value could be increased 8-fold or decreased to 1/8 of its value and it would still fall within the 95% confidence interval. It is not uncommon, however, for Arrhenius pre-exponential factors to have large relative uncertainties. In contrast, the uncertainty in the activation energy is reasonable (plus or minus ca. 6 out of 58.3 kJ mol^-1^ or about 10%).
