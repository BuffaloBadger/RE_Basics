# Reaction Mechanisms and Mechanistic Rate Expressions {#sec-2_mechanisms}
...
Identifying the types of reactions in a chain reaction mechanism and ensuring that the mechanism satisfies the rule that there must be some linear combination of the mechanistic steps that exactly equals the apparent, non-elementary reaction is illustrated in [Example -@sec-example_5_3_1].

## Generating Rate Expressions from Mechanisms

Once a proposed mechanism has been checked to make sure it obeys the rules given above, it can be used to write an expression for the apparent rate of generation of a reactant or product in the corresponding non-elementary reaction. That non-elementary reaction can also be referred to as the apparent reaction or the macroscopically observed reaction.

### Overview

From a macroscopic perspective, the only reaction taking place is the non-elementary reaction, and the only reagents present are the reactants and products of that reaction, along with any non-reactive species present in the system. Consequently, it is desirable to generate an expression for the rate of the apparent reaction in terms of the concentrations or partial pressures of those macroscopically observable reagents.

The general approach for generating an expression for the rate of the non-elementary reaction begins by writing an expression for the non-elementary reaction in terms of rate expressions for the non-observable mechanistic steps. The sub-sections that follow will describe two primary ways of doing this.

Using the rate expressions for mechanistic steps to generate the rate expression for the non-elementary reaction initially results in a rate expression that includes concentrations or partial pressures of reactive intermediates. As already noted, the reactive intermediates are difficult to detect, let alone measure their concentrations. Therefore, the next step in generating the rate expression for the non-elementary reaction is to eliminate concentrations of reactive intermediates from it. The sub-sections that follow will describe a few ways of accomplishing this.

Finally, once an expression for the rate of the non-elementary reaction in terms of observable reagents has been generated, it may be possible to further simplify that expression. Doing so is also described in the remainder of this sub-section.

The remainder of this sub-section describes the procedures for generating the non-elementary rate expression, eliminating concentrations of reactive intermediates from it, and simplifying it. It may be difficult to follow the verbal descriptions that are presented, so each sub-section refers to examples at the end of the chapter that illustrate the topics being described.

### Rates of the Mechanistic Steps

The apparent net rate of generation of species $i$ in the overall macroscopically observed reaction, $j$, will be the sum of its rates of generation in each of the mechanistic steps. That is, the apparent rate of generation of $i$ via the non-elementary reaction, $j$, is given by @eq-gen_rate_from_mech. Because the steps are elementary, their rates are given by @eq-rate_mech_step, leading to @eq-gen_rate_expr_from_mech as the expression for the apparent rate of generation of $i$ via non-elementary reaction $j$. In *Reaction Engineering Basics*, such rate expressions are referred to as *mechanistic rate expressions*. Examples [-@sec-example_5_3_1], [-@sec-example_5_3_2], [-@sec-example_5_3_4], and [-@sec-example_5_3_5] illustrate the use of Equations [-@eq-gen_rate_from_mech] and [-@eq-gen_rate_expr_from_mech] in problems involving the generation of a rate expression from a mechanism.
...
It is important to note that $i$ in Equations [-@eq-gen_rate_from_mech] and [-@eq-gen_rate_expr_from_mech], can be any of the reactants or products in the apparent, non-elementary reaction, $j$. The resulting rates of generation of reagent $i$ via non-elementary reaction $j$ will all be stoichiometrically equivalent. 
...
The generation of a mechanistic rate expression from a heterogeneous catalytic reaction mechanism is illustrated in Examples [-@sec-example_5_3_6] and [-@sec-example_5_3_7].
...
The process for generating an expression for the rate of a non-elementary reaction from its mechanism proceeds as follows.
...
4. Substitute the expressions for the concentrations (partial pressures) of the reagents that are not macroscopically observable into the rate expression from step 2.
5. If there is a most-abundant intermediate, simplify the rate expression, if possible.

## Examples

When generating a rate expression from a mechanism, some, but not all, of the steps in the summary above used. Nonetheless, each step in that summary is used in at least one of the examples that follow. This section is one of the few places in *Reaction Engineering Basics,* where numerical methods are not used. Instead, the associated algebra is preformed manually. To prevent lines and lines of algebraic manipulation from obscuring the overall analysis process, the formulations contain multiple instances of "where did that come from" callouts that, by default, are hidden. Generally these callouts provide the algebraic details.

### Validity, Classification of Steps and Mechanistic Rate Expression for a Chain Reaction Mechanism {#sec-example_5_3_1}
...
Substitution of equation (6) into equation (9), followed by substitution of equations (7) and (8) into the resulting equation yields an expression for the apparent rate of genertion of NO by reaction (1), equation (10).
...
It will be seen in later chapters of *Reaction Engineering Basics*, that a reaction engineer will commonly use the rate expression by substituting it into mole and energy balances for a reactor (see @sec-3_react_eng_models). When writing those mole and energy balances, the reaction engineer will not know the reaction mechanism; the reaction engineer will only know that reaction (1) takes place and that equation (10) is the rate expression for reaction (1).
...
### Mechanistic Rate Expression for HBr Synthesis using the Bodenstein Steady State Approximation {#sec-example_5_3_2}
...
I always like to begin mechanism problems by identifying the reactive intermediates and the macroscopically observable reagents. I see that H~2~, Br~2~, and HBr all appear in the apparent, non-elementary reaction. Looking at the mechanism proposed in reactions (2) through (5), I see that H∙ and Br∙ also appear, so they are the reactive intermediates.
...
**Macroscopically Observable Reagents:** H~2~, Br~2~, and HBr
...
After substitution of equations (7) through (10) into equations (11) and (12), the latter can be solved to obtain expressions for the concentrations of H∙ and Br∙, equations (13) and (14).
...
Start by adding equations (11) and (12).

$$
0 = 2r_2 - 2r_5 \qquad \Rightarrow \qquad r_2 = r_5
$$

Substitute equations (7) and (10).

$$
k_{2,f} \left[ Br_2 \right]^{-(-1)} - k_{2,r}\left[ Br \cdot \right]^{2} = 0
$$

Solve for $\left[ Br \cdot \right]$, yielding equation (13).

Substitute equations (8), (9), and (10) in equation (12).

$$
0 = k_{3,f} \left[ Br \cdot \right] \left[ H_2 \right] - k_{3,r} \left[ HBr \right] \left[ H \cdot \right] - k_{4,f} \left[ H \cdot \right] \left[ Br_2 \right] - 0
$$

Solve for $\left[ H \cdot \right]$.

$$
\left[ H \cdot \right] = \frac{k_{3,f} \left[ Br \cdot \right] \left[ H_2 \right]}{k_{3,r} \left[ HBr \right] + k_{4,f} \left[ Br_2 \right]}
$$

Substitute equation (13), yielding equation (14).

:::

Substitution of equations (13) and (14) into equations (8) and (9), followed by substitution of the results into equation (6) yields a mechanistic rate expression for the apparent rate of generation of HBr via non-elementary reaction (1), as shown in equation (15).
...
Substitute equations (8) and (9) into equation (6).

$$
r_{HBr,1} = k_{3,f} \left[ Br \cdot \right] \left[ H_2 \right] - k_{3,r} \left[ HBr \right] \left[ H \cdot \right] + k_{4,f} \left[ H \cdot \right] \left[ Br_2 \right]
$$

Factor $\left[ H \cdot \right]$ out of the last two terms.

$$
r_{HBr,1} = k_{3,f} \left[ Br \cdot \right] \left[ H_2 \right] -  \left[ H \cdot \right]\left(k_{3,r} \left[ HBr \right] - k_{4,f}  \left[ Br_2 \right]\right)
$$

Substitute equations (13) and (14).

$$
r_{HBr,1} = k_{3,f} \sqrt{\frac{k_{2,f}}{k_{2,r}}} \sqrt{\left[ Br_2 \right]} \left[ H_2 \right] -  \frac{k_{3,f}\sqrt{\frac{k_{2,f}}{k_{2,r}}} \sqrt{\left[ Br_2 \right]}\left[ H_2\right]}{k_{3,r}\left[ HBr\right] + k_{4,f}\left[ Br_2 \right]}\left(k_{3,r} \left[ HBr \right] - k_{4,f}  \left[ Br_2 \right]\right)
$$

Factor out $k_{3,f} \sqrt{\frac{k_{2,f}}{k_{2,r}}} \sqrt{\left[ Br_2 \right]} \left[ H_2 \right]$.

$$
r_{HBr,1} = k_{3,f} \sqrt{\frac{k_{2,f}}{k_{2,r}}} \sqrt{\left[ Br_2 \right]} \left[ H_2 \right]\left(1 - \frac{k_{3,r}\left[ HBr\right] - k_{4,f}\left[ Br_2 \right]}{k_{3,r}\left[ HBr\right] + k_{4,f}\left[ Br_2 \right]}\right)
$$

Subtract the final two terms.

$$
r_{HBr,1} = k_{3,f} \sqrt{\frac{k_{2,f}}{k_{2,r}}} \sqrt{\left[ Br_2 \right]} \left[ H_2 \right]\left(\frac{\cancelto{}{k_{3,r}\left[ HBr\right]} + k_{4,f}\left[ Br_2 \right]- \cancelto{}{k_{3,r}\left[ HBr\right]} + k_{4,f}\left[ Br_2 \right]}{k_{3,r}\left[ HBr\right] + k_{4,f}\left[ Br_2 \right]}\right)
$$

Add the two terms in the numerator.

$$
r_{HBr,1} = k_{3,f} \sqrt{\frac{k_{2,f}}{k_{2,r}}} \sqrt{\left[ Br_2 \right]} \left[ H_2 \right]\left(\frac{2k_{4,f}\left[ Br_2 \right]}{k_{3,r}\left[ HBr\right] + k_{4,f}\left[ Br_2 \right]}\right)
$$

Note that $\sqrt{\left[ Br_2 \right]}\left(\left[ Br_2 \right]\right) = \left[ Br_2 \right]^{3/2}$, yielding equation (15).
...
If rate coefficients are added or subtracted, this will not be true. That is, if $k = k_1 + k_2$, the combined rate coefficient $k$ will *not* display Arrhenius temperature dependence, even if $k_1$ and $k_2$, individually, do display Arrhenius temperature dependence.

:::

### Mechanistic Rate Expression from a Mechanism with a Rate-Determining Step {#sec-example_5_3_3}
...
Equations (6) and (7) can be solved to obtain expressions for the concentrations of the reactive intermediates, C~3~H~5~∙ and I∙, equations (8) and (9). Substitution of those equations in equation (5) gives an expression, equation (10), for the apparent rate of reaction (1) if reaction (2) is rate-determining.
...
$$
\begin{align}
r_1 &= k_{2,f} \left[ C_3H_5I \right] - k_{2,r} \left[ C_3H_5 \cdot \right] \left[ I \cdot \right]  \\&= k_{2,f} \left[ C_3H_5I \right] - k_{2,r}\frac{\sqrt{\left[ C_6H_{10} \right]}}{\sqrt{K_4}}\frac{\sqrt{\left[ C_6H_{10} \right]}\left[ I_2 \right]}{K_3 \sqrt{K_4}\left[ C_3H_5I \right]} \\&= k_{2,f} \left[ C_3H_5I \right] - \frac{k_{2,r}}{K_3 K_4} \frac{\left[ C_6H_{10} \right] \left[ I_2 \right]}{\left[ C_3H_5I \right]}
\end{align}
$$
...
$$
r_1 = r_3 = k_{3,f} \left[ C_3H_5I \right] \left[ I \cdot \right] - k_{3,r} \left[ C_3H_5 \cdot \right] \left[ I_2 \right] \tag{11}
$$
...
$$
r_1 = r_4 = k_{4,f} \left[ C_3H_5 \cdot \right]^2 - k_{4,r} \left[ C_6H_{10} \right] \tag{17}
$$
...
Similar grouping of the kinetics parameters in rate expressions (16) and (22) leads to equations (25) and (26)
...
### Michaelis-Menten Rate Expression for an Enzymatic Reaction {#sec-example_5_3_4}
...
Setting the apparent rate of generation of P via reaction (1) equal to the sum of its rates of generation in each of the mechanistic steps yields equation (4) because P is only generated in step (3) of the mechanism.
...
After substitution of equations (5) and (6) in equation (7), equations (7) and (8) can be solved to obtain expressions for the concentrations of E and E-S shown in equations (9) and (10).
...
After substitution of equation (10) in equation (6), equation (6) can be substituted in equation (4) to obtain the expression for the apparent rate of generation of P via non-elementary reaction (1) shown in equation (11).

$$
r_{P,1}  = r_3 = k_{3,f} \left[ E\!-\!S \right] = \frac{k_{2,f} k_{3,f} E_0 \left[ S \right]}{k_{2,r} + k_{3,f} + k_{2,f}\left[ S \right]} = \frac{k_{3,f} E_0 \left[ S \right]}{\frac{k_{2,r} + k_{3,f}}{k_{2,f}} + \left[ S \right]} \tag{11}
$$
...
### Mechanistic Rate Expression from an Enzymatic Mechanism with an Inhibitor {#sec-example_5_3_5}
...
After substitution of equations (6) through (8), equations (9) through (11) can be solved to obtain the expressions for the concentrations of the reactive intermediates shown in equations (12) through (14).
...
Substitute equation (7) into equation (10) and solve for $\left[E\!-\!I \right]$.
...
Substitute equations (6) and (8) in equation (9) and solve for $\left[ E\!-\!S \right]$.
...
Substitute equations (A) and (B) in equation (11) and solve for $\left[ E \right]$.
...
Substitution of equation (12) in equation (A) yields equation (13), and substitution of equation (12) in equation (B) yields equation (14).

:::

Substitution of equation (8) into equation (5), followed by substitution of equation (14) leads to equation (15) for the rate apparent rate of generation of P.
...
Note that since the stoichiometric coefficient of P in the non-elementary reaction is 1, the apparent general rate of reaction (1) is equal to the apparent rate of generation of P via reaction (1), equation (16).

$$
r_{P,1} = \nu_{P,1}r_1 \qquad \Rightarrow \qquad r_1 = r_{P,1} \tag{16}
$$

The assignment asks for a "Michaelis-Menten type rate expression." In a manner analogous to the note at the end of [Example -@sec-example_5_3_4], The kinetics parameters, $V_{max}$, $K_m$, and $K_I$, can be defined as shown in equations (17) through (19). Substitution into equation (15) then yields the Michaelis-Menten type rate expression shown in equation (20).
...
### Langmuir-Hinshelwood Rate Expression for a Heterogeneous Catalytic Reaction {#sec-example_5_3_6}
...
Substitute equations (A) through (D) in equation (12) and solve for $\theta_{\text{vacant}}$ to get equation (13). Then substitute the result in equations (A) through (D) to get equations (14) through (17).
...
Langmuir-Hinshelwood rate expressions for heterogeneous catalytic reactions, Michaelis-Menten rate expressions for enzymatic reactions and general rate expressions for homogeneous catalytic reactions often take the form of a fraction. The numerator is typically a single positive term when the apparent reaction is irreversible or the difference between two terms when the apparent reaction is reversible. The denominator is typically the sum of several terms. [Example -@sec-example_5_3_7] shows how the summation in the denominator in these rate expressions often reduces to a single term when the amount of one of the reactive intermediates is much greater than all the other reactiven intermediates. 

:::

### Mechanistic Rate Expression from a Mechanism with a Most Abundant Intermediate {#sec-example_5_3_7}
...
Equations (8) through (12), can be solved to obtain the expressions for the five surface coverages shown in equations (13) through (17).
...
Solve equations (8) through (11) for the coverages of O~2~, CO~3~, O, and O~2~, and call the results equations (A) through (D). Then substitute equations (A) through (D) into equation (12), and solve for $\theta_\text{vacant}$. Finally substitute that result into equations (A) through (D). The algebra is messy, but straightforward.

:::

Substitution into equation (7) then yields the expression for the apparent rate of reaction shown in equation (18).
...
Assuming $O\!-\!\ast$ to be the most abundant surface intermediate leads to inequalities (19) through (16). 
...
## Symbols Used in @sec-2_mechanisms
