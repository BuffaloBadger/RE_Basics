...
#| output: false
path_to_data <- './Data/'
path_to_results <- '../RE_Basics_Examples/Results/reb_4_2/'
...
#| output: false
path_to_data <- './Data/'
path_to_results <- '../RE_Basics_Examples/reb_4_3/Results/'
path_to_figures <- './Graphics/'
```
...
{{< include examples/reb_4_5_4/narrative.qmd >}}
...
The data are available in the .csv file  [reb_4_5_4_data.csv](./examples/reb_4_5_4/reb_4_5_4_data.csv).
...
This assignment presents experimental data and then asks whether those data are "consistent with" the Arrhenius expression, @eq-arrhenius. In other words, does the Arrhenius expression accurately predict the value of the rate coefficient, $k$, at each of the Celsius tempertures, $T_C$, listed in @tbl-data_reb_4_5_4. I'll start by summarizing the assignment. That is, I'll list the quantities whose values are given in the problem statement, assigning them to approprite variable symbols. In this problem I'm given several values of $T_C$ and $k$; I'll underline these in my listing to indicate that they are vector quantities. I know I'll need the ideal gas constant, so I'll add that to the given and known quantities.
...
To answer this question, I'll need to fit the Arrhenius expression to the experimental data and then assess the accuracy of the resulting model (see @sec-apndx_parameter_est). I know that the most common way to fit the Arrhenius expression to experimental data is by using the linearized form given in @eq-linear_arrhenius. I need to make sure the temperatures are in absolute units when I calculate $x$.

:::

**Linearized Arrhenius Expression**: 
...
After calculating the variables, $x$ and $y$, I need to fit equation (2) to the data. This can be done using a spreadsheet program. The original data would first be entered in two columns, then new columns would be added for $y$ and $x$. Those columns would then be plotted as a $x-y$ scatterplot. Adding a linear trendline will fit equation (2) to the data, and in most spreadsheets the equation can be displayed on the graph along with the coefficient of determination, $R^2$. I would also like a measure of the undertainty in the values of the slope, $m$, and intercept, $b$. The way those are calculated will depend upon the specific spreadsheet program being used.

An alternative approach is to simply use a linear least squares fitting function from a mathematics software package to do the fitting as described in Example [-@sec-example_L_7_1]. I'm going to use the 95% confidence interval as the measure of uncertainty. Looking at equation (2), I can see that $m$ is equal to $E$ and $b$ is equal to $\ln{k_0}$, so I need to calculate $k_0$ and its 95% confidence interval.

:::

The linear least squares fitting function will return the best values of $m$ and $b$, the upper and lower limits of their 95% confidence intervals, $m_{CI,u}$, $m_{CI,l}$, $b_{CI,u}$, and $b_{CI,l}$, and the coefficient of determination, $R^2$.
...
I'll want to generate a model plot to assess the accuracy of the Arrhenius expression for these data. To do so, I can calculate model-predicted values, $y_model$. Then I can plot $y$ *vs*. $x$ and points and $y_model$ *vs*. $x$ as a line on the same axes.
...
The uncertainty in $k_0$, that is the difference between the upper and lower limits of the 95% confidence interval, is `r k0_uncert` `r df$units[1]`. This is quite large compared to the estimated value of $k_0$, `r df$value[1]` `r df$units[1]`. Estimated pre-exponential factors are often found to have large uncertainties. The uncertainty in the activation energy is `r as.numeric(df$value[6]) - as.numeric(df$value[5])` `r df$units[4]` (18%) compared to the estimated value of `r df$value[4]`  `r df$units[4]`. The coefficient of determination is quite close to 1.0. While the model plot shows good agreement between the experimental data and the model there does appear to be a trend in the deviations. At low $T_{-1}$ the points fall above the line, and intermediate values they are below it, and at the the largest value it is again above the line. Nonetheless, the deviations are small, and it appears that the experimental data are consistent with the Arrhenius expression within the range of temperatures studied.
