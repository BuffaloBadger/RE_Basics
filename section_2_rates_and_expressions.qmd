...
The estimation of Arrhenius parameters and assessment of the resulting expression are illustrated in Examples [-@sec-example_4_5_4] and [-@sec-example_L_7_1].
...
To answer this question, I'll need to fit the Arrhenius rate coefficient model to the experimental data and then assess the accuracy of the fit (see @sec-apndx_parameter_est). I know that the most common way to fit the Arrhenius expression to experimental data is by using the linearized form given in @eq-linear_arrhenius.
...
The Arrhenius expression requires that the temperatures be in absolute units, so the first thing I need to do is convert each Celsuis temperature, $T_C$ to a temperature in K, which I'll denote simply as $T$. Then, before I can fit the linear model to the data, I need to calculate $x$ and $y$ as indicated above for each experiment.
