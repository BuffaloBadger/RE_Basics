...
To answer this question, I'll need to fit the Arrhenius rate coefficient model to the experimental data and then assess the accuracy of the fit (see @sec-apndx_parameter_est). I know that the most common way to fit the Arrhenius expression to experimental data is by using the linearized form given in @eq-linear_arrhenius. I need to make sure the temperatures are in absolute units when I calculate $x$.
...
I'll want to generate a model plot to assess the accuracy of the Arrhenius expression for these data. To do so, I can calculate model-predicted values, $y_{model}$. Then I can plot $y$ *vs*. $x$ as points and $y_{model}$ *vs*. $x$ as a line on the same axes.
