...
This example illustrated the preferred way to report a value together with its 95% confidence interval: "value units, 95% CI [lower bound, upper bound]," Thus $E$ = `r results$value[4]` `r results$units[4]`, 95% CI [`r results$value[5]`, `r results$value[6]`] means that the estimated value of $E$ is `r results$value[4]` `r results$units[4]`, and if the experiments were repeated many times, 95% of those times the estimated value of $E$ would be between `r results$value[5]` `r results$units[4]` and `r results$value[6]` `r results$units[4]`. 
