...
So, before I can fit the linear model to the data, I need to calculate $\underline{x}$ and $\underline{y_{\text{expt}}}$ as indicated above. I'll use the following notation, for example, to mean take each value of $k_{\text{expt}}$ in the experimental data set, calculate the corresponding value of $y_{\text{expt}}$, and combine the results as a data set.

$$
\underline{k_{\text{expt}}} \qquad \Rightarrow \qquad y_{\text{expt}} = \ln{\left(k_{\text{expt}}\right)} \qquad \Rightarrow \qquad \underline{y_{\text{expt}}}
$$

I can then provide the resulting data sets to a linear model fitting function to get $m$, $b$, $R^2$, $m_{CI,u}$, $m_{CI,l}$, $b_{CI,u}$, and $b_{CI,l}$.
...
I'll want to generate a model plot to assess the accuracy of the Arrhenius expression for these data. To do so, I can use the estimated values of $k_0$ and $E$ to calculate model-predicted values, $k_{model}$ using equation (1). Then I can plot $k_{\text{expt}}$ *vs*. $\frac{1}{T_C + 273.15}$ as points and $k_{model}$ *vs*. $\frac{1}{T_C + 273.15}$ as a line on the same axes.
