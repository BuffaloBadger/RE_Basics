...
As noted, the axial dispersion model is simply the ideal PFR model with dispersion terms added to the mole and energy balances. Importantly, plug flow is still assumed. In **[add cross-ref to appendix]** the axial dispersion mole and energy balances and the associated initial values and boundary conditions are derived, the steady-state forms are generated, and then they are converted to an equivalent set of first-order CVODEs. Familiarity with the first-order design equations and the variables ($\omega_i$ and $\omega_T$) introduced with them is assumed here.
...
The mole and energy balances for the axial dispersion model are shown in [Equations -@eq-axial_disp_conc_var] through [-@eq-axial_disp_vfr_deriv]. [Equations -@eq-axial_disp_conc_var] and [-@eq-axial_disp_temp_var] are essentially defining equations for $\omega_i$ and $\omega_T$. [Equations -@eq-axial_disp_conc] and [-@eq-axial_disp_temp] are the PFR design equations with the dispersion terms added. If the reacting fluid is an ideal, incompressible liquid mixture, the volumetric flow rate is constant and equal to $\dot{V}_{feed}$, the derivative of the volumetric flow rate in @eq-axial_disp_conc equals zero and @eq-axial_disp_vfr_deriv is not needed. If there is pressure drop in the reactor, a momentum balance must be added to the design equations, and if an external heat exchange fluid transfers sensible heat to or from the reactor, an energy balance on the exchange fluid must be added.
...
$$
\frac{d\dot{V}}{dz} = \frac{-\dot{m} \displaystyle \sum_i \left(M_i \omega_i \right)}{\left( \displaystyle \sum_i \left(M_i C_i \right)\right)^2}
$${#eq-axial_disp_vfr_deriv}

The Danckwerts boundary conditions shown in [Equations -@eq-danckwerts_conc] and [-@eq-danckwerts_temp] can be used to calculate initial values for the concentrations and the temperature. It isn't possible to write expressions for calculating the initial values of each $\omega_i$ and of $\omega_T$ directly. Their initial and final values are coupled, leading to implicit equations for calculating the initial initial values, [-@eq-implicit_omega_i] and [-@eq-implicit_omega_T].  Noting that at stead-state, the mass flow rate of the feed equals the mass flow rate at the reactor inlet leads to @eq-vfr_bound_cond for the initial value of the volumetric flow rate if the reacting fluid is an ideal gas. If a momentum balance is included among the design equations, the feed pressure can be used as the initial value. If there is an exchange fluid energy balance, it will be an ATE and the exchange fluid temperature will be coupled with the ODE design equations.

$$
C_{i,0} = \frac{\dot{V}_{feed}C_{i,feed} + D_{ax} \frac{\pi D^2}{4} \omega_{i,0}}{\dot{V}_{feed}}
$${#eq-danckwerts_conc}

$$
T_0 = \frac{\dot{V}_{feed} \left(\sum_i C_i \hat{C}_{p,i}\right) T_{feed} + \lambda_{ax} \frac{\pi D^2}{4} \omega_{T,0}}{\dot{V}_{feed} \left(\sum_i C_i \hat{C}_{p,i}\right)}
$${#eq-danckwerts_temp}
...
**Reactor Model Function** The reactor model function is the final component of the axial dispersion reactor model. Briefly, it simply needs to solve the coupled values equations using an ATE solver and then use the results to solve the design equations using an IVODE solver. More specifically the reactor model function works as follows.
...
* Calculate the initial values of each $C_i$ and of $T$ using [Equations -@eq-danckwerts_conc] and [-@eq-danckwerts_temp].
...
$$
\frac{dC_A}{dz} = \omega_A \tag{3}
$$

$$
\frac{dC_Y}{dz} = \omega_Y \tag{4}
$$

$$
\frac{dC_Z}{dz} = \omega_Z \tag{5}
$$

$$
\frac{dC_I}{dz} = \omega_I \tag{6}
$$

$$
\frac{dT}{dz} = \omega_T \tag{7}
$$

$$
- D_{ax}\frac{d \omega_A}{d z} + \frac{4 C_A}{\pi D^2}\frac{d \dot{V}}{d z} = - \frac{4 \dot{V}}{\pi D^2}\omega_A - r \tag{8}
$$

$$
- D_{ax}\frac{d \omega_Y}{d z} + \frac{4 C_Y}{\pi D^2}\frac{d \dot{V}}{d z} = - \frac{4 \dot{V}}{\pi D^2}\omega_Y + r \tag{9}
$$

$$
- D_{ax}\frac{d \omega_Z}{d z} + \frac{4 C_Z}{\pi D^2}\frac{d \dot{V}}{d z} = - \frac{4 \dot{V}}{\pi D^2}\omega_Z + 3r \tag{10}
$$

$$
- D_{ax}\frac{d \omega_I}{d z} + \frac{4 C_I}{\pi D^2}\frac{d \dot{V}}{d z} = - \frac{4 \dot{V}}{\pi D^2}\omega_I \tag{11}
$$

$$
- \lambda_{ax}\frac{d \omega_T}{d z} =- \frac{4 \dot{V}}{\pi D^2} \left(\sum_i C_i \hat{C}_{p,i}\right) \omega_T + \frac{4U}{D}\left(T_{ex} - T\right) - r \Delta H_j \tag{12}
$$

$$
\frac{d\dot{V}}{dz} = \frac{-\dot{m} \left(M_I \omega_I + M_A \omega_A + M_Y \omega_Y + M_Z \omega_Z \right)}{\left( M_I C_I + M_A C_A + M_Y C_Y + M_Z C_Z\right)
^2} \tag{13}
$$
...
: Initial values and stopping criterion for solving the axial dispersion design equations with $z=0$ defined as the inlet to the reactor. {#tbl-axial_disp_init_value_table}

**Derivatives Function**

[Arguments]{.underline}: $z$, $C_A$, $C_Y$, $C_Z$, $C_I$, $T$, $\omega_A$, $\omega_Y$, $\omega_Z$, $\omega_I$, $\omega_T$, $\dot{V}$, and $P$.

[Returns]{.underline}: $\frac{dC_A}{dz}$, $\frac{dC_Y}{dz}$, $\frac{dC_Z}{dz}$, $\frac{dC_I}{dz}$, $\frac{dT}{dz}$, $\frac{d\omega_A}{dz}$, $\frac{d\omega_T}{dz}$, $\frac{d\omega_Z}{dz}$, $\frac{d\omega_I}{dz}$, $\frac{d\omega_T}{dz}$, $\frac{d\dot{V}}{dz}$, and $\frac{dP}{dz}$.

[Algorithm]{.underline}:

$$
\dot{m} = \dot{V}_{feed} \left(M_A C_{A,feed} + M_Y C_{Y,feed} + M_Z C_{Z,feed} + M_I C_{I,feed} \right) \tag{15}
$$

$$
\dot{V} = \frac{\dot{m}}{M_A C_{A} + M_Y C_{Y} + M_Z C_{Z} + M_I C_{I}} \tag{16}
$$

$$
k_f = k_{0,f} \exp{\left( \frac{-E_f}{RT} \right)} \tag{17}
$$ 

$$
k_r = k_{0,r} \exp{\left( \frac{-E_r}{RT} \right)} \tag{18}
$$

$$
P_A = \frac{C_A}{C_A + C_Y + C_Z +C_I}P \tag{19}
$$

$$
P_Y = \frac{C_Y}{C_A + C_Y + C_Z +C_I}P \tag{20}
$$

$$
P_Z = \frac{C_Z}{C_A + C_Y + C_Z +C_I}P \tag{21}
$$
...
$$
\frac{d \omega_A}{d z} = \frac{-1}{D_{ax}}\left(- \frac{4 C_A}{\pi D^2}\frac{d \dot{V}}{d z} - \frac{4 \dot{V}}{\pi D^2}\omega_A - r\right) \tag{21}
$$

$$
\frac{d \omega_Y}{d z} = \frac{-1}{D_{ax}}\left(- \frac{4 C_Y}{\pi D^2}\frac{d \dot{V}}{d z} - \frac{4 \dot{V}}{\pi D^2}\omega_Y + r\right) \tag{22}
$$

$$
\frac{d \omega_Z}{d z} = \frac{-1}{D_{ax}}\left(- \frac{4 C_Z}{\pi D^2}\frac{d \dot{V}}{d z} - \frac{4 \dot{V}}{\pi D^2}\omega_Z + 3r\right) \tag{23}
$$

$$
\frac{d \omega_I}{d z} = \frac{-1}{D_{ax}} \left(- \frac{4 C_I}{\pi D^2}\frac{d \dot{V}}{d z} - \frac{4 \dot{V}}{\pi D^2}\omega_I \right) \tag{24}
$$

$$
\frac{d \omega_T}{d z} = \frac{-1}{\lambda_{ax}} \left(- \frac{4 \dot{V}}{\pi D^2} \left(\sum_i C_i \hat{C}_{p,i}\right) \omega_T + \frac{4U}{D}\left(T_{ex} - T\right) - r \Delta H_j\right) \tag{25}
$$
...
$$
\left(\omega_{A,0}, \omega_{Y,0}, \omega_{Z,0}, \omega_{I,0}, \omega_{T,0}\right)\,:\, \omega_{A,L} = 0 = \epsilon_A \tag{26}
$$
...
1. Calculate $C_{A,0}$, $C_{Y,0}$, $C_{Z,0}$, $C_{I,0}$, $T_0$, and $\dot{V}_0$, @tbl-axial_disp_init_value_table.
...
3. Calculate the initial values and stopping criterion, @tbl-axial_disp_init_value_table.
