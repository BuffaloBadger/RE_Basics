# Generation and Analysis of Kinetics Data {#sec-4_kin_data_gen}
...
The development of a rate expression for a reaction begins by postulating its mathematical form. As discussed in @sec-2_rates_rate_express and @sec-2-mechanisms, the form might be chosen empirically, theoretically or mechanistically. Then a reactor must be chosen for use in kinetics experiments. Chapters [-@sec-4_bstr_data_analysis], [-@sec-4_cstr_data_analysis], and [-@sec-4_pfr_data_analysis] discuss the use of ideal BSTRs, CSTRs, and PFRs, respectively, in the development of rate expressions.
...
The kinetics experiments entail adjusting each of the inputs to one of its levels, measuring the corresponding response, adjusting one or more of the inputs to a different one of it levels, measuring the response, etc. The final decision involves which combinations of levels to use to generate the kinetics data. There are a few ways to do this. A common one, sometimes called a full factorial design simply includes every possible combination of the input levels. [Example -@sec-example_8_1] illustrates full factorial design of kinetics experiments.

## Analysis of Kinetics Data

When the reactor operates isothermally and an experimental design with a limited number of temperature levels is used, the kinetics data can be analyzed in two phases. First the data are grouped in blocks where each block includes *all* of the experiments at one of the temperature levels. In the first phase of the analysis, each of these blocks is analyzed separately to assess how accurately the proposed rate expression describes the effect of composition and pressure on the reaction rate.

Since all of the experiments in each data block are at a single temperature, the pre-exponential factor and activation energy cannot be estimated. So during this phase of the analysis, the rate coefficient is the kinetics parameter to be estimated. The same is true of any other kinetics parameters that may vary with temperature.

Once the first phase of the analysis is completed, that is, once all of the blocks of data have been analyzed individually, the Arrhenius expression is fit to the resulting rate coefficients to assess the accuracy of the proposed rate expression in describing the temperature dependence of the rate. Typically the linearized form of the Arrhenius expression is used, as was illustrated in [Example -@sec-example_4_4].

It was noted in @sec-3_design_eqns that when a reactor operates isothermally at a known temperature, the mole balance design equations can be solved separately from the energy and momentum balance design equations. In fact, when only one reaction is taking place, as is always the case for kinetics data analysis in *Reaction Engineering Basics*, it is possible to use a single mole balance on one reactant or product. The amounts of the other reagents are related to that one reactant or product through the reaction stoichiometry. Three different ways of analyzing each block of experimental data are considered next. 

### Using a Response Function

The first approach to kinetics data analysis uses a **response function**. As described in @sec-3_response_func, all known reactor inputs, reactor parameters, fluid properties and reaction properties are made available within the response function. The missing reaction parameters are passed to the response function as arguments as are the values of the reactor inputs that were adjusted from experiment to experiment. Within the response function the design equations for the reactor are first solved to find the reactor outputs. Either the full set of mole balances or a single mole balance on a reactant or product can be used as the reactor model. After the reactor outputs have been found, the response function calculates the value of the experimental response predicted by the model. **This approach can *always* be used** and it does not introduce any approximations that can affect the accuracy of the results. For this reason, this is the approach used in most examples in *Reaction Engineering Basics*.

When using a response function with parameter estimation software, the response function is passed to that software as an argument. The parameter estimation software one chooses to use will specify the order and form of the arguments to the response function and its return values. When creating the response function these specifications must be met as mentioned in @sec-3_response_func.

As an example, in the Python computer language a function named `curve_fit` can be used to perform parameter estimation. When `curve_fit` is called, the response function is passed to it as an argument. `curve_fit` expects that the first argument to the response function will be a matrix containing *all* of the experimentally adjusted inputs. Each row in the matrix corresponds to one of the adjusted inputs and each column corresponds to one of the experiments. `curve_fit` further expects that the next arguments will be the unknown kinetics parameters to be estimated, each as a separate argument. Finally, `curve_fit` expects that the response function will return a row vector where the columns contain the values of the response predicted by the reactor model for each of the experiments. 

Once the response function has been written as specified by the parameter estimation software, it is passed to parameter estimation software along with the adjusted inputs data and the *experimental* responses data. (In addition, most parameter estimation software also requires that a guess for the values of the kinetics parameters be provided.) [Appendix -@sec-apndx_parameter_est] describes how parameter estimation software works. In essence, it finds values for the kinetics parameters that minimize the difference between the responses predicted by the model and the responses that were measured experimentally.
...
It will not be necessary to perform 72 experiments, however. Using a reactor at 65 °C with an initial concentration of A equal to 0.5 M, six responses can be recorded in the experiment (at reaction times of 5, 10, 15, 20, 25, and 30 min). The number of experiments needed to record all 72 responses is 12. The initial conditions for those 12 experiments are shown in @tbl-example_8_1. Each experiment in the table will yield responses at all six reaction time levels.
