...
3. Identify any additional unknowns that appear in the design equations, classifying them as computable, uncomputable, or coupled.
4. Write equations for calculating the additional computable and coupled unknowns.
5. If appropriate, define a range of values for the adjusted variable (parameter) to be used for generating a graph or optimizing a target variable.
...
7. Use the results from solving the design equations to calculate any additional quantities of interest and for generating requested graphs.

At this point, it may not be clear what some of these steps mean or entail. The examples included in the next four chapters will illustrate the use of this strategy and should clarify the meaning of the steps. The strategy is intended to be general enough to apply to any assignment involving the analysis of a single ideal reactor, but should not be viewed as a rigid recipe that must be followed exactly. It offers a general approach that can be adapted as needed.

The remainder of this section describes a workflow that supports the preceding computational strategy and that will be used in every *Reaction Engineering Basics* example assignment involving the analysis of a single ideal reactor. Later chapters of the book will add items to this workflow as new reactor systems or engineering tasks are introduced. The workflow progresses through five sequential stages.
...
The intent of the assignment summary is to replace the longer assignment narrative with a concise listing of the information it provides and requests. That information is organized under the following headings: Reactions, Rate Expressions, Reactor, Given Constants, Basis, Parameters, and Deliverables. Some examples will not require all of these headings. Upon completion of the assignment summary, it should not be necessary to read through the assignment narrative again.

The formulation of the equations begins with the equations for modeling the reactor. The necessary reactor design equations are selected and simplified first, and the variables to be found by solving them are enumerated. Next, if there are additional computable unknowns appearing in the design equations, the equations for calculating them are listed. If there are additional coupled unknowns (see [Appendix -@sec-apxd_ivodes]), implicit equations for calculating them are written. Then, if there are uncomputable unknowns, they are listed next (they will need to be provided as input when the reactor design equations are solved). 

The reactor design equations will either be algebraic-transcendental equations (ATEs) or initial value ordinary differential equations (IVODEs). The formulation of the equations continues with addition of the input that must be provided to the numerical equation solver or solvers that will be used to solve the ATEs or IVODEs. Finally, equations for calculating any additional quantities requested in the assignment are added.
