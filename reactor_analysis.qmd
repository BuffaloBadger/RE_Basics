...
Other times the heat exchange fluid may be treated as a constant temperature thermal sink with a known, constant temperature. In this situation, an energy balance on the exchange fluid is not needed. The other reactor design equations can be solved without one.
...
Alternatively the results from solving the reactor design equations are plotted. The maximum in the graph is identified and the maximum value of the target and the corresponding value of the adjusted variable are read from the graph. In both approaches, it is possible that a maximum will not be present. In that case, the range of values of the adjusted variable must be increased until a maximum is observed. Once a maximum is observed, the precision of the results can be improved by adding additional values of the adjusted variable between two points that bracket the maximum.
...
**Parameter estimation tasks** are inherently different from the three types of tasks described above, but they do use the same reactor design equations and those equations are solved the same way as in the other tasks. Briefly, a parameter estimation task involves using a reactor to generate data, and then using the data, together with a model for the reactor, to estimate the values of parameters appearing in a rate expression. The use of the ideal reactor models to estimate rate expression parameters is examined in @sec-ch11. A similar approach can be used to estimate parameters that appear in empirical models for non-ideal reactors such as those considered in @sec-ch13.
...
3. Draw sets of axes with reagent concentrations, temperature, and any other quantity of interest on the y-axes and reaction time or space time on the x-axes.
4. Plot a point on each y-axis representing the initial values of the quantities. (The axes need not have a scale, and as such, plotted values only indicate whether the quantity is initially large, intermediate or small in value.)
5. Determine the initial slope of each graph by considering how the concentrations and temperature will change during a short interval of time and how those changes will affect the other quantities plotted.
...
As the preceeding discussion suggests, the reactor design equations that are needed to complete a reaction engineering assignment will vary from assignment to assignment, as will the way that they are solved. To one who is just beginning to learn reaction engineering, it can seem that every assignment is different, and knowing how to proceed can seem overwhelming. As such, it can be helpful to establish a workflow that can be used consistently when completing assignments.

The assignments in the next four chapters will all involve the analysis of a single, ideal reactor. In *Reaction Engineering Basics,* the reactor design equations are always solved numerically. That introduces additional variability in that the computer code for doing so can often be structured in a variety of ways. The remainder of this section will describe the workflow that will be used in every assignment involving the analysis of a single ideal reactor. Later chapters of the book will add items to this workflow as new reactor systems or engineering tasks are introduced. 

The workflow involves in six stages that are completed sequentially, resulting in a completed assignment write-up with the following component sections.
...
The intent of the assignment summary is to replace the longer assignment narrative with a concise listing of the information and requests it contains. That information is organized under the following headings: Reactions, Rate Expressions, Reactor, Given Constants, Basis, Parameters, and Deliverables. Upon completion of the assignment summary, it should not be necessary to read through the assignment narrative again.

The next component is a computational strategy. Completing the assignment is always going to require solving the reactor design equations, and computer functions will be written for doing so. The purpose of the computational strategy is to identify the quantities that will be found by solving the design equations, the input that will need to be provided to the functions that solve them, and the way the results from solving them will be used to calculate any other quantities that are needed to fulfil the requests made in the assignment.

The formulation of the equations begins with the equations for modeling the reactor. The necessary reactor design equations are selected and simplified first, and the variables to be found by solving them are enumerated. Next equations for calculating any additional unknowns appearing in the equations are listed. If there are uncomputable unknowns, they are listed next (they will need to be provided as input to the function that solves the design equations). Input that must be provided to the numerical equation solver is then listed. Finally, equations for calculating additional quantities using the results from solving the design equations are listed.

The implementation of the calculations involves writing computer code. *Reaction Engineering Basics, The Book* describes the code in general terms so readers can write it using whatever programming environment and language they choose. Links to *Reaction Engineering Basics, The Course* are also provided and can be followed to see a Python implementation of the calculations.

The results and discussion section presents the results from performing the calculations. Qualitative analysis is used to interpret them on a physical basis. Interesting or unexpected features of the results are identified and rationalized. If the assignment requires the engineer to make an assessment or recommendation, the basis for it is developed in the discussion.

Finally, the deliverables section concisely fulfils the requests made in the assignment narrative. If that includes an assessment or recommendation, only the assessment/recommendation is presented since its justification was included in the results and discussion section.

This is a *suggested* workflow, not a required one. Readers may adapt it however they choose. It has not been described at length, but upon progression through the examples in the following chapters, the purpose and structures of the six sections should become evident. 
...
* be able to qualitatively describe how the reaction rate, composition, temperature, and related quantities evolve with reaction time, given appropriate information about rate expressions, reaction exothermicity, and reactor heat exchange
