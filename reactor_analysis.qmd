...
Alternatively the results from solving the reactor design equations are plotted. The maximum in the graph is identified and the maximum value of the target and the corresponding value of the adjusted variable are read from the graph. In both approaches, it is possible that a maximum will not be present. In that case, the range of values of the adjusted variable must be shifted until a maximum is observed. Once a maximum is observed, the precision of the results can be improved by adding additional values of the adjusted variable between two points that bracket the maximum.

Many software packages include a function (an optimizer) that performs optimization, that is finds a maximum or minimum. The second approach involves using such an optimizer. In this case, the numerical implementation of the solution would entail writing a reactor model function that receives the adjusted variable as an argument and returns target variable. That reactor model function would then be passed to the optimizer. Typically an initial guess for the optimum value of the adjusted variable would also need to be provided.
...
As the preceeding discussion suggests, the reactor design equations that are needed to complete a reaction engineering assignment will vary from assignment to assignment, as will the way that they are solved. To one who is just beginning to learn reaction engineering, it can seem that every assignment is different, and knowing how to proceed can seem overwhelming. As such, it can be helpful to establish a computational strategy and aworkflow that can be used consistently when completing assignments.

The assignments in the next four chapters will all involve the analysis of a single, ideal reactor. In *Reaction Engineering Basics,* the reactor design equations are always solved numerically, and the computer code for doing so can often be structured in a variety of ways. However the same computational strategy can be employed in each analysis.

1. Select and simplify the design equaations needed to model the reactor.
2. Identify the reactor model variables that will be found by solving the design equations.
3. Identify any additional unknowns that appear in the design equations and classify them as computable, uncomputable, or coupled.
4. Write equations for calculating the additional computable and coupled unknowns.
5. Define a range of values for the adjusted variable (parameter) if necessary for generating a graph or optimizing a target variable.
6. Solve the reactor design equations to get the values of the reactor model variables (for each value of the adjusted variable, if there is one).
7. Use the results from solving the design equations to calculate any additional quantities of interest.

The remainder of this section describes a workflow that supports that strategy, and that will be used in every *Reaction Engineering Basics* example assignment involving the analysis of a single ideal reactor. Later chapters of the book will add items to this workflow as new reactor systems or engineering tasks are introduced. The workflow progresses through five stages that are completed sequentially.
...
The intent of the assignment summary is to replace the longer assignment narrative with a concise listing of the information provides and requests. That information is organized under the following headings: Reactions, Rate Expressions, Reactor, Given Constants, Basis, Parameters, and Deliverables. Some examples will not require all of these headings. Upon completion of the assignment summary, it should not be necessary to read through the assignment narrative again.

The formulation of the equations begins with the equations for modeling the reactor. The necessary reactor design equations are selected and simplified first, and the variables to be found by solving them are enumerated. Next equations for calculating any additional unknowns appearing in the equations are listed. Then if there are additional coupled unknowns (see [Appendix -@sec-apxd_ivodes]), implicit equations for calculating them are written. If there are uncomputable unknowns, they are listed next (they will need to be provided as input when the reactor design equations are solved). Input that must be provided to the numerical equation solver or solvers is then listed. Finally, equations for calculating any additional quantities requested in the assignment are listed.

The implementation of the calculations involves writing computer code. *Reaction Engineering Basics, The Book* describes the code in general terms so readers can write it using whatever programming environment and language they choose. Most of the examples in the book are also presented in  *Reaction Engineering Basics, The Course,* and there the implementation of the calculations using Python is presented and discussed. Links are provided so interested readers can view the Python implementation.
...
The term "Deliverables" is used here to refer to the things requested in the assignment narrative. The deliverables section is simply a concise presentation of the deliverables, based upon the preceding results and discussion.

This is a *suggested* workflow, not a required one. Readers may adapt it however they choose, or ignore it completely. It has not been described at length, but upon progression through the examples in the following chapters, the purpose and content of the five sections should become evident. 
