...
As explained in @sec-3_response_func, in *Reaction Engineering Basics*, reactor modeling calculations are performed in three steps. First the mathematical solution of the equations is formulated, then the solution is implemented numerically, and finally the results are presented and discussed. This appendix examines the numerical implementation of the solution when the equations are a set of coupled, initial-value ordinary differential equations (IVODEs).

There are many different software packages that can be used to accomplish this task. Different readers may choose to use different software when solving IVODEs, so here, the implementation is described in general terms. Students seeking a more complete understanding should consult a numerical methods textbook, take a course on numerical methods, or read the documentation for the software they are using.

An IVODE solver is a computer function that solves a set of IVODEs numerically, While there are many software packages that include IVODE solvers and they will differ to some extent from package to package, all IVODE solvers require certain specific inputs and return the same basic results. In order to follow *Reaction Engineering Basics* example assignments wherein sets of IVODEs ae solved, and in order to implement them using an IVODE solver of one's choosing, a general understanding of how IVODE solvers work and the input that must be provided to them is needed. The intent of this appendix is to provide that necessary understanding.

## Identifying IVODEs

A differential equation contains at least one derivative. A distinguishing feature of a set of **ordinary** differential equations is that every derivative that appears in the equations has the same independent variable. In *Reaction Engineering Basics*, the independent variables is almost always either the elapsed time, $t$, or the axial position measured from the reactor inlet, $z$. Put differently, there are no partial derivatives in a set of ordinary differential equations.

Solving a set of ordinary differential equations involves finding the functional relationship between each of the dependent variables and the independent variable. In order to solve a set of $N$ ODEs, $N$ must equal the number of different dependent variables appearing in the set. For example, if the derivatives of three different dependent variables are present then to find how each of them depends upon the independent variable, there must be three ODEs in the set.

In order solve a set of ODEs, the value of each dependent variable must be known at one or more values of the independent variable. In particular, if all of the derivatives in a set of ODEs are first derivatives, then to solve the ODEs, the value of each dependent variable must be known at one value of the independent variable. In that situation, if the value of *every* dependent variable is known at *the same* value of the independent variable, the equations are known as **initial value ordinary equations**. In reaction engineering, the values of the dependent variables are *typically* known at the start of the process ($t=0$) or at the inlet to the reactor ($z=0$).

To summarize, a set of $N$ IVODEs contains ordinary derivatives of $N$ different dependent variables with respect to the same independent variable. In *Reaction Engineering Basics*, the independent variable is usually $t$ or $z$. In addition, the value of *every* dependent variable in the set is known at the *same* value of the independent variable. Typically the value of every dependent variable is known when the independent variable is equal to zero, hence the name *initial* value ODE.
...
When an IVODE is solved *numerically*, there are two significant differences. First, **the solution is not an analytical function**, it is a set of values of the independent variable, $x$ in the example above, starting at its initial value, along with a set of the corresponding values of each dependent variable, $y$ in the example above. A numerical solution of @eq-ivode, with the initial values given in @eq-initial-value is shown in @tbl-ivode-num-soln.
...
Notice that the IVODEs have been written with all of the derivatives on one side of the equation and the remaining terms on the other side. The set of reactions presented in Equations [-@eq-example-ode-1], [-@eq-example-ode-2], [-@eq-example-ode-3], and [-@eq-example-ode-4] are a generalization of the sets of IVODEs that must be solved in *Reaction Engineering Basics*. In those equations, $y_1$, $y_2$, $y_3$, and $y_4$ are the dependent variables; they are each functionally dependent upon only $x$, the independent variable. The coefficients, $m_{1,1}$, $m_{1,2}$, etc., as well as $g_1$, $g_2$, $g_3$, and $g_4$ each may be a function of $x$, $y_1$, $y_2$, $y_3$, and $y_4$.
...
## Formulating the Solution of a Set of IVODEs Mathematically

## IVODE Solvers

## Implementing the Solution of a Set of IVODEs Numerically

## Example: Solving a Set of 3 IVODEs
...
![Graphical Representation of an IVODE Integration Step. (a) The initial value. (b) Using the slope to approximate y(x). (c) Incrementally increasing x and approximating the corresponding y.](./Graphics/IVODE_Integration_Step.png){#fig-ivode-integration-step}
