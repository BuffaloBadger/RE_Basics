...
* If the initial guess is not a solution of the ATEs, the ATE solver generates an improved guess and tests that.
    * Unless the ATEs are actually a set of linear equations, the improved guess will **not** be an exact solution, but it will be closer to the solution than the previous guess. 
...
When the ATE solver is not able to converge to a solution, it is often because the guess provided to the solver wasn't close enough to the solution or because of a singularity in one or more of the residuals. For example, fractions that contain one or more unknowns in the denominator may appear in the ATEs and, if not eliminated, in the definitions of the residuals. If a guess for the solution causes such a denominator to equal zero, the residual becomes infinite.

It is advisable to remove as many of those denominators as possible at the time the residuals are defined. To do so, first rearrange each ATE so there is a zero on one side of the equation. Then multiply through the equation by each denominator. Finally, use the resulting non-zero side of the equation as the residual. If this advice is not heeded and the solver does not converge, the residuals function should be re-written, removing as many singularities as possible, and the solver should be called using a different initial guess.
...
In this system, I expect reagent A to be consumed and an equal amount of reagent Z to be produced. Hence, I might guess that $\dot{n}_A$ will equal one-half of $\dot{n}_{A,in}$ and that $\dot{n}_Z$ also will equal one-half of $\dot{n}_{A,in}$. I have no idea what the temperature might equal, but since the reaction is endothermic and the reactor is adiabatic I expect the temperature to be less than the inlet temperature. So I might guess that $T$ will be 10 K less than $T_{in}$.
...
In this situation (an endothermic reaction in an adiabatic CSTR) the engineer would not expect multiple solutions to be possible. If the engineer knew that multiple solutions were possible, steps 3 through 5 would be repeated using different initial guesses to find additional solutions.
