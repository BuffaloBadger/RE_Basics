...
This appendix examines the numerical solution of sets of algebraic/transcendental equations (ATEs). In *Reaction Engineering Basics*, example assignments that entail numerical calculations are presented in three steps (see @sec-3_response_func): (1) the solution of the equations is formulated mathematically, (2) the numerical implementation of that solution is described (but not illustrated), and (3) the solution is presented and discussed. The numerical implementation is not illustrated because there are numerous software packages that include an ATE solver (function that solves a set of ATEs numerically). Assuming not all readers will want to use the same software, the examples in *Reaction Engineering Basics* describe numerical implementation generally.
