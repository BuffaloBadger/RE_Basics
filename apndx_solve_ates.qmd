...
Having identified the design equations as ATEs, you know that to solve them you must provide an initial guess for the ATE unknowns and a residuals function. As part of your workflow, you concisely summarized the assignment including the listing of known constants and quantities of interest.
...
Within the residuals function, the additional unknowns, $r$, $k$, $C_A$, and $C_B$, must be calculated before the residuals can be evaluated. In this example, that is possible, but if any of them couldn't be calculated within the residuals function, they too would need to be made available to it by some other means.
...
The purpose of the residuals function specifications and the CSTR function specifications is to concisely provide all of the information that is needed to write them. It may be assumed that all of the given and known constants are available to all functions.
...
In this assignment, the ATEs are to be solved using two different values for $\dot{V}$, so it makes sense to pass the a value for it to the CSTR function. Its purpose is to calculate the ATE unknowns, so those are the quantities it should return.

The CSTR function algorithm is straightforward. To start, the guess is calculated. Next the value of $\dot{V}$ received as an argument is made available to the residuals function. Finally the ATE solver can be called to get one solution of the design equations. This leads to the following specifications for the CSTR function.
...
Additional code was written to call the CSTR function, display the results, and save the results to a file. @tbl-example_I_4_1_results shows the results.
...
c("Quantity","$\\dot{V}$ = 75 mol min^-1^","$\\dot{V}$ = 100 mol min^-1^","units"))
```
