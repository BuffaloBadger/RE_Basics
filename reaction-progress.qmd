...
$$
g_i = \frac{f_i}{{f_i}\Bigr\rvert_{eq}}= \frac{\frac{n_{i,0} - n_i}{n_{i,0}}}{\frac{n_{i,0} - \left( n_i \right)\Bigr\rvert_{eq}}{n_{i,0}}} = \frac{n_{i,0} - n_i}{n_{i,0} - \left( n_i \right)\Bigr\rvert_{eq}}
$$ {#eq-frac_of_eq_conversion_closed}

$$
g_i = \frac{\dot n_{i,in} - \dot n_i}{\dot n_{i,in} - \left( \dot n_i \right)\Bigr\rvert_{eq}}
...
In this way, the rows in the third column corresponding to each of the species are completed. Then, for the last row, the expressions in all the other rows are simply added together to complete @tbl-mole_table_reb_3_1
...
The problem directs me to use @tbl-mole_table_reb_3_1 to calculate the final mole fraction of CO~2~. Having practiced reacion engineering for some time, now, I know that when a problem provides the initial composition of a system and some measure of its final composition (in this case the conversion and selectivity) and asks for a different measure of the final composition (in this case, a mole fraction), generating the answer will require three steps. First, the measures of the final composition provided in the problem must be expressed in terms of the apparent extents of reaction. Second, the resulting equations are solved to obtain the values of the apparent extents of reaction. Finally, the desired measure of final composition is expressed in terms of the apparent extents of reaction and calculated using the values obtained in the second step.

Here, upon reading this problem, I also can see that every quantity that is provided is an intensive quantity. That means that if I need to, I can assume the value of any one extensive quantity and use it as the basis for my calculations.

Finally, I think it is good practice to begin by assigning each quantity given in the problem statement to the appropriate variable.

:::

The following quantities are provided in the problem statement. (In the cases of the mole fractions of O~2~ and N~2~, the problem notes that the system initially contains 10% air. Multiplying that 10% by the fraction of O~2~ found in air (21%), gives the mole fraction of O~2~. The same is true for N~2~, except its fraction in air is 79%.)
...
The conversion and the selectivity are given in the problem statement. Their definitions can be used together with the entries in the mole table to express them in terms of the apparent extents of reaction, equations (3) and (4).
...
2. Calculate $n_{O_2,0}$, $n_{C_2H_4,0}$, and $n_{CO_2,0}$ using equations (4) through (6), and substitute the results into all remaining equations.
3. Solve equations (3) and (4) for $\xi_1$ and $\xi_2$, and substitute the results into all remaining equations.
4. Calculate the mole fraction of CO~2~ using equation (9).
...
This problem provides the starting composition and asks me to express one final value in terms of another. Back when I was first studying kinetics and reaction engineering, I had to do this often as one part of solving various problems. From that experience, I know that the way to do this is to express each of the final values in terms of the apparent extent of reaction, and then eliminate the apparent extent from the two equations. Now that I solve most problems numerically, I don't need to do this as often as I used to, but it's still good to know how to do it.

I could write a mole table, but with only one reaction taking place, I think I can get by without one.
...
This is an open, steady-state system, so the concentration of NO~2~ is defined as shown in equation (2). Both the molar flow rate and the volumetric flow rate in this equation need to be expressed in terms of $\dot n_{N_2O_5}$ and the other variables listed in the problem statement.

$$
C_{NO_2} = \frac{\dot n_{NO_2}}{\dot V} \tag{2}
$$

As shown in equation (3), the molar flow rate of N~2~O~5~ can be expressed in terms of the apparent extent of reaction, and the resulting equation can be solved for the apparent extent of reaction. Then, as shown in equation (4), the molar flow rate of NO~2~ can be expressed in terms of the apparent extent of reaction. Substituting the expression for the extent of reaction found in equation (3) yields an expression for the molar flow rate of NO~2~ in terms of the molar flow rate of N~2~O~5~ as shown in the second equality of equation (4).
...
Assuming that the gases in this problem are ideal, the volumetric flow rate can be expressed in terms of the total molar flow rate using the ideal gas law. The total molar flow rate can then be expressed in terms of the apparent extent of reaction. Here, the total moles increase by 3 each time the reaction occurs (two moles of reactants yield five moles of products), equation (5). Noting that the total inlet molar flow rate is the sum of the inlet molar flow rates of N~2~O~5~ and N~2~ and again substituting the expression for the apparent extent of reaction, equation (3), yields an expression for the volumetric flow rate as shown in the final equality of equation (5).
...
Substitution of the expressions for the molar flow rate of NO~2~ and the volumetric flow rate, equations (4) and (5), into equation (2) yields the requested expression for the concentration of NO~2~ in terms of the molar flow rate of N~2~O~5~, the inlet molar flows, the temperature, and the pressure, equation (6).
...
That answers the first part of the question. It remains to calculate the value of the concentration of NO~2~ if the final mole fraction of N~2~ is 0.3. As before, I know the initial composition and I know one measure of the final composition (the mole fraction of N~2~) and am asked to find a different measure of the final composition (the concentration of NO~2~). 

I know that different measures of final composition are related through the apparent extent of reaction. So normally I would express the mole fraction of N~2~ in terms of $\dot \xi$, solve the resulting expression for the value of $\dot \xi$, express the concentration of NO~2~ in terms of $\dot \xi$, and substitute the value of $\dot \xi$ to cacluate the value of the concentration of NO~2~.

Here, however, I already have an expression for the concentration of NO~2~ in terms of the molar flow rate of N~2~O~5~, so I'm going to alter that approach slightly. After I calculate the apparent extent of reaction from the mole fraction of N~2~, I'm going to calculate the molar flow rate of N~2~O~5~ and then substitute the result into equation (6) to calculate the concentration of NO~2~.

:::

By definition, the mole fraction of N~2~ for a flow system is the molar flow rate of N~2~ divided by the total molar flow rate. Each of those quantities can be expressed in terms of the apparent extent of reaction as shown in equation (7). The only unknown quantity in equation (7) is the apparent extent of reaction, so equation (7) can be solved to find the value of $\dot \xi$. 
...
The molar flow rate of N~2~O~5~ can then be calculated using equation (8). That result can be substituted into equation (6) to find the final concentration of NO~2~.
...
Equation (6) expresses the concentration of NO~2~ in terms of the molar flow rate of N~2~O~5~, the inlet molar flows, the temperature, and the pressure. For the conditions described in the problem, the outlet concentration of NO~2~ is `r results$value[1]` mol L^-1^.
...
The rank of this matrix will then equal the number of mathematically independent reactions. Once I know the number of independent reactions, I can build a matrix that only contains independent reactions. To do so, I start with a matrix that only contains the first reaction. This has to be independent and will have a rank of 1. Then I add reactions one at a time. If adding the next reaction increases the rank, I retain the added reaction, otherwise I discard it. I keep doing this until the number of retained reactions equals the number of mathematically independent reactions that I determined previously.

:::

**Reaction Matrix**

Equation (7) shows a reaction matrix for this problem. The first row corresponds to reaction (1), the second row to reaction (2), and so on. The first column corresponds to CO, the second to H~2~, the third to CH~3~OH, the fourth to CH~4~, the fifth to H~2~O, and the last to CO~2~. Looking at reaction (1), the stoichiometric coefficient of CO in that reaction is -1, and so there is a -1 in the first column of row 1. The stoichiometric coefficeint of H~2~ in the first reaction is -2, so there is a -2 in the second column of row 1. The stoichiometric coefficient of CH~3~OH in reaction (1) is +1, so there is a 1 in the third column of row 1. CH~4~, H~2~O, and CO~2~ do not appear in reaction (1), so there are zeros in the fourth, fifth and sixth columns of row 1 in equation (7). The remaining rows are filled in analogously.
...
It is easy to find that the rank of the matrix in equation (7) is equal to 3. While this could be done by hand, it is easier to use a computer program that performs matrix calculations. The meaning of the rank being equal to 3 is that there will be 3 equations in any complete, mathematically independent subset of reactions (1) through (6). Looking at reactions (1) and (2), it is apparent that they do not contain the exact same reagents. As a consequence, reactions (1) and (2) must be mathematically independent; neither is a simple multiple of the other. It remains to find a third mathematically independent reaction. To continue, the matrix shown in equation (8) is generated. The rows in this matrix correspond to reactions (1), (2), and (3). It is again easy to find that the rank of the matrix in equation (8) is 2. Since reactions (1) and (2) are mathematically independent, this means that reaction (3) is not independent; it is a linear combinations of reactions (1) and (2).
...
Proceeding, reaction (3) is removed from the matrix and replaced with reaction (4). This gives the matrix shown in equation (9). The rank of the matrix shown in equation (9) is 3, so the equations corresponding to the rows of that matrix, in this case reactions (1), (2), and (4), represent a complete, mathematically independent subset of reactions (1) through (6).
...
3. Express the desired composition measures in terms of the apparent extents of reaction.
4. Substitute the values of the apparent extents of reaction from step 2 into the expressions for the disired composition measures from step 3 and calculate the values of the desired composition measures.
...
Values are given for the conversion, yield and selectivity. Each of these quantities is related to the initial and final molar amounts as shown in equations (7) through (9). The expressions for the molar amounts in @tbl-mole_table_reb_3_3 can then be substituted to obtain the final equalities in equations (7) through (9).
...
| Symbol | Meaning |
|:-------|:--------|
| $eq$ | Abbreviation for "equilibrium." |
| $f_i$ | Fractional conversion of reactant $i$. |
| $g_i$ | Fraction of equilibrium conversion of reactant $i$. |
| $i$ | Index denoting a specific reagent. |
| $j$ | Index denoting a specific reaction. |
| $j^\prime$ | Index denoting a specific reaction from a complete, mathematically independent subset of the reactions occurring in the system. |
...
| $N_i$ | Number of molecules of species $i$, ; an additional subscripted $0$ denotes the initial number of molecules of $i$. |
...
| $\xi_{j^\prime}$ | Apparent extent of reaction $j^\prime$. |
| $\dot \xi_{j^\prime}$ | Apparent extent of reaction $j^\prime$ in an open, steady-state system. |
