...
## Creating a Response Function
 ...
 @sec-3_design_eqns described how to generate, simplify and solve the reactor design equations for each of the four ideal reactor types.  The quantities that appear in the reactor design equations were categorized as reaction properties, fluid properties, reactor parameters, reactor input and reactor output. @sec-3_response_func then introduced response functions. A response function is a computer function. Using response functions facilitates the use of numerical methods when performing reaction engineering tasks.
 
The reaction properties, fluid properties, reactor parameters, and reactor input are provided to the response either internally or as arguments. Within the response function, the reactor output  is found by solving the design equations. It then uses the reactor output to calculate and return the reactor response of interest if it includes quantity other than the reactor outputs. Response functions were used for kinetics data analysis in the previous section of *Reaction Engineering Basics* where the response of interest was the quantity measured in the kinetics experiments after all of the inputs had been set. 

Response functions are used again in this section of the book for modeling isolated ideal reactors. When modeling an isolated ideal reactor, the reaction and fluid properties are known. Those known values, together with known reactor parameters and reactor inputs, should be made available within the response function. An unknown reactor parameter or input should be passed to the response function as an argument. Similarly, when performing an optimization task, the optimization parameter should be passed to the response function as an argument.
...
The reactor response function described above can be used in every isolated ideal reactor analysis. It is used to perform response, optimization, and design tasks. Because the nature of these tasks differ, the manner in which the response function is used also differs depending upon the task.
...
**Design tasks** involve much more thought and analysis on the part of the reaction engineer. Chapters [-@sec-5_non_continuous_design] and [-@sec-5_continuous_design]  provide a small glimpse of the design process. As noted above, design almost always includes one or more optimization sub-tasks. Those sub-tasks can be performed as described here.
...
This chapter has presented a general description of modeling isolated ideal reactors. It was not specific to any reactor type, and for this reason, no examples are included with this chapter. In the absence of illustrative examples, some readers may have difficulty assimilating this chapter's verbal description how isolated reactor modeling calculations are performed. Examples that illustrate how the calculation are performed for each kind of task and for each ideal reactor type are included in the chapters that follow.
