...
The one exception to this occurs when the missing input is the reaction time (for a BSTR or SBSTR) or the reactor lenght (for a steady-state PFR). In those cases, the specified response can be used as the stopping criterion for solving the reactor design equations, and doing so will yield the missing value. (See Chapters [-@sec-5_bstr_analysis], [-@sec-5_sbstr_analysis] and [-@sec-5_pfr_analysis].)
