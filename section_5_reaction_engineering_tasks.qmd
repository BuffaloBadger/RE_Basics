...
@sec-4_kin_data_gen then introduced response functions. A response function is a computer function. The reaction properties, fluid properties, reactor parameters, and reactor input are provided to it either internally or as arguments. The response function first calculates the reactor output by solving the reactor design equations. It then uses the reactor output to calculate and return the reactor response of interest, if it is a quantity other than the reactor output.
