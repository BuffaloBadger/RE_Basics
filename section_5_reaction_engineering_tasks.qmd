...
The general approach suggested above entails creating two computer functions. Generally the response function solves the reactor design equations, calculates quantities of interest in the assignment, and returns the values of those quantities of interest. The calculations function uses the response function to perform all of the calculations needed to complete the assignment. In some cases the calculations function does little more than calling the response function and displaying and saving the results. In other cases the calculations function may process the results returned by the response function. Examples of this include plotting the results, finding the minimum or maximum value of results returned by the response function, etc. In less straightforward scenarios, the calculations function may need to call other system functions, passing the response function to them as an argument and then processing the information they return. An example of this type was encountered when performing kinetics data analysis: The calculations function for an assignment passed the response function to a parameter estimation function, together with other arguments, after which the calculations function processed, reported and saved the information returned by the parameter estimation software.
