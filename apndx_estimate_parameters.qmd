...
The formulation of the **reactor model** should

* list the reactor design equations
* identify the initial values and stopping criterion for IVODEs or the initial guesses needed for ATEs
* list any required input must be provided in order to solve the design equations
* describe the derivatives function or residuals function including
    * the arguments provided to it
    * the quantities it returns
    * the algorithm and equations it uses to calculate the return values
* describe the use of an ATE or IVODE solver to solve the design equations including
    * calculating unknown initial values, unknown final values, or initial guesses
    * the arguments passed to the solver and the quantities returned by it

The formulation of the **predicted responses function** should list the arguments provided to it and the values it returns. It should provide the algorithm it uses, noting that for each expeirment it uses the reactor model to solve the design equations and then the results to calculate the model-predicted response.

The use of a **numerical fitting function** to estimate the rate expression parameters and statistics should be described including making an initial guess for the rate expression parameters, identifying the arguments that will be passed to the fitting function, identifying quantities that will be returned by the fitting function, and describing any final calculations that must be perfromed on the returned values

The generation of **assessment graphs** should be described, noting what will be plotted and how the plotted quantities will be calculated.

Finally a succinct **summary** of how to use the components described above to perform the calculations should be added.
