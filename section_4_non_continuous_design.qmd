...
@sec-4_response_opt_design describes and differentiates between response, optimization, and design tasks or problems. @sec-4_bstr_analysis and @sec-4_sbstr_analysis examine the modeling of two kinds of non-continuous, ideal reactors: BSTRs and SBSTRs. They illustrate the use of the reactor design equations for those non-continuous reactors to complete reaction engineering assignments involving response and optimization tasks. This chapter introduces design tasks for non-continuous ideal reactors.
...
Chemical reactors are virtually always one part of a larger process. The highest level of design starts with a decision to build a process from scratch. The decision to do this would be informed by some number of preliminary analyses of the market for the products from the process, the economic potential of the process, the technical feasibility of the process, etc. Full reactive process design will involve a team comprising members with expertise in finance, planning, procurement, law, construction, chemistry, engineering, safety, etc. The members with engineering epertise would include a reaction engineer among other engineering specializations including separations, control, heating and cooling, etc.
...
Some of the economic factors that must be evaluated as part of the process design are the capital costs (one time expenses to purchase the necessary equipment, etc.), the operating costs (including labor, utilities, feedstock costs, waste disposal, etc.), maintenance costs, licensing costs (e. g. royalties for use of patented technology), income the process will generate and the return on investment. Business and legal activities include choosing the site for the process, securing necessary permits, licensing any necessary technology, identifying all regulatory restrictions that apply to the process, and purchasing the equipment as specified by the design engineers.The engineering team lays out the piping and designs the individual components, making sure that they are all properly and efficiently integrated. Specific tasks that a reaction engineer would be involved in include selecting the type of reactor(s) to be used, sizing the reactor(s) and specifying operational prameters (heating and cooling, feed temperature and composition), specifying turn-around tasks and timing for non-continuous processes, and developing procedures for start-up, operation, shut-down, and various possible emergencies.

## Preliminary Reactor Design

Participation in a full reactive process design of the kind just described requires much deeper knowledge of reaction engineering and much more reaction engineering experience than most readers of this book are likely to possess. Preliminary design involves a more limited scope. A preliminary reactor design might be one of the factors that go into the decision to build the full process. The goals of a preliminary design can include identifying the kind of reactor and the preferred way to operate it for the process under consideration. It might use approximate feed compositions and educated estimates of necessary conversion and selectivity. A detailed economic model is not required, but an awareness of factors that affect both capital and operating costs is important.

Preliminary design starts with an underspecifed reactor process. That is, there isn't enough information about the reactor and its operation to generate reactor design equations and perform an analysis. One way to envision preliminary design is as a process wherein the reaction engineer is given a set of design specifications. The engineer then adds sufficient specifications so that the proposed reactor can be modeled and analyzed. The engineer must choose which additional values to specify. There are three general kinds of specification that the engineer can use. First, the engineer can specify fixed values for some quantities. Second, the engineer can specify that one particular quantity (the key metric) must have the smallest or largest value possible. Third, if the engineer specifies a key metric that must be maximized or minimized, then the engineer must specify one or more quantities (the optimization parameters) that should have values that result in the key metric being maximized or minimized. In other words, the engineer specifies that the key metric is maximized or minimized with respect to the opimization parameters. Once the reactor is fully specified, the engineer models it to determine how it performs.

Often, the engineer will generate multiple sets of additional specifications and analyze and compare the performance of the corresponding reactors. The results might inspire or suggest a different set of added specifications. In some cases the type of reactor to be used may not be among the design specifications. The engineer might elect to consider two different kinds of reactor and develop full specifications for each reactor type. Ultimately, the simulated reactor performace for each set of specifications are compared, and the most promising reactor specifications are selected. The different reactor types that the engineer considers and the various combinations of specifications they make can be referred to collectively as the engineer's design strategy.
...
If the design specifications set the net production rate, but do not fix either the volume per batch or the time per batch, then the design engineer can specify one or the other. If the reaction engineer specifies the volume per batch, the time per batch is constrained as shown in @eq-time_per_batch_constraint, and if the engineer specifies the time per batch, the volume per batch is constrained as shonw in @eq-volume_per_batch_constraint. Noting that the BSTR and SBSTR design equations are IVODEs, and that processing time is the independent variable while the volume is a constant that appears within the design equations, the solution of the IVODEs is a little easier if the engineer specifies the volume per batch. (See @sec-apndx_solve_ivodes.)
...
The selling price of the product may depend upon its purity, and production of hazardous wastes can add to operating costs. In both of these cases, selectivity may be an important metric that is related to process profit. Each situation will have key metrics that have a significant impact on process profits. In a preliminary design these metrics can be appropriately optimized in lieu of an economic analysis.
...
The missing values can be specified in two ways. They can be assigned a fixed value, or they can be used as optimization parameters. Typically fixed values are specified for the quantities that are not expected to strongly affect the key metric or those that are difficult to vary. Quantities that are expected to strongly affect the key metric typically are used as optimization parameters. (Note, the optimization tasks illustrated in Chapters [-@sec-4_bstr_analysis] and [-@sec-4_sbstr_analysis] involved a single optimization parameter and possibly a single constraint. However computer functions are readily available for maximizing or minimizing the key metric with respect to multiple optimization parameters and subject to multiple constraints.)

It should be apparent that when a reaction engineer performs a preliminary design task the engineer will fully specify one or more reactor systems, model each system, and calculate the key reactor metric for each system. The key metric may be calculated directly, or it might be a minimum or maximum that is found by optimzing other parameters. Other reactor metrics may also be calculated and used when comparing the reactor systems. The different sets of fully specified reactors, and the way the key metric is calculated in each (directly or minimization/maximization) constitute the engineer's design strategy. A key to efficient and effective design is a deep understanding of reactive processes and an ability to think through the consequences of changing various reactor parameters without performing unnecessary calculations.
...
For this reason, the two examples provided here do not include any calculations. Instead, they focus on the engineer's thought processes and the resulting design strategy they develop. The engineer may revise or expand their design strategy they proceed through the analyses, but the examples here do not illustrate doing so. These examples are relatively simple, but they suffice to illustrate ways engineers might think and design strategies they might develop based on that thinking.
...
* For this non-continuous process, I could use either a BSTR or an SBSTR.
    ...
    * I'll start with a design strategy that assumes the reactor operates as a BSTR.
...
* I'll assume that capital and operating costs scale with the reactor volume, in which case I'll want to minimize the volume of the reactor.
    * I don't have to minimize the volume directly; maximizing the number of batches or minimizing the processing time per batch will simultaneously minimize the volume.
    * I will use the processing time as the key metric and minimize it.
    * Increasing the rate will decrease the processing time.
* Having some Z present initially is essential for an acceptable rate.
    ...
    * This
* The temperature will rise continually during processing; the largest average rate will result if the final temperature is as large as possible
* The product that is left in the reactor during turnaround can be left at the final temperature or it can be cooled
    * I'll use the volume of product left in the reactor after processing and its temperature at the instant A starts to be added (either all ar once in BSTR mode or as a feed stream in SBSTR mode) as two of the optimization parameters.
        * If the reactor operates as a BSTR that will fix the initial amount of A as the amount needed to fill the reactor and the initial temperature as the temperature that results after mixing the fresh feed with the product that was left in the reactor.
        * If the reactor operates as an SBSTR, the same amount will be added, but the feed rate of A can be used as another optimization parameter.
    * To provide a margin of safety and keep the product from boiling, I'll specify the final temperature to be 90 °C.
    * I'll need to apply a constraint that the final conversion must be greater than or equal to 99%.
* If I cannot keep the reaction temperature below 90 °C, I'll need to revise my design strategy to use an SBSTR.
