...
$$
r_{i,j} =\sum_{j\text{-}m}\nu_{i,j\text{-}m}r_{j\text{-}m}
$$ {#eq-gen_rate_from_mech}

$$
r_{i,j} = \sum_{j\text{-}m}\nu_{i,j\text{-}m} \left(k_{j\text{-}m,f} \prod_{i\text{-}r}\left[ i\text{-}r \right]^{-\nu_{i\text{-}r,j\text{-}m}} - k_{j\text{-}m,r}\prod_{i\text{-}p}\left[ i\text{-}p \right]^{\nu_{i\text{-}p,j\text{-}m}} \right)
...
$$
r_{j\text{-}m,\text{insignificant}} = 0
...
$$
r_{j\text{-}m,\text{irreversible}} = k_{j\text{-}m,f} \prod_{i\text{-}r} \left[ i\text{-}r \right]^{-\nu_{i\text{-}r,j\text{-}m}}
...
$$
r_j = r_{j\text{-}rd} = k_{j\text{-}rd,f} \prod_{i\text{-}r}\left[ i\text{-}r \right]^{-\nu_{i\text{-}r,j\text{-}rd}} - k_{j\text{-}rd,r}\prod_{i\text{-}p}\left[ i\text{-}p \right]^{\nu_{i\text{-}p,j\text{-}rd}}
$$ {#eq-rds_rate_expr}
...
$$
\Delta G_j = \sum_{j\text{-}m} \sigma_{j\text{-}m} \Delta G_{j\text{-}m}
$$ {#eq-delta_G_from_steps}

$$
\Delta G_j = \sigma_{j\text{-}rd} \Delta G_{j\text{-}rd}
$$ {#eq-delta_G_from_rds}

$$
\Delta G_{j\text{-}nrd} \approx 0
$$ {#eq-delta_G_not_rds}
...
$$
K_{j\text{-}nrd} = \prod_i \left[i\right]^{\nu_{i,j\text{-}nrd}}
...
$$
C_{0,cat} = C_{cat,free} + \sum_{i\text{-}c}\kappa_{i\text{-}c}C_{i\text{-}c}
$$ {#eq-cat_conservation}
...
In simple situations, it is usually safe to assume that the total surface area of the heterogeneous catalyst remains constant during the course of the reaction. It can also be safely assumed that the number (and therefore the concentration) of active sites, $C_{\text{sites}}$, remains constant. Note that this is a two-dimensional concentration (sites per surface area) not a three-dimensional one (sites per volume). In heterogeneous catalytic kinetics, the concentration of an adsorbed species, $i\text{-}s$, is usually expressed in terms of the fraction of the total sites that it covers, $\theta_{i\text{-}s}$. This quantity is then referred to as the **fractional coverage** or the surface coverage of species $i\text{-}s$. When this is done, the concentration, $C_{i\text{-}s}$ of any surface species, $i\text{-}s$, adsorbed on a single site is given by @eq-surf_conc_to_coverage.

$$
C_{i\text{-}s} = C_{\text{sites}} \theta_{i\text{-}s}
$$ {#eq-surf_conc_to_coverage}

Transition state theory can be used to derive the mathematical form of rate expressions for an elementary surface reaction. When this is done, the concentration of sites, $C_{\text{sites}}$, can be combined with the pre-exponential factor of the rate coefficient. After doing so, the resulting rate expression for an elementary surface reaction, $j\text{-}s$, is given in @eq-elem_surf_reaction_rate.

$$
\begin{align}
r_{j\text{-}s} &= k_{j\text{-}s,f}\prod_{i\text{-}r }\left[ i\text{-}r  \right]^{- \nu_{i\text{-}r ,j\text{-}s} } \prod_{i\text{-}rs}\theta_{i\text{-}rs}^{- \nu_{i\text{-}rs,j\text{-}s} } \\
&- k_{j\text{-}s,r}\prod_{i\text{-}p}\left[ i\text{-}p \right]^{ \nu_{i\text{-}p,j\text{-}s} }\prod_{i\text{-}ps}\theta_{i\text{-}ps}^{ \nu_{i\text{-}ps,j\text{-}s}}
\end{align}
...
$$
\theta_{\text{vacant}} + \sum_{i\text{-}s } \theta_{i\text{-}s } = 1
$$ {#eq-cons_surf_sites}
...
$$
C_{i\text{-}ma} \gg C_{i\text{-}nma}
$$ {#eq-fluid_mai}

$$
\theta_{i\text{-}ma} \gg \theta_{i\text{-}nma}
$$ {#eq-surf_mai}
...
Finally, the problem asks me to write the generalized rate expression predicted by the mechanism. From the information in this chapter, I know that there are two ways to relate the apparent rate of the non-elementary reaction to the rates of the mechanistic steps. One of those ways only applies if there is a rate-determining step. This mechanism does not have a rate-determining step, so I'll use the other approach, which involves using @eq-gen_rate_from_mech.

$$
r_{i,j} =\sum_{j\text{-}m}\nu_{i,j\text{-}m}r_{j\text{-}m}
$$

I can choose any of the three reagents appearing in the non-elementary reaction (1), and then use this equation to write an expression for its rate of generation. Here I will choose NO, but I would get a stoichiometrically equivalent rate expression if I chose N~2~ or O~2~. The stoichiometric coefficient of NO is zero for the reactions (2) and (5), and it is equal to 1 for reactions (3) and (4).

Since all mechanistic steps are elementary reactions, I know that the mathematical forms of the rate expressions for reactions (3) and (4) are given by @eq-rate_mech_step.

$$
r_{j\text{-}m} = k_{j\text{-}m,f} \prod_{i\text{-}r}\left[ i\text{-}r \right]^{-\nu_{i\text{-}r,j\text{-}m}} - k_{j\text{-}m,r}\prod_{i\text{-}p}\left[ i\text{-}p \right]^{\nu_{i\text{-}p,j\text{-}m}}
$$

The problem asks for the apparent rate of reaction (1), not the apparent rate of generation of NO via reaction (1), but I know that they are related by @eq-def_net_gen_rate.
...
The apparent rate of generation of NO, equation (6), can be found using @eq-gen_rate_from_mech.

$$
r_{NO,1} = r_3 + r_4 \tag{6}
$$

The mathematical forms of $r_3$ and $r_4$, equations (7) and (8), are given by @eq-rate_mech_step.
...
The apparent rate of reaction (1) can be found from the apparent rate of generation of NO via reaction (1) using equation (9).
