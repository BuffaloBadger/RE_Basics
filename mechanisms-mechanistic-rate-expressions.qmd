...
By definition, every reaction in a reaction mechanism is an elementary reaction. Consequently, the rate expression for each mechanistic step will have the functional form shown in @eq-net_rate_elem_rxn, or, for gases in @eq-net_rate_elem_rxn_part_press. Those equations include the equilibrium constant for the elementary step. Very often, in mechanistic steps, thermodynamic data are not available for the reactive intermediates. In those cases, the equilibrium constant for the mechanistic step won’t be known. For this reason, in this chapter the rate expressions for the elementary mechanistic steps will be written as shown in @eq-rate_mech_step. The square brackets in that equation can represent concentrations or (if the reagents are gases) partial pressures. When writing the rate expressions in this way, it is important to remember that the forward and reverse rate coefficients *must* be consistent with thermodynamics. The use of @eq-rate_mech_step when solving problems involving mechanisms is illustrated every example at the end of this chapter.

$$
\begin{align}
r_e =&\, k_{e,f} \prod_{i_1}\left[ i_1 \right]^{-\nu_{i_1,e}} - k_{e,r}\prod_{i_2}\left[ i_2 \right]^{\nu_{i_2,e}} \\
&\text{where } \\ 
&i_1 \text{ indexes all reactants in reaction } e \\
&i_2 \text{ indexes all products in reaction } e
\end{align}
$$ {#eq-rate_mech_step}
...
\begin{align}
r_{i,j} =&\,\sum_e\nu_{i,e}r_e \\
& \text{where } e \text{ indexes all steps in the mechanism}
\end{align}
$$ {#eq-gen_rate_from_mech}

$$
\begin{align}
r_{i,j} =&\,\sum_e\nu_{i,e} \left(k_{e,f} \prod_{n_1}\left[ n_1 \right]^{-\nu_{n_1,e}} - k_{e,r}\prod_{n_2}\left[ n_2 \right]^{\nu_{n_2,e}} \right) \\
&\text{where} \\
&e \text{ indexes all steps in the mechanism} \\
&n_1 \text{ indexes all reactants in reaction } e \\
&n_2 \text{ indexes all products in reaction } e
\end{align}
...
$$
\begin{align}
r_{e,\text{irreversible}} =&\, k_{e,f} \prod_i\left[ i \right]^{-\nu_{i,e}} \\
&\text{where } i \text{ indexes all reactants in reaction } e
\end{align}
...
$$
\begin{align}
r_j =&\, r_{e_\text{rds}} = k_{e_\text{rds},f} \prod_{i_1}\left[ i_1 \right]^{-\nu_{i_1,e_\text{rds}}} - k_{e_\text{rds},r}\prod_{i_2}\left[ i_2 \right]^{\nu_{i_2,e_\text{rds}}} \\
&\text{where} \\
&i_1 \text{ indexes all reactants in reaction } e_\text{rds} \\
&i_2 \text{ indexes all products in reaction } e_\text{rds}
\end{align}
$$ {#eq-rds_rate_expr}
...
$$
\Delta G_j = \sigma_{e_{\text{rds}}} \Delta G_{e_{\text{rds}}}
$$ {#eq-delta_G_from_rds}

$$
\Delta G_e \approx 0; \quad e \ne \text{rate-determining step}
$$ {#eq-delta_G_not_rds}
...
&\text{where } i \text{ indexes all species in the system}
\end{align}
...
$$
\begin{align}
C_{0,cat} =&\, C_{cat,free} + \sum_i\kappa_iC_i \\
&\text{where } i \text{ indexes all catalyst complexes}
\end{align}
$$ {#eq-cat_conservation}
...
$$
\begin{align}
0 =&\, \sum_p C_pq_p + \sum_n C_nq_n \\
&\text{where} \\
& p \text{ indexes all positively charged species} \\
& n \text{ indexes all negatively charged species}
\end{align}
...
$$
\begin{align}
r_j &= k_{j,f}\prod_{i_1}\left[ i_1 \right]^{- \nu_{i_1,j} } \prod_{i_2}\theta_{i_2}^{- \nu_{i_2,j} } - k_{j,r}\prod_{i_3}\left[ i_3 \right]^{ \nu_{i_3,j} }\prod_{i_4}\theta_{i_4}^{ \nu_{i_4,j} } \\
&\text{where} \\
&i_1 \text{ indexes all fluid phase reactants in reaction } j \\
&i_2 \text{ indexes all surface reactants in reaction } j \\
&i_3 \text{ indexes all fluid phase products in reaction } j \\
&i_4 \text{ indexes all surface products in reaction } j
\end{align}
...
$$
\begin{align}
\theta_{\text{vacant}}&\, + \sum_i \theta_i = 1 \\
&\text{where } i \text{ indexes all surface species}
\end{align}
$$ {#eq-cons_surf_sites}
...
$$
r_e = k_{e,f} \prod_{i_1}\left[ i_1 \right]^{-\nu_{i_1,j}} - k_{e,r}\prod_{i_2}\left[ i_2 \right]^{\nu_{i_2,j}}
$$
...
The problem states that step (6) is rate-determining, so the rate of the apparent, non-elementary reaction can be set equal to the rate of step (6). In addition, every mechanistic step, including step (6), is an elementary reaction. The mathematical form of the rate expression for elementary steps is known from theory, @eq-rate_mech_step.
...
| Symbol | Meaning |
|:-------|:--------|
| $e$ | Index or subscript denoting an elementary reaction; an additional subscripted "rds" denotes it is the rate-determining step. |
| $i$ | Index or subscript denoting a reagent. |
