...
## Numerical Solution of the Reactor Design Equations
...
## Examples
 ...
 The examples in this chapter illustrate choosing those design equations that are needed to perform a few specific reactor analyses. After the necessary design equations have been selected, the formulation of their solution leading to values for the reactor outputs is also presented. The examples involve each of the ideal reactor types and both steady-state and transient operation. They illustrate many of the common simplifications, a variety of ancillary equations, and the use of the minimum number of mole balances.
...
The problem asks me to write the design equations an ideal BSTR model for an isothermal system involving five reagents and one reaction. It further instructs me to use the minimum number of mole balances. Reagent I does not participate in reaction (1), so a mole balance *will* be needed for it. The minimum number of additional mole balances is equal to the number of independent reactions. Here there is only one reaction, so only one other mole balance is needed. I can write it for any of the reagents that participate in the reaction: A, B, Y, or Z; here I'll write it for A.

The problem also states that the reactor operates isothermally and the output temperature is known. That means the mole balances can be solved separately from the energy balances. Beyond that, the problem does not ask any questions or provide sufficient information about heating or cooling to allow solution of the energy balances. (That is, the heat transfer coefficient, heat transfer area, exchange fluid temperatures, etc. are not provided.) As such, energy balances on the reaction fluid and exchange fluid are not needed. Neither is a momentum balance needed because they are not used with stirred tank reactors.
...
The design equation that needs to be solved, equation (4), is an IVODE. That one equation constains one dependent variable, $n_A$, so no additional equations or simplification is needed. I know that IVODEs are *always* solved to find the values of the the independent and dependent variables over some range of the independent variable. I also know that to solve a set of IVODEs I need to provide three things: (1) ancillary equations to calculate every quantity appearing in, or substituted into, the IVODEs from known constants, the independent variable and the dependent variables, (2) initial values, and (3) a stopping criterion.
...
Prior to the widespread availability of computers it was quite common to formulate reactor modeling equations using the minimum number of mole balances. In a problem like this one, doing so makes it possible to solve the resulting IVODE analytically. Very often, using a single mole balance requires use of the apparent extent of reaction to calculate molar amounts of reagents other than the one used as the dependent variable. This was seen in this example where it was necessary to calculate $n_B$ for use in the rate expression. As a reminder, this use of the apparent extent of reaction was presented in @sec-1_reaction_progress.
...
It is useful to begin by listing all of the known constant quantities. Normally I would list the quantity *and its value*, but here I'm not told the values.
...
It is useful to begin by listing all of the known constant quantities; if their values were available, they would be included in the listing.
...
I know that because the design equations are DAEs I need to provide three things to solve them: (1) ancillary equations to calculate every quantity appearing in, or substituted into, the IVODEs from known constants, the unknown being found by solving the ATE, the independent variable and the dependent variables, (2) initial values, and (3) a stopping criterion.
