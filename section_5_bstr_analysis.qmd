...
This chapter examines reaction engineering of single, isolated batch stirred-tank reactors (BSTRs). Reasons vary for using a batch reactor and not a continuous reactor like a CSTR or PFR. BSTRs offer flexibility both in terms of the reactions being run and in the details of operating the process. In a situation where the demand for products is too small to warrant continuous production, a batch reactor might be used to make one product one week and a different product the next week. This kind of flexibility avoids having the reactor sit idle for extended periods of time. Similarly, if the production of a product involves several sequential steps, it may be easier to produce it in a batch reactor than to set up a chain of continuous reactors to handle each of the steps in succession. Batch reactos are often used to produce so-called value-added products. The market demand for value products is smaller, but they can be sold at a high price per pound. This is in contrast to commodity products where the demand for large quantities is high, but the price per pound is relatively small.

In addition to being unsuitable for the production of products in very large quantities, operating batch reactors is typically more labor intensive than operating CSTRs. This adds to the cost of production, meaning a higher selling price is necessary in order to make a profit. Another issue with batch processes is that of batch-to-batch consistency. As an example, consider the batch production of an expensive perfume. It is critically imporant that every batch smells the same. Food and beverage production similarly demand high batch-to-batch consistency. This can be even more challenging if, as an example, the process utilizes agricultural reagents that can vary from season to season or year to year.
...
3. Draw sets of axes for each plot with the quantity of interest on the y-axis and time on the x-axis.
4. Plot a point on each y-axis representing the initial values of the quantities of interest.
...
7. Determine whether the initial curvature eventually will result in the system approaching equilibrium (i. e. will the graph asymptotically approach a horizontal line?).
8. If the initial curvature will not lead to equilibrium, infer what must happen so that the system will go to equilibrium (i. e. will there be an inflection point or a minimum/maximum?), and qualitatively explain what will cause these changes in the curvature.

Qualitative analyses of this type are included in [Example -@sec-example_13_1] and [Example -@sec-example_13_4]. They can be very useful during the initial stages of a reactor design assignment when a reaction engineer is deciding what type of reactor to use, as will be discussed in @sec-5_continuous_design.

## BSTR Design Equations

The reactor design equations for BSTRs were presented in @sec-3_design_eqns. The BSTR mole and energy balances shown below were presented in Equations [-@eq-bstr_mole_balance] and [-@eq-bstr_energy_balance]. That chapter also presents the energy balances shown below for a heat exchange fluid that transfers sensible heat, @eq-exchange_energy_bal_sensible, and one that transfers latent heat, @eq-exchange_energy_bal_latent.  @sec-3_design_eqns further describes how to select the design equations needed to model BSTRs under different circumstances. That discussion will not be duplicated here.
...
When rate expressions are substituted into the BSTR mole and energy balances, they typically introduce concentrations or partial pressures of reagents. Those concentrations and partial pressures must be expressed in terms of the molar amounts of the reagents when the design equations are solved. The concentration is simply the molar amount divided by the volume, @eq-concentration_closed.
...
@sec-5_response_opt_design described how to identify an assignment that involves the modeling of an isolated ideal reactor and presented a general approach for doing so using a response function. This section describes the application of that approach to the modeling of an isolated BSTR. The first step is to write the reactor design equations for the system as described in @sec-3_design_eqns and in the preceding section of this chapter.
...
1. Call the response function to get sets of values of $t$, $C_A$, $C_B$, $C_Y$, $C_Z$, $T$, and $r$ at times between $t=0$ and $t=t_{rxn}$.
