# Ideal Reactor Models  {#sec-ideal-reactor-models}
...
$$\dot Q + \sum_i \dot n_{i,in} \hat h_{i,in} = \dot W +  \sum_i \left( n_i \frac{d \hat h_i}{dt} + \hat h_i \frac{dn_i}{dt} \right) - P\frac{dV}{dt} -V\frac{dP}{dt}$$

The summation in the fourth term can be written as two terms.

$$\dot Q + \sum_i \dot n_{i,in} \hat h_{i,in} = \dot W +  \sum_i \left( n_i \frac{d \hat h_i}{dt}\right) + \sum_i \left( \hat h_i \frac{dn_i}{dt} \right) - P\frac{dV}{dt} -V\frac{dP}{dt}$$
...
$$\dot Q + \sum_i \dot n_{i,in} \hat h_{i,in} = \dot W +  \sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} + \left(\sum_i  \hat h_i \frac{dn_i}{dt} \right) - P\frac{dV}{dt} -V\frac{dP}{dt}$$

The time derivative of $n_i$ can be replaced using the STSTR mole balance.

$$\begin{split}\dot Q + \sum_i \dot n_{i,in} \hat h_{i,in} &= \dot W +  \sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} + \left(\sum_i  \hat h_i \left(\dot n_{i,in} + V \sum_j \nu_{i,j}r_j \right)\right) \\&- P\frac{dV}{dt} -V\frac{dP}{dt}\end{split}$$

The distributive property of multiplication can be applied to the fifth term after which it can be written as two terms.

$$\begin{split}\dot Q + \sum_i \dot n_{i,in} \hat h_{i,in} &= \dot W +  \sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} + \sum_i   \dot n_{i,in}\hat h_i + \sum_i   \left(V\hat h_i \sum_j \nu_{i,j}r_j \right) \\&- P\frac{dV}{dt} -V\frac{dP}{dt}\end{split}$$

The order of the sums in the sixth term can be switched.

$$\begin{split}\dot Q + \sum_i \dot n_{i,in} \hat h_{i,in} &= \dot W +  \sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} + \sum_i   \dot n_{i,in}\hat h_i + V\sum_j  \sum_i \hat h_i\nu_{i,j}r_j  \\&- P\frac{dV}{dt} -V\frac{dP}{dt}\end{split}$$

The rate can be factored out of the inner sum in the sixth term.

$$\begin{split}\dot Q + \sum_i \dot n_{i,in} \hat h_{i,in} &= \dot W +  \sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} + \sum_i   \dot n_{i,in}\hat h_i + V\sum_j r_j \sum_i \hat h_i\nu_{i,j}  \\&- P\frac{dV}{dt} -V\frac{dP}{dt}\end{split}$$
...
$$\begin{split}\dot Q + \sum_i \dot n_{i,in} \hat h_{i,in} &= \dot W +  \sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} + \sum_i   \dot n_{i,in}\hat h_i + V\sum_j r_j \Delta H_j  \\&- P\frac{dV}{dt} -V\frac{dP}{dt}\end{split}$$

The derivatives can all be moved to one side of the equation.

$$\begin{split}\sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} - P\frac{dV}{dt} -V\frac{dP}{dt} &= \dot Q - \dot W + \sum_i \dot n_{i,in} \hat h_{i,in} - \sum_i   \dot n_{i,in}\hat h_i \\&+ V\sum_j r_j \Delta H_j\end{split}$$

The third and fourth terms following the equals sign can be combined and the molar flow rate of $i$ factored out.

$$\sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} - P\frac{dV}{dt} -V\frac{dP}{dt} = \dot Q - \dot W + \sum_i \dot n_{i,in} \left(\hat h_{i,in} - \hat h_i \right)+ V\sum_j r_j \Delta H_j$$
...
$$\sum_i \left( n_i \hat C_{p,i}\right) \frac{dT}{dt} - P\frac{dV}{dt} -V\frac{dP}{dt} = \dot Q - \dot W - \sum_i \dot n_{i,in} \int_{T_{in}}^T \hat C_{p,i}dT  + V\sum_j r_j \Delta H_j$$
