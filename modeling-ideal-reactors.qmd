# Reactor Design Equations {#sec-modeling-ideal-reactors}

Reactor models are essential for reaction engineering analyses. A reactor model is a set of equations that relates what goes into a reactor to what comes out. Other parameters that affect the relationship between what goes in and what come out appear within the model. In most cases the model equations cannot be solved analytically, making numerical solution necessary. For this reason, *Reaction Engineering Basics* assumes that the reactor model equations will always be solved numerically. This chapter describes and discusses reactor models and some aspects of their numerical solution.

## A Generic Reactor

The four types of ideal reactors used in *Reaction Engineering Basics*, batch stirred-tank reactors (BSTRs), semi-batch stirred-tank reactors (SBSTRs), continuous stirred-tank reactors (CSTRs), and plug flow reactors (PFRs), were briefly introduced in @sec-react-eng-def. Each of these will be discussed in detail later in this chapter. To begin, however, a generic reactor (one that could be any one of those four ideal reactor types) is considered. Doing so identifies the reactor input variables (what goes into the reactor), the reactor output variables (what comes out of the reactor), and the reactor model parameters (the things that affect the relationship between what goes in and what comes out).

Each of the four ideal reactor types has a compartment that contains reacting fluid. Each of them *may* have a second compartment which can be called the reactor shell or jacket. If the reactor does not require heating or cooling, that is if it operates adiabatically, it will not have a shell/jacket, but instead it will be insulated. Otherwise the reactor will have a shell/jacket through which an "exchange fluid" flows. It is called the exchange fluid because its purpose is to exchange heat with the reacting fluid. The exchange fluid is physically separated from the reacting fluid by a solid wall that prevents the two fluids from mixing. Heat is transferred through the wall separating the two reactor compartments.

There are other ways to heat and cool the reacting fluid, but in order to focus on the reactor and reactions, *Reaction Engineering Basics* only considers reactors that use an exchange fluid in a shell/jactet for heating or cooling, and even then, with additional constraints. The exchange fluid is always assumed to be a pure, single component fluid that is perfectly mixed within the volume of the shell/jacket. In addition, any heat that is transferred through the wall is associated with *either* latent heat *or* sensible heat of the exchange fluid, *but not both*. That is, as heat is transferred through the wall it is accompanied by either a change of exchange fluid phase or a change of exchange fluid temperature. 
...
Reactor input and reactor output were defined above as what goes into the reactor and what comes out of the reactor. These definitions work fine for the continuous reactors (CSTRs and PFRs). However, for BSTRs and SBSTRs the meaning of reactor input and reactor output requires additional refinement. For BSTRs the reactor input refers to what is present within the reaction compartment just before the reactions begin together with what flows into the shell/jacket. BSTR output refers to what is present within the reaction compartment right after the reactions end together with what flows out of the shell/jacket. The same meanings for input and output apply to SBSTRs, but SBSTR input additionally includes what flows into the reaction compartment while the reactions are taking place.

The words **"transient" and "steady state"** are used to differentiate two modes of reactor operation. When a reactor operates at steady state, one can watch any one point in the reactor and the conditions (temperature, pressure, composition, flow rate, etc.) will not change over time. At a different point in the reactor, the conditions may be different, but they, too, will be constant over time. Putting it another way, when a reactor operates at steady state, the conditions may vary spatially, but they won't vary temporally (i. e. over time). In a transient reactor one will observe changes in the conditions over time when watching any one point within the reactor. That is, within a transient reactor the conditions *will* vary temporally (and possibly spatially, too). Note that BSTRs and SBSTR are always transient while CSTR and PFR operating modes can be either steady state or transient.

### Reactor Variables and Reactor Model Parameters
...
The other difference is work done by the reacting system on its surroundings. If the reactor has a moving boundary, such as a pistion, it is possible for the reactor to perform mechanical work on its surroundings. That work is included among the reactor output variables. This is important, for example, if the combustion chamber in an automobile engine is modeled as a chemical reactor. The reactors considered in *Reaction Engineering Basics* are typically vessels located in a chemical processing facility or laboratory. The only moving boundaries associated with those reactors is usually a shaft connected to an agitator that is used to mix the reagents.

**Reactor model parameters** are variables that appear in the reactor model equations, and therefore can affect the relationship between the reactor input and output. They can be grouped in various and arbitrary ways. Here they are grouped as physical and operational parameters, fluid properties, and reaction properties.

Physical and operational parameters include the dimensions of the reactor (volume for stirred tanks or diameter and length for PFRs), the shell/jacket volume, the heat transfer area, the heat transfer coefficient, and some measure of the reaction time. If there is a packed bed of catalyst particles in the reactor, the bed porosity is also included with these parameters.
...
Reactor model parameters classified as fluid properties include heat capacities, density, viscosity, etc. for both the reacting fluid and the exchange fluid. Those classified as reaction properties include kinetics parameters and heats of reaction. Some of these properties may vary as a function of temperature, pressure or composition. In those cases, equations for calculating the property must be available, and any constants that appear in those equations are considered  parameters.

@tbl-reactor_variables summarizes the variable groups that are used when modeling an ideal reactor.
...
A separate equation is not always needed to related the output pressure to the input variables. In some cases, the outlet pressure can be calculated from the other *output* variables using the **equation of state** (i. e. using the ideal gas law or assuming liquids to have constant density). For gas phase reactions in the stirred tank reactors (BSTR, SBSTR, abd CSTR) a differential form of the equation of state *may* be needed.

A **momentum balance** must sometimes be included in a reactor model for a PFR. When it is included, the momentum balance relates the outlet pressure of the reacting fluid to the reactor inputs. Momentum balances are not needed with any of the stirred tank reactors. The momentum balance accounts for the conversion of kinetic energy associated with the flow of the reacting fluid to thermal energy. While this energy is negligible with respect to the energy balance, it can have a significant effect upon the pressure of the reacting fluid.

Collectively, the specific mole balances, energy balances, momentum balance, and differential form of the ideal gas law that are needed to model a particular reactor system are sometimes referred to as the **design equations** for that reactor system. In addition to the reactor design equations a few other types of equations are needed. **Rate expressions for every reaction** taking place it the system are *always* needed when modeling a reactor. As noted above, if heat capacities, heats of reaction, density, viscosity, etc., are functions of temperature, pressure, and/or composition, equations for calculating these quantities are needed, and any constants appearing in those equations become reactor model parameters. @tbl-reactor_equations summarizes the equations used to model a generic ideal reactor.
...
**A momentum balance is only needed when the reactor is a PFR**. Even then, some PFR problems indicate that pressure drop is negligible, in which case the momentum balance is not needed. Similarly, for gas phase systems **a differential form of the ideal gas law** is only needed when the design equations are a set of initial value ordinary differential equations and the number of dependent variables in the design equations is greater than the number of design equations.
...
As discussed in [Appendix -@sec-reactor-heat-transfer], there are two forms of the energy balance on the exchange fluid: one is used when the temperature of exchange fluid changes, and the other is used when the exchange fluid changes phase. Examples [-@sec-example_6_3] and [-@sec-example_6_5] illustrate writing, simplifying and solving reactor design equations where the exchange fluid temperature changes and [Example -@sec-example_6_4] illustrates writing, simplifying and solving reactor design equations where the exchange fluid undergoes a phase change.
...
*Reaction Engineering Basics* assumes that the reactor design equations will be solved numerically, but it does not provide computer code for doing so. Instead, the numerical solution of the design equations is fully formulated in a manner that the reader can use it to write the necessary computer code using whatever computer and software they choose. The results of solving the design equations are then presented. The full formulation of the solution of the design equations involves the following steps.

1. Write and simplify the necessary reactor design equations (see @sec-necessary_equations); if they include IVODEs, make sure that the number of IVODEs is equal to the number of dependent variables appearing in them.
2. Identify the unknown variables to be found by solving them.
    a. For the IVODEs the unknowns are the independent variable and the dependent variables over a specified range.
    b. For the ATEs it is a set of unknowns equal in number to the number of ATEs.
3. Write the ancillary equations to calculate every quantity that appears in the design equations from known constants and the unknowns.
4. If the design equations include IVODEs, provide the initial values and stopping criterion to be used when solving them.

Often solving the design equations does not complete the analysis. Additional calculations that use the results from solving the design equations may be necessary to complete the analysis.

## Examples

 The examples in this chapter illustrate the formulation of a solution of the reactor design equations. They involve each of the ideal reactor types and both steady-state and transient operation. They illustrate many of the common simplifications, a variety of ancillary equations, and the use of the minimum number of mole balances.
...
The problem asks me to formulate an ideal BSTR model for an isothermal system involving five reagents and one reaction. The problem instructs me to use the minimum number of mole balances. Reagent I does not participate in reaction (1), so a mole balance *will* be needed for it. The minimum number of additional mole balances is equal to the number of independent reactions. Here there is only one reaction, so only one other mole balance is needed. I can write it for any of the reagents that participate in the reaction: A, B, Y, or Z; here I'll write it for A.
...
Equation (3) was solved analytically, so it remains to solve equation (4).

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

The design equation that needs to be solved, equation (4), is an IVODE. That one equation constains one dependent variable, $n_A$, so no additional equations or simplification is needed. I know that IVODEs are *always* solved to find the values of the the independent and dependent variables over some range of the independent variable. I also know that to solve a set of IVODEs I need to provide three things: (1) ancillary equations to calculate every quantity appearing in the IVODEs from known constants, the independent variable and the dependent variables, (2) initial values, and (3) a stopping criterion.

To generate the necessary ancillary equations, I'll go through the mole balance one quantity at a time. If the quantity is a known constant, $t$, or $n_A$, I don't need an ancillary equation. If it isn't, I'll write an ancillary equation to calculate its value. If that ancillary equation contains quantities other than known constants, $t$, and $n_A$, I'll write ancillary equations for those quantities, and so on. I will continue this process until I have a set of ancillary equations that allow every quantity in the design equation to be calculated from known constants, $t$, and $n_A$.

It is useful to begin by listing all of the known constant quantities. That list can then be used as I go through the design equations one quantity at a time, allowing me to quickly see if the quantity is a known constant without needing to read through the entire problem statement. Normally I would list the quantity *and its value*, but here I'm not told the values.

:::

**Given Constants**: $V$, $n_{A,0}$, $n_{B,0}$, $n_{I,0}$, $n_{Y,0} = 0$, $n_{Z,0} = 0$, $T_0$, $P_0$, $k_{0,1}$, $E_1$, $t_0 = 0$, and $t_f$.

**Calculated Constants**: $n_I$, equation (3), and $T$, equation (5).
...
The molar amount of B, $n_B$, that appears in equation (8) can be calculated using equation (9) and the apparent extent of reaction, $\xi_1$, that appears in equation (9) can be calculated using equation (10).
...
The instant that the reaction begins can be defined as $t=0$ and used as the initial value of the independent variable. To solve equation (4), the corresponding initial value of the dependent variable $n_A$ must be specified. In this problem the initial moles of A is given.

A stopping criterion is also needed. Here the problem statement says that the reaction time is $t_f$, another given constant.
...
Using the initial values and the stopping criterion from @tbl-example_6_1, the design equation can be solved numerically to find the value of $n_A$ as a function of time in the range from $t=0$ to $t=t_f$.

:::{.callout-note collapse="false"}
## Note

Prior to the widespread availability of computers it was quite common to formulate reactor modeling equations using the minimum number of mole balances. In a problem like this one, doing so makes it possible to solve the resulting IVODE analytically. Very often, doing so requires use of the apparent extent of reaction to calculate molar amounts of reagents other than the one used as the dependent variable. This was seen in this example where it was necessary to calculate $n_B$ for use in the rate expression. As a reminder, this use of the apparent extent of reaction was presented in @sec-reaction-progress.
...
The reactor design equations are IVODEs. There are three design equations at this point, but they contain four dependent variables: $n_A$, $n_B$, $T$, and $P$. That means I either need another design equation or I need to eliminate one dependent variable from the current design equations. A differential form of the ideal gas law will provide the missing IVODE.
...
For a closed system, the differential form of the ideal gas law is given by @eq-differential_ideal_gas_law.

$$
P \frac{dV}{dt} + V \frac{dP}{dt} - R \left( T \sum_i \frac{dn_i}{dt} + \left( \sum_i n_i \right) \frac{dT}{dt} \right) = 0
$$

In this system, the volume is constant, so its time derivative is equal to zero.
...
The design equations form a set of four coupled IVODEs, equations (3) through (6). The independent variable is $t$ and the dependent variables are $n_A$, $n_B$, $T$, and $P$. Solving these equations will yield values of $t$ and corresponding values of $n_A$, $n_B$, $T$, and $P$ over a range of $t$. I know that to solve them I need to provide three things: (1) ancillary equations to calculate every quantity appearing in the IVODEs from known constants, the independent variable and the dependent variables, (2) initial values, and (3) a stopping criterion.

To generate the necessary ancillary equations, I'll go through the each of the design equations one quantity at a time. If the quantity is a known constant, $t$, $n_A$, $n_B$, $T$, or $P$, I don't need an ancillary equation. If it isn't, I'll write an ancillary equation to calculate its value. If that ancillary equation contains quantities other than known constants, $t$, $n_A$, $n_B$, $T$, and $P$, I'll write ancillary equations for those quantities, and so on. I will continue this process until I have a set of ancillary equations that allow every quantity in the design equation to be calculated from known constants, $t$, $n_A$, $n_B$, $T$, and $P$.

It is useful to begin by listing all of the known constant quantities. That list can then be used as I go through the design equations one quantity at a time, allowing me to quickly see if the quantity is a known constant without needing to read through the entire problem statement. Normally I would list the quantity *and its value*, but here I'm not told the values.

:::

**Given Constants**: $k_{0,1}$, $E_1$, $\Delta H^0_1$, $\hat C_{p,A}$, $\hat C_{p,B}$, $V$, $T_0$, $P_0$, and $T_f$.
...
The instant that the reaction begins can be defined as $t=0$ and used as the initial value of the independent variable. To solve equations (3) through (6), the corresponding initial values of the dependent variables, $n_A$, $n_B$, $T$, and $P$, must be specified. 

The reactor contains only reagent A, so the initial molar amount of reagent B is zero. The initial temperature and pressure are also given. The initial molar amount of reagent A can be calculated from the volume and initial temperature and pressure using the ideal gas law and noting that A is the only reagent initially present.

A stopping criterion is also needed. Here the problem statement provides the final temperature.

:::

Because A is the only reagent present initially, $n_{A,0}$ can be found using equation (9).

$$
n_{A,0} = \frac{P_0V}{RT_0} \tag{9}
$$

@tbl-example_6_2 provides the initial values needed to solve the design equations. Having defined $t=0$ as the time when the process begins, it will continue until the time when the temperature equals $T_f$, giving the stopping criterion that is also listed in @tbl-example_6_2. 
...
Solving the design equations gives the values of $n_A$, $n_B$, $T$, and $P$ over the range of times from $t=0$ to the time when the temperature reaches $T_f$. 
...
Substitution of the right side of equation (10) into equation (5) would then eliminate the derivative of P with respect to time, leaving *three* IVODEs with *three* dependent variables: $n_A$, $n_B$, and $T$.
...
I am asked to formulate a reactor model for a system with an ideal SBSTR, four reagents and one reaction. Mole balances for each of the four reagents will be included in the design equations. The design equations will also include an energy balance on the reacting fluid and an energy balance on the exchange fluid. Momentum balances are not used with SBSTRs.
...
Assuming that the reacting fluid is an incompressible ideal mixture, the change of the reacting fluid volume over time is equal to the inlet volumetric flow rate, equation (10).
...
The design equations form a set of seven coupled IVODEs, equations (4) through (10). The independent variable is $t$ and the dependent variables are $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$. Solving these equations will yield values of $t$ and corresponding values of $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$ over a range of $t$. I know that to solve them I need to provide three things: (1) ancillary equations to calculate every quantity appearing in the IVODEs from known constants, the independent variable and the dependent variables, (2) initial values, and (3) a stopping criterion.

To generate the necessary ancillary equations, I'll go through the each of the design equations one quantity at a time. If the quantity is a known constant, $t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, or $V$, I don't need an ancillary equation. If it isn't, I'll write an ancillary equation to calculate its value. If that ancillary equation contains quantities other than known constants, $t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$, I'll write ancillary equations for those quantities, and so on. I will continue this process until I have a set of ancillary equations that allow every quantity in the design equation to be calculated from known constants, $t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$.

It is useful to begin by listing all of the known constant quantities. That list can then be used as I go through the design equations one quantity at a time, allowing me to quickly see if the quantity is a known constant without needing to read through the entire problem statement. Normally I would list the quantity *and its value*, but here I'm not told the values.

:::

**Given Constants**: $\Delta H_1^0$, $\breve C_p$, $k_{0,1}$, $E_1$, $V_0$, $C_{B0}$, $C_{A,in}$, $T_{in}$, $\alpha$, $P$, $T_{ex,0}$, $T_{ex,in}$, $\dot V_{ex,in}$, $\tilde C_{p,ex}$, $\rho_{ex}$, $V_{ex}$, $A$, $U$, and $t_f$
...
The instant that the solution containing reagent A begins to flow into the system can be defined as $t=0$ and used as the initial value of the independent variable. To solve equations (4) through (10), the corresponding initial values of the dependent variables, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$, must be specified. 

Initially the reactor contains only reagent B, so the initial molar amounts of reagents A, S, and W are zero. The initial temperatures of the reacting and exchange fluids and the initial volume of the reacting fluid are given. The initial molar amount of reagent B can be calculated from the initial volume its initial concentration.

A stopping criterion is also needed. Here the problem statement provides the final time.

:::

The initial molar amount of B is calculated using its known initial concentration and the initial reacting fluid volume, equation (16).
...
Solving the design equations will yield the values of $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$ as functions of $t$ in the range from $t=0$ to $t=t_f$.
...
I'm asked to formulate a reactor model for a system with a steady-state CSTR, three reagents and one reaction. Mole balances for each of the three reagents, A, B, and Z, will be included in the design equations. The reactor is being heated by the condensation of steam, so an energy balance on the reacting fluid is also necessary.
...
The design equations for this system form a set of four ATEs, equations (3) through (6). I know that they can be solved to find the values of four unknown quantities. In this problem, I know all of the reactor input variables and reactor operating parameters, so I will solve the design equations to find the values of $\dot n_A$, $\dot n_B$, $\dot n_Z$, and $T$.

Before I can solve the design equations numerically, I need to write ancillary equations for calculating every other quantity that appears in the ATEs that isn't a known constant or one of the four variables they will be solved for, $\dot n_A$, $\dot n_B$, $\dot n_Z$, and $T$. To generate the ancillary equations, I'll go through the each of the design equations one quantity at a time. If the quantity is a known constant, $\dot n_A$, $\dot n_B$, $\dot n_Z$, or $T$, I don't need an ancillary equation. If it isn't, I'll write an ancillary equation to calculate its value. If that ancillary equation contains quantities other than known constants, $\dot n_A$, $\dot n_B$, $\dot n_Z$, and $T$, I'll write ancillary equations for those quantities, and so on. I will continue this process until I have a set of ancillary equations that allow every quantity in the design equation to be calculated from known constants, $\dot n_A$, $\dot n_B$, $\dot n_Z$, and $T$.

It is useful to begin by listing all of the known constant quantities. That list can then be used as I go through the design equations one quantity at a time, allowing me to quickly see if the quantity is a known constant without needing to read through the entire problem statement.

:::

**Given Constants**: $\Delta H_1^0$, $\tilde C_p$, $\rho$, $k_{0,1}$, $E_1$, $K_{0,1}$, $C_{A,in}$, $C_{B,in}$, $\dot V_{in}$, $T_{in}$, $V$, $P$, $T_{ex}$, $V_{ex}$, $A$, and $U$.
...
At this point the reactor design equations, equations (3) through (6), can be solved numerically to find values for the unknowns, $\dot n_A$, $\dot n_B$, $\dot n_Z$, and $T$
...
The design equations for this system form a coupled set of differential-algebraic equations (DAEs), equations (3) through (7). The IVODEs are equations (3) through (6) wherein $z$ is the independent variable and $\dot n_A$, $\dot n_{A_2}$, $\dot n_I$, and $T$ are the dependent variables. Solving them will yield values of $z$ and corresponding values of $n_A$, $n_B$, $T$, and $P$ over a range of $t$. The ATE, equation (6), can be solved to find the value of one other unknown. Here it will be solved to find $T_{ex}$.

I know that because the design equations are DAEs I need to provide three things to solve them: (1) ancillary equations to calculate every quantity appearing in the IVODEs from known constants, the unknown being found by solving the ATE, the independent variable and the dependent variables, (2) initial values, and (3) a stopping criterion.

To generate the necessary ancillary equations, I'll go through the each of the design equations one quantity at a time. If the quantity is a known constant, $z$, $T_{ex}$, $\dot n_A$, $\dot n_{A_2}$, $\dot n_I$ or $T$, I don't need an ancillary equation. If it isn't, I'll write an ancillary equation to calculate its value. If that ancillary equation contains quantities other than known constants, $z$, $T_{ex}$, $\dot n_A$, $\dot n_{A_2}$, $\dot n_I$ and $T$, I'll write ancillary equations for those quantities, and so on. I will continue this process until I have a set of ancillary equations that allow every quantity in the design equations to be calculated from known constants, $z$, $T_{ex}$, $\dot n_A$, $\dot n_{A_2}$, $\dot n_I$ and $T$.

It is useful to begin by listing all of the known constant quantities. That list can then be used as I go through the design equations one quantity at a time, allowing me to quickly see if the quantity is a known constant without needing to read through the entire problem statement.

:::

**Given Constants**: $k_{0,1}$, $E_1$, $\Delta H_1^0$, $\hat C_{p,A}$, $\hat C_{p,A_2}$, $\hat C_{p,I}$, $D$, $L$, $y_{A,in}$, $y_{I,in}$, $\dot V_{in}$, $T_{in}$, $P$, $T_{ex,in}$, $\dot m_{ex}$, $\tilde C_{p,ex}$, $V_{ex}$, and $U$.
...
The inlet to the reactor, $z=0$, is used as the initial value of the independent variable. To solve equations (3) through (6), the corresponding initial values of the dependent variables, $\dot n_A$, $\dot n_{A_2}$, $\dot n_I$ and $T$, must be specified. 
...
Solving the design equations will yield the values of $\dot n_A$, $\dot n_{A_2}$, $\dot n_I$, and $T$ as functions of $z$ in the range from $z=0$ to $z=L$ along with the value of $T_{ex}$ (which does not vary with $z$).
