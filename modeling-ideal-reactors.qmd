...
The four types of ideal reactors used in *Reaction Engineering Basics*, batch stirred-tank reactors (BSTRs), semi-batch stirred-tank reactors (SBSTRs), continuous stirred-tank reactors (CSTRs), and plug flow reactors (PFRs), were briefly introduced in @sec-react-eng-def. Each of these will be discussed in detail later in this chapter. To begin, however, a generic reactor (one that could be any one of those four ideal reactor types) is considered. Doing so helps identify the different kinds of variables needed to model a reactor as well as the equations are used to describe the relationships between those variables.
