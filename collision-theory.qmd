...
![Collisions According to the Kinetic Theory of Gases (a) Two point masses collide when the distance between them equals the sum of their collision radii. (b) As one point mass travels through space, it sweeps out a cylindrical volume.](./Graphics/collisions.pdf){#fig-collision}
...
$$
r_{ABC,f}=8N_{Av}\sigma _{AB}\sigma _{BC}IC_AC_BC_C\sqrt{\frac{2k_BT}{\pi}}\left( \frac{1}{\mu_{AB}} -\frac{1}{\mu_{BC}} \right)\exp{\left(\frac{-E_j}{RT}\right)}
$$ {#eq-forward-rate-ABC}
...
$$
r_{j,f} = k_{0,j,f}\sqrt{T}\exp{\left(\frac{-E_{j,f}}{RT}\right)}\prod_{i=1}^{N_{\text{Reactants}}} C_i^{-\nu_{i,j}}
$$ {#eq-gen-forward-rate}
...
$$
r_{j,r} = k_{0,j,r}\sqrt{T}\exp{\left(\frac{-E_{j,r}}{RT}\right)}\prod_{i=1}^{N_{\text{Products}}} C_i^{\nu_{i,j}}
$$ {#eq-gen-reverse-rate}
...
$$
k_{0,j,f}\sqrt{T}\exp{\left(\frac{-E_{j,f}}{RT}\right)}\prod_{i=1}^{N_{\text{Reactants}}} C_{i,eq}^{-\nu_{i,j}} =k_{0,j,r}\sqrt{T}\exp{\left(\frac{-E_{j,r}}{RT}\right)}\prod_{i=1}^{N_{\text{Products}}} C_{i,eq}^{\nu_{i,j}}
$$
...
$$
\begin{split}r_j &= k_{0,j,f}\sqrt{T}\exp{\left(\frac{-E_{j,f}}{RT}\right)}\prod_{i=1}^{N_{\text{Reactants}}} C_i^{-\nu_{i,j}} \\&- k_{0,j,r}\sqrt{T}\exp{\left(\frac{-E_{j,r}}{RT}\right)}\prod_{i=1}^{N_{\text{Products}}} C_i^{\nu_{i,j}}\end{split}
$$ {#eq-net-rate-as-diff}

$$
r_j = k_{0,j,f}\sqrt{T}\exp{\left(\frac{-E_{j,f}}{RT}\right)}\left(\prod_{i=1}^{N_{\text{Reactants}}} C_i^{-\nu_{i,j}}\right)\left(1-\frac{\prod_i C_i^{\nu_{i,j}}}{K_{j,eq_c}}\right) 
...
| $N_{\text{Products}}$ | Number of different reagents produced in a reaction. |
| $N_{\text{Reactants}}$ | Number of different reagents participating as reactants in a reaction. |
