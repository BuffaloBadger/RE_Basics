...
The mole balances are IVODEs, so to solve them I need initial values and a stopping criterion. The problem does not provide the inlet molar flow rates of the reagents, but it does provide the inlet volumetric flow rates. The ideal gas law can be used to calculate the inlet molar flow rates.
...
The initial values of the molar flow rates are given by equations (11) through (14) where $\tilde{V}_{STP}$ represents the molar volume of an ideal gas at standard temperature and pressure (1 mol = 22.4 L).
...
The first few data points are shown in @tbl-example_10_3_data; the full data set are available in the file, [reb_10_2_data.csv](./Data/reb_10_2_data.csv). This is the same data set as that used in [Example -@sec-example_10_2].
...
library(tidyverse)
source("./Code/fmt_tibble_col.R")
d <- read.csv("../RE_Basics_Examples/reb_10_3/python/reb_10_3_results.csv")
d <- fmt_tibble_col(d, 2, 3, 2, 2)
...
The reactor model was fit to the experimental data, and the results are presented in @tbl-example_10_3_results.
...
The estimated rate coefficient was used in the reactor model to calculate the model-predicted response and the residual for every experiment. The results were used to generate the parity plot shown in @fig-example_10_3_parity and the residuals plots shown in @fig-example_10_3_residuals_plots.

![Parity Plot.](./Graphics/reb_10_3_parity.png){#fig-example_10_3_parity width=80%}

::: {#fig-example_10_3_residuals_plots layout-ncol=2}

![](./Graphics/reb_10_3_residuals_vs_yAin.png)

![](./Graphics/reb_10_3_residuals_vs_yBin.png)

![](./Graphics/reb_10_3_residuals_vs_yYin.png)

![](./Graphics/reb_10_3_residuals_vs_yZin.png)
...
library(tidyverse)
source("./Code/fmt_tibble_col.R")
d <- read.csv("../RE_Basics_Examples/reb_10_3/python/reb_10_3_two_par_results.csv")
d <- fmt_tibble_col(d, 2, 3, 2, 2)
a <- c("k'","K~B~'",'R^2^')
b1 <- paste0(d$value[1],' ',d$units[1],', 95% CI [',d$value[2],', ',d$value[3],']')
b2 <- paste0(d$value[4],' ',d$units[4],', 95% CI [',d$value[5],', ',d$value[6],']')
...
The analysis was repeated a third time using equation (3) as the rate expression. The parameter estimation results are shown in @tbl-example_10_3_results_2_par. The parity plot is shown in @fig-example_10_3_parity_two_par and the residuals plots are shown in @fig-example_10_3_residuals_plots_two_par.
...
![Parity Plot for Rate Expression (3).](./Graphics/reb_10_3_2_par_parity.png){#fig-example_10_3_parity_two_par width=80%}

::: {#fig-example_10_3_residuals_plots_two_par layout-ncol=2}

![](./Graphics/reb_10_3_2_par_residuals_vs_yAin.png)

![](./Graphics/reb_10_3_2_par_residuals_vs_yBin.png)

![](./Graphics/reb_10_3_2_par_residuals_vs_yYin.png)

![](./Graphics/reb_10_3_2_par_residuals_vs_yZin.png)
