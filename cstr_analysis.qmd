...
The reactor design equations for CSTRs are derived in [Appendix -@sec-apxc_CSTR_balances]. The general CSTR mole balance is shown in @eq-gen_cstr_mol_bal, and the CSTR reacting fluid energy balance is presented in @eq-gen_cstr_energy_bal. The energy balances for a heat exchange fluid that exchanges only sensible heat, @eq-exchange_energy_bal_sensible, and for one that only exchanges latent heat, @eq-exchange_energy_bal_latent, are also reproduced below.
...
The transient CSTR design equations are initial value ordinary differential equations (IVODEs, see [Appendix -@sec-apxd_ivodes]). The independent variable is the elapsed time ($t$) and the dependent variables are the outlet molar flow rates of the reagents ($\dot{n}_i$), the volume of reacting fluid within the reactor ($V$), the outlet volumetric flow rate ($\dot{V}$), the reacting fluid temperature ($T$), the reacting fluid pressure ($P$), and either the exchange fluid temperature ($T_{ex}$) or the fraction of the exchange fluid that changes phase ($\gamma$). The number of dependent variables is three greater than the number of IVODEs, so before the design equations can be solved, either three IVODEs must be added or three of the dependent variables must be eliminated. 
...
For readers who are interested, the [Assignments Involving CSTR Analysis page from *REB, The Course*](https://buffalobadger.github.io/RE_Basics_Course/CSTR_examples.html) describes and discusses performing the calculations for each example in this chapter using Python and Matlab, and includes links to the source code.

:::

### Gas Phase Reaction in an Adiabatic, Steady-State CSTR{#sec-ch6_ex1}
...
[CSTR Design Equations]{.underline}
...
[CSTR Model Variables]{.underline}: $V$, $\dot{n}_B$, $\dot{n}_Z$, $\dot{n}_I$, and $T$.
...
1. Define the initial guesses for the reactor model variables.
...
2. Call the CSTR model function, passing the initial guess as an argument, to solve the design equations for the CSTR model variables.

3. Calculate the space time using its defining equation.
...
### Response of a Steady-State CSTR {#sec-ch6_ex2}
...
[CSTR Model Variables]{.underline}: $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_D$, $\dot{n}_U$, and $T$.
...
1. Initial guesses for the reactor model variables(provided as an argument to the CSTR model function).

2. Residuals function that
	a. receives guesses for the reactor model variables ($\dot{n}_A$, $\dot{n}_B$, $\dot{n}_D$, $\dot{n}_U$, and $T$),
	b. calculates the additional unknowns using equations, (3), (4), (10) - (15), and 
	c. evaluates and returns the CSTR design equation residuals, equations (5) - (9).

**Deliverables Function**

1. Define an initial guess for the CSTR model variables.
...
3. Calculate the conversion and yield.
$$
...
[CSTR Model Variables]{.underline}: $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_Z$, and $T$
...
1. Initial guesses for the CSTR model variables (provided as an argument to the CSTR model function).

2. $T_{in}$ must be made available to the residuals function.

3. Residuals function that
	a. receives guesses for the reactor model variables ($\dot{n}_A$, $\dot{n}_B$, $\dot{n}_Z$, and $T$)
	c. calculates the additional computable unknowns using equations (2), (3), (8) - (10), and
	d. evaluates and returns the CSTR design equation residuals, equations (4) - (7)
...
1. Define a range of values for $T_{in}$

$$
\underline{T}_{in} = \left(\left[75, \cdots, 125\right] + 273.15\right) \tag{11}
$$

2. Define an initial guess for the CSTR model variables for the first value of $T_{in}$
...
3. Loop through the values of $T_{in}$ and
	a. Calls the CSTR model function, passing the initial guess as an argument, to solve the design equations for the CSTR model variables.
	b. Calculates and saves the corresponding conversion and outlet temperature

$$
f_{A} = \frac{\dot{n}_{A,in} - \dot{n}_A}{\dot{n}_{A,in}} \tag{14}
$$

	c. Sets the initial guess equal to the current results
4. Identifies the maximum conversion within the selected range of $T_{in}$, the corresponding outlet temperature, and the value of $T_{in}$ that gave that conversion
...
[Parameter]{.underline}: $T$ (for multiplicity plot)
 
[Deliverables]{.underline}: $\underline{T}$ *vs.* $\underline{T}_{in}$ as a graph and $N_{ss}$ (for multiplicity plot); $f_A$ and $T$ (for steady state responses).
...
[CSTR Model Variables]{.underline}: $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_Y$, $\dot{n}_Z$, and either $T_{in}$ (Part A) or $T$ (Part B)
...
1. Initial guesses for the CSTR model variables (provided as an argument to the CSTR model function)
2. $T$ must be made available to the residuals function for multiplicity plot
3. Residuals function that
	a. receives guesses for the reactor model variables ($\dot{n}_A$, $\dot{n}_B$, $\dot{n}_Y$, $\dot{n}_Z$, and either $T_{in}$ (Part A) or $T$ (Part B))
	b. calculates the additional unknowns using equations (2), (8) - (10)
c.	evaluates and returns the CSTR design equation residuals, equations (3) - (7)

**Deliverables Functions**

1. For the multiplicity plot
	a. Define a range of values for $T$
$$
\underline{T} = \left[-25, \cdots, 350\right] + 273.15K \tag{14}
$$

	b. Define an initial guess for the CSTR model variables for the first value of 
$$
\dot{n}_{i,guess} = \dot{n}_{i,in} \qquad i = A, B, Y, \text{ and } Z \tag{15}
$$
$$
T_{in,guess} = T - 1\,K\tag{16}
$$

	c. Loop through the values of $T$ and
		i. Call the CSTR model function, passing the initial guess as an argument, to solve the design equations for the CSTR model variables
		ii. Save the value of $T_{in}$
		iii. Set the initial guess equal to the current solution of the design equations
	d. Plot $T$ *vs.* $T_{in}$
2. For calculating steady states
	a. Define initial guesses for the three steady states
$$
\dot{n}_{i,guess} = \dot{n}_{i,in} \qquad i = A, B, Y,  \text{ and } Z \tag{12}
$$
$$
T_{guess} = T \text{ read from multiplicity plot}\tag{13}
$$

	b. For each steady state
		i. Call the CSTR model function, passing the initial guess as an argument, to solve the design equations for the CSTR model variables
		ii. Calculate the conversion
$$
...
[CSTR Model Variables]{.underline}: $\underline{t}$, $\underline{\dot{n}}_A$, $\underline{\dot{n}}_B$, $\underline{\dot{n}}_W$, $\underline{\dot{n}}_X$, $\underline{\dot{n}}_Y$, $\underline{\dot{n}}_Z$, $\underline{T}$, and $\underline{T}_{ex}$
...
1. Initial values of the CSTR model variables

$$
t \big|_{t=0} = 0 \tag{19}
$$
$$
\dot{n}_i \big|_{t=0} = 0 \qquad i = A, W, X, Y, \text{ and } Z \tag{20}
$$
$$
\dot{n}_B \big|_{t=0} = C_{B,0}\dot{V} \tag{21}
$$
$$
T \big|_{t=0} = T_0 \tag{22}
$$
$$
T_{ex} \big|_{t=0} = T_{ex,0} \tag{23}
$$
...
3. Derivatives function that
	...
	b. calculates the additional unknowns using equations (4) - (6), and (15) - (18)
	c. evaluates and returns the design equation derivatives using equations (7) - (14)

**Deliverables Function**

1. Call the CSTR model function to get the CSTR model variables

2. Calculate the outlet concentration of B over time
...
[Given Constants]{.underline}: $V$ = 500 cm^3^, $\dot{V}_{in}$ = 1.0 cm^3^ s^-1^, $C_{A,in}$ = 0.015 mol cm^-3^, $C_{B,in}$ = 0.015 mol cm^-3^, $T_{in,u}$ = 50 °C, $\tilde{C}_p$ = 0.35 cal g^-1^ K^-1^, $\rho$ = 0.93 gm cm^-3^, $\Delta H_1$ = -20 kJ mol^-1^, $k_{0,1}$ = 3.24 x 10^12^ cm^3^ mol^-1^ s^-1^, and $E_1$ = 105.0 kJ mol^-1^

[Parameters]{.underline}: $\underline{f}_A$ = [0.03, 39.9, 97.5] %, $\underline{T}$ = ([50, 183, 265] + 237.15) K, and $\underline{\Delta T}$ = [20, 1, 20] K.
...
:::
[CSTR Model Variables]{.underline}: $\underline{t}$, $\underline{\dot{n}}_A$, $\underline{\dot{n}}_B$, $\underline{\dot{n}}_Y$, $\underline{\dot{n}}_Z$, and $\underline{T}$.
...
1. Initial values of the CSTR model variables (provided as an argument to the CSTR model function)
2. Stopping criterion (provided as an argument to the CSTR model function)
3. Derivatives function that
	a. receives values of the reactor model variables at the start of an integration step
	b. calculates the additional unknowns using equations (2) and (8) - (11)
	c. evaluates and returns the design equation derivatives using equations (3) - (7)

**Deliverables Function**

Loop through the three sets of parameter values, and for each set of parameters, $n$

1. define the initial values for a positive perturbation of the temperature

$$
t \big|_{t=0} = 0 \tag{12}
$$
$$
\dot{n}_A \big|_{t=0} = \dot{n}_{A,in} \left(1-f_{A,0,n}\right)\tag{13}
$$
$$
\dot{n}_B \big|_{t=0} = \dot{n}_{B,in} - f_{A,0,n}\dot{n}_{A,in} \tag{14}
$$
$$
\dot{n}_i \big|_{t=0} = f_{A,0,n}\dot{n}_{A,in} \qquad i = Y \text{ and } Z \tag{15}
$$
$$
T \big|_{t=0} = T_n + \Delta T_{n} \tag{16}
$$

2. define the stopping criterion

$$
t= 1 \text{ h} \tag{17}
$$

3. call the CSTR model function to get $\underline{t}$ and $\underline{T}$
	
4. repeat using a negative temperature perturbation
$$
T \big|_{t=0} = T_n - \Delta T_n \tag{18}
$$

5. Plot $\underline{T}$ *vs*. $\underline{t}$ for both perturbations
...
The duration of the transients in @fig-ch6_ex6_unsteady are different. This is likely because following the positive temperature perturbation the temperature is rising causing the reaction rate to rise also. Due to the faster reaction rates, the changes occur rapidly. Following the negative temperature perturbation, the temperature is decreasing and changes occur more slowly.
