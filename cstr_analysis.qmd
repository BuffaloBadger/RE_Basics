...
The requirement that the reacting fluid be perfectly mixed can be a disadvantage. If the reacting fluid has a high viscosity, mixing it thoroughly can be very difficult and can consume a great deal of power. Similarly if the reaction requires a solid catalyst, use of a CSTR is impractical or impossible because "stirring" a tank full of solid particles doesn't work well. 
...
The pressure is almost always constant in a CSTR in which case its time derivative is zero. For gases in a rigid-walled reactor, the same is true of the volume. For liquids, if the inlet and outlet flow rates are not equal, the reacting fluid volume in a CSTR will change over time. However, when the pressure or volume derivatives are not equal to zero, it is important to note that the $V\frac{dP}{dt}$ and $P\frac{dV}{dt}$ terms in the transient reacting fluid energy balance, @eq-gen_cstr_energy_bal, will have different units (pressure times volume per time) than the other terms (energy per time). The pressure-volume units can be converted to energy units by simply multiplying by the ideal gas constant in energy units and dividing by the ideal gas constant in pressure-volume units. The power required to agitate the reacting fluid in a CSTR is most often negligible, $\dot{W} = 0$, compared to heat terms.
...
### Stability of Steady States{#sec-ch6_stability}
...
[Example -@sec-ch6_ex6] presents a rigorous analysis of such a perturbation. However, a non-rigorous analysis may provide insight and understanding as to why the middle-temperature steady state is unstable. It involves a very roundabout way of solving the steady-state CSTR design equations, and can be illustrated by reference to [Example -@sec-ch6_ex4].

The reacting fluid energy balance in [Example -@sec-ch6_ex4], reproduced below, consists of two terms. One term represents sensible heat absorbed by the feed as it enters the reactor, @eq-heat_abs_term and the other represents the heat generated by the reaction, @eq-heat_gen_term. Clearly, the reacting fluid energy balance is only satisfied if the two terms are equal, @eq-heat_terms_equal.
...
That roundabout way of solving the design equations was applied to [Example -@sec-ch6_ex4]. The resulting heat generated and heat absorbed curves are shown in @fig-heat_gen_and_heat_abs. The curves cross three times corresponding to the three steady states. In the figure the three steady states are labeled "low", "middle," and "high."

![Heat absorbed and heat generated as function of the outlet temperature in [Example -@sec-ch6_ex4].](solutions/ch6_ex6/heat_vs_T.png){#fig-heat_gen_and_heat_abs width="80%"}
