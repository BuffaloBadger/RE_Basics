...
The full set of assumptions used in the ideal CSTR model is provided in [Appendix -@sec-apx_CSTR_balances]. While stirred tanks are the most common type of CSTR in commercial facilities, other physical geometries and formats are possible, but their contents must always be perfectly mixed. Stirred tanks are well-suited to liquid phase reactions because a simple impeller can be used to do the mixing. Attaining perfect mixing of gases in large vessels can be more challenging.
...
The transient CSTR design equations are initial value ordinary differential equations (IVODEs). The dependent variables are the molar flow rates, reacting fluid temperature, and exchange fluid temperature and the independent variable is the elapsed time. The design equations are solved to find corresponding sets of these variables over a span of time from the start of the transient until a one of them attains a known final value. In some instances, two final values are known, allowing the design equations to additionally be solved for one additional variable that does not vary with time. 

The numerical solution of IVODEs requires initial values for each of the dependent variables. This can lead to confusion when first learning to model transient CSTRs because both initial and inlet molar flow rates and temperatures appear in the design equations. The key point to remember is that the dependent variables in the IVODEs are the *[outlet]{.underline}* molar flow rates, temperature and exchange fluid temperature. As a consequence, the initial values that are needed to solve the IVODEs are the [initial]{.underline} values of the [outlet]{.underline} molar flows and temperatures, not their inlet values.

The pressure is almost always constant in a CSTR in which case its time derivative is zero. For gases in a rigid-walled reactor, the same is true of the volume. For liquids, if the inlet and outlet flow rates are not equal, the reacting fluid volume in a CSTR will change over time. In situations where the pressure or volume derivatives are not equal to zero, it is important to note that the $V\frac{dP}{dt}$ and $P\frac{dV}{dt}$ terms in the transient reacting fluid energy balance, @eq-gen_cstr_energy_bal, will have different units (pressure × volume ÷ time) than the other terms (energy ÷ time). The pressure-volume units can be converted to energy units by simply multiplying by the ideal gas constant in energy units and dividing by the ideal gas constant in pressure-volume units. The power required to agitate the reacting fluid in a CSTR is most often negligible, $\dot{W} = 0$, compared to heat terms.

When the reacting fluid is a liquid, the gravimetric or volumetric heat capacity of the whole solution may be known instead of the molar heat capacities of the individual reagents. When this is the case, the sensible heat terms can be modified as indicated below.
...
As previously noted, when a CSTR operates at steady-state, nothing changes over time. Consequently the time derivatives in the design equations all equal zero, resulting in the steady-state mole balance shown in equation @eq-ss_cstr_mole_bal and the steady-state energy balance shown in @eq-ss_cstr_energy_bal. For completeness, the steady state versions of the energy balances for a heat exchange fluid that exchanges only sensible heat, @eq-exchange_energy_bal_sensible_ss, and for one that only exchanges latent heat, @eq-exchange_energy_bal_latent_ss, are also reproduced below.
...
To illustrate, the black line in @fig-ch6_multiplicity shows the outlet temperature of a CSTR as a function of its inlet temperature, with all other inputs and parameters held constant. It can be seen that between the two vertical red lines, the temperature curve doubles back above itself twice leading to an "S" shape. The two red lines in the figure represent inlet temperatures where two steady states are possible. At any inlet temperature between the two red lines, three steady states are possible.  For example, the vertical blue line shows an inlet temperature where there are three possible steady states denoted as $S_{high}$, $U_{mid}$, and $S_{low}$. Only one steady state is possible at inlet temperatures less than the left red line or greater than the right red line.

![Outlet CSTR temperature as a function of inlet temperature. The blue line indicates an inlet temperature where three steady states ($S_{high}$, $U_{mid}$, and $S_{low}$) are possible. At the two inlet temperatures indicated by red lines two steady states are possible.](figures/CSTR_multiplicity.png){#fig-ch6_multiplicity}
...
Returning to @fig-ch6_multiplicity, one might wonder then, if the inlet temperature to the reactor is set to the inlet temperature indicated by the blue line, which of the three steady-state outlet temperatures shown in the figure will actually be observed? The answer to this question is that the transient procedure used to start up the reactor will determine which of the steady states is actually observed. Having said that, however, it is unlikely that the steady state labeled $U_{mid}$ will reached for reasons discussed next.

### Steady-State Stability {#sec-ch6_stability}

The red lines in @fig-ch6_multiplicity divide the black curve into three branches. A low temperature branch, shown as a solid line, starts at the lowest inlet temperature and ends at the higher temperature red line. An intermediate temperature branch, shown as a dashed line, then extends from the higher temperature red line back to the lower temperature red line. Finally a high temperature branch, shown again as a solid line, extends from the lower temperature red line to higher inlet temperatures. The significance of the dashed line is that the steady states with the intermediate outlet temperature are unstable while the solid branches represent stable steady states. Thus the steady states denoted as $S_{high}$ and $S_{low}$ are stable steady states while the one labeled $U_{mid}$ is unstable.

If a CSTR was somehow operating at an unstable steady state such as $U_{mid}$, a slight perturbation of any reactor property would initiate a period of transient operation. Depending on the specifics of the perturbation, that transient lead either the high temperature steady-state, $S_{high}$, or at the low temperature steady state, $S_{low}$. That is, if a CSTR somehow operates at an unstable steady state, it will not return to that unstable steady state following even a small perturbation away from it.

The instability following a small perturbation from a steady state can be demonstrated by performing a rigorous transient analysis, and there are other mathematical techniques that can be used to identify steady states that are unstable. Nonetheless, a non-rigorous analysis may provide more insight and better understanding of why the middle-temperature steady state is unstable. It involves a roundabout way of solving the steady-state CSTR design equations.

Consider an adiabatic, steady-state CSTR where the work term is negligible, as is usually the case. That leaves two terms in the reacting fluid energy balance, @eq-ss_cstr_energy_bal, as shown in @eq-ss_cstr_energy_bal_no_work. One term, $\dot{Q}_{abs}$, represents sensible heat absorbed by the feed as it enters the reactor, @eq-heat_abs_term and the other, $\dot{Q}_{gen}$, represents the heat generated by the reaction, @eq-heat_gen_term. Clearly, the reacting fluid energy balance is only satisfied if the two terms are equal, @eq-heat_terms_equal.
...
The solution of the design equation begins by choosing a range of values for the reactor outlet temperature. For each value in that range, the mole balance design equations are solved to find the outlet molar flow rates of the reagents. Knowing the inlet and outlet molar flow rates and the inlet temperature, $\dot{Q}_{abs}$ and $\dot{Q}_{gen}$ can be calculated for each outlet temperature in the chosen range using @eq-heat_abs_term and @eq-heat_gen_term. Finally $\dot{Q}_{abs}$ and $\dot{Q}_{gen}$ can each be plotted *vs*. outlet temperature on a single graph as shown in @fig-ch6_stability. The reacting energy balance is satisfied when the two curves cross, @eq-heat_terms_equal, so the steady-state outlet temperatures are the points where the curves cross. The curves cross three times corresponding to the three steady states, $S_{high}$, $U_{mid}$, and $S_{low}$.
...
* know the definition and/or defining equation for instantaneous yield, instantaneous selectivity, steady-state multiplicity, stable/unstable steady state, and parametric sensitivity
* understand that
...
* be able to
	...
	* select and simplify the CSTR design equations for modeling a given CSTR
	...
	* use instantaneous selectivity/yield to assess the effects of temperature and composition on overall selectivity
	* determine whether multiple CSTR steady states are possible and obtain solutions of the design equations for each of them
	* identify unstable steady states, ignition points and extinction points
	* use transient CSTR analysis to determine the stability or parametric sensitivity of a steady state

Reaction engineers must know how to analyze steady-state and transient CSTRs, isothermal, non-isothermal and adiabatic CSTRs, CSTRs that transfer latent and sensible exchange fluid heat, CSTRs with gas and liquid phase reacting fluids, CSTRs with multiple reactions, CSTRs with multiple steady states, and CSTRs with steady states that are unstable and parametrically sensitive. The examples that follow illustrate most of these possibilities, but a few of the heat transfer possibilities and isothermal operation are illustrated in later chapters.
...
#| echo: false
#| output: false
df <- read.csv('solutions/ch6_ex1/results.csv')
...
#| echo: false
#| output: false
df <- read.csv('solutions/ch6_ex2/results.csv')
df <- fmt_tibble_col(df, 2, 3, 3, 2)
```
...
#| echo: false
#| output: false
df <- read.csv('solutions/ch6_ex4/results.csv')
df <- fmt_tibble_col(df, 1:2, 3, 3, 2)
df2 <- read.csv('solutions/ch6_ex4/discussion_results.csv')
df2 <- fmt_tibble_col(df2, 2, 3, 3, 2)
```

The calculations were performed as described above. The steady-state outlet temperatures and conversions are presented in @tbl-ch6_ex4_results. The steady states at `r df[1,2]` and `r df[3,2]` °C are expected to be stable while the steady state at `r df[2,2]` °C is expected to be unstable.

```{r}
#| echo: false
#| warning: false
#| label: tbl-ch6_ex4_results
...
One way to determine whether multiple steady states are possible is to plot the outlet temperature as a function of the inlet temperature. The "trick" that makes this easier to do is to choose a range of **outlet** temperatures and then solve the reactor design equations to find the outlet molar flow rates and the **inlet** temperature corresponding to each outlet temperature. As noted in @sec-ch6_multiplicity, for each outlet temperature there is only one inlet temperature.

:::

The reactor model was used to calculate the steady-state outlet temperature and the steady-state conversion as functions of the feed temperature for this reactor. The results are presented in @fig-ch6_ex4_T_vs_Tin and @fig-ch6_ex4_f_vs_Tin. In both cases the curves exhibit the "S" shape described in @sec-ch6_multiplicity. The dashed sections of the (black) curves are unstable steady states (see @sec-ch6_stability). The ignition point occurs at an inlet temperature of 90.5 °C and is shown as a solid red line. The extinction point occurs at an inlet temperature of 3.9 °C and is indicated by a dashed red line.
...
![Steady-state outlet temperature as a function of inlet temperature.](solutions/ch6_ex4/T_vs_Tin.png){#fig-ch6_ex4_T_vs_Tin width="70%"}

![Steady-state conversion as a function of inlet temperature.](solutions/ch6_ex4/f_vs_Tin.png){#fig-ch6_ex4_f_vs_Tin width="70%"}
...
When these IVODEs are solved numerically, it will be necessary to evaluate the derivatives at the start of each integration step. That will be straightforward if they are written in the form of derivative expressions (see @sec-apxd_ivodes), so I'll multiply both sides of the equations by the volumetric flow rate and divide both sides by the reacting fluid volume.

$$
\frac{d \dot n_i}{dt} = \frac{\dot{V}}{V} \left(\dot n_{i,in} - \dot n_i + V\left(\nu_{i,1}r_1 + \nu_{i,2}r_2 + \nu_{i,3}r_3\right)\right)
$$

The reactor is not isothermal, so the mole balances cannot be solved independently, and an energy balance on the reacting fluid must be added to the design equations. The general form of the transient energy balance on the reacting fluid in a CSTR is given in @eq-gen_cstr_energy_bal. As previously noted, the time derivative of the reacting fluid volume is zero. The pressure is also constant, so its time-derivative is also zero. There are no shafts or moving boundaries that do work, so $\dot{W}$ is also equal to zero.
...
: Initial values and stopping criterion for solving  equations, (4) through (11). {#tbl-ch6_ex5_initial_values}

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

In order to solve the IVODEs numerically I'll need to do two things: calculate the values of the derivatives at the start of each integration step and calculate all of the initial and final values in @tbl-ch6_ex5_initial_values.
...
3. When it is necessary to calculate the initial and final values in @tbl-ch6_ex5_initial_values
...
3. Write a reactor model function that
	a. calculates the initial and final values in @tbl-ch6_ex5_initial_values as described in step 3 of the calculations summary,
	c. gets corresponding sets of values of $t$, $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_W$, $\dot{n}_X$, $\dot{n}_Y$, $\dot{n}_Z$, $T$, and $T_{ex}$, spanning the range from their initial values to their final values by calling an IVODE solver and passing the following information to it
		i. the initial values and stopping criterion in @tbl-ch6_ex5_initial_values and
...
### Perturbations from an Unstable Steady State and Near Ignition and Extinction Points{#sec-ch6_ex6}
...
The outlet temperature for this reactor was plotted as a function of the feed temperature in @fig-ch6_ex4_T_vs_Tin. That graph is reproduced here as @fig-ch6_ex6_initial_steady_states, with three steady states highlighted as red dots labeled "u," "ign," and "ext." Steady state u is the unstable steady state from [Example -@sec-ch6_ex4]. Steady state ign ($T_{in}$ = 89 °C, $f_A$ = 3.2%, and $T$ = 96 °C) is just below the ignition point at $T_{in}$ = 90.5 °C, and steady state ext ($T_{in}$ = 6 °C, $f_A$ = 88.1%, and $T$ = 200 °C) is just above the extinction point at $T_{in}$ = 3.9 °C

![Steady-state temperatures prior to perturbation.](solutions/ch6_ex4/initial_steady_states.png){#fig-ch6_ex6_initial_steady_states width="80%"}
...
This analysis involves a transient, adiabatic CSTR. To model it I will need mole balances and an energy balance on the reacting fluid. The general form of the transient CSTR mole balance is given in @eq-gen_cstr_mol_bal. In all three cases, the volumetric flow rate and the reacting fluid volume do not change, so their time derivatives are equal to zero. Only A and B flow into the reactor, so $\dot{n}_{Y,in}$ and $\dot{n}_{Z,in}$ will equal zero. There is one reaction, so the summation reduces to a single term.

$$
\frac{V}{\dot V}\frac{d \dot n_i}{dt} + \frac{\dot n_i}{\dot V}\cancelto{0}{\frac{dV}{dt}} - \frac{\dot n_iV}{\dot V^2}\cancelto{0}{\frac{d \dot V}{dt}} = \dot n_{i,in} - \dot n_i + V \cancelto{\nu_{i,1}r_1}{\sum_j \nu_{i,j}r_j}
$$

The general form of the transient CSTR reacting fluid energy balance is presente d in @eq-gen_cstr_energy_bal. As already noted, the reacting fluid volume is constant and its derivative is zero. The same is true of the pressure. Furthermore there is no heat exchange and negligible work.
...
: Initial values and stopping criterion for solving the reactor design equations, (3) through (7) for scenario $x$ ($u$, $ign$, or $ext$). {#tbl-ch6_ex6_initial_values}

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

In order to solve the IVODEs numerically I'll need to do two things: calculate the values of the derivatives at the start of each integration step and calculate all of the initial and final values in @tbl-ch6_ex6_initial_values.
...
When I solve the IVODEs numerically, I'll also need to calculate the initial and final values in @tbl-ch6_ex6_initial_values. The initial outlet temperature for the three analyses are provided in the assignment narrative. The outlet molar flow rates of A, B, Y, and Z at the instant the system is perturbed will equal their steady-state values just prior to the perturbation. They can be calculated using the steady-state conversions provided in the assignment narrative.
...
3. When it is necessary to calculate the initial and final values in @tbl-ch6_ex6_initial_values
...
2. Write a derivatives function that
	...
	c. returns the values of the derivatives.
3. Write a reactor model function that
	a. calculates the initial and final values in @tbl-ch6_ex6_initial_values as described in step 3 of the calculations summary,
	c. gets corresponding sets of values of $t$, $\dot{n}_A$, $\dot{n}_B$, $\dot{n}_Y$, $\dot{n}_Z$, and $T$, spanning the range from their initial values to their final values by calling an IVODE solver and passing the following information to it
		i. the initial values and stopping criterion in @tbl-ch6_ex6_initial_values and
...
![Reacting fluid temperature following a 1 °C increase in the feed temperature.](solutions/ch6_ex6/ignition.png){#fig-ch6_ex6_ignition width="80%"}

The ext steady state in @fig-ch6_ex6_initial_steady_states is just above the extinction point. @fig-ch6_ex6_extinction shows that if the feed temperature drops from 6 to 3 °C, the reactor temperature will drop from 200 to 3 °C. While this doesn't pose a safety risk like ignition does, it essentially stops the reaction so that the process is no longer producing any Y and Z. Clearly this is not desirable, and to prevent if from happening, the reactor should not operate at a steady state close to the extinction point.

![Reacting fluid temperature following a 1 °C decrease in the feed temperature.](solutions/ch6_ex6/extinction.png){#fig-ch6_ex6_extinction width="80%"}
