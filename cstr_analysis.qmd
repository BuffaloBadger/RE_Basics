...
@sec-ch5 presented an overview of the analysis of ideal reactors. This chapter introduces ideal continuous stirred tank reactors and discusses their analysis.
...
For reasons just described, real CSTRs do not operate at unstable steady states. This leads to one type of parametric sensitivity known as hysteresis. @fig-hysteresis shows only the stable steady states from @fig-T_vs_Tin_unstable. Consider a CSTR operating at steady state with an inlet temperature of 90 °C and an outlet temperature of ca. 100 °C. This point is on the lower solid black line just to the left of the solid red vertical line. If the inlet temperature is very slowly increased in a way that the reactor remains at steady-state, the outlet temperature will slowly rise along the lower black curve in the figure until it reaches the solid red line.

![Steady-state outlet temperature of a CSTR as a function of the inlet temperature.](solutions/ch6_ex4/hysteresis.png){#fig-hysteresis width="80%"}
...
The first four examples in this section involve steady-state CSTRs. [Example -@sec-ch6_ex1] and [Example -@sec-ch6_ex2] illustrate steady-state CSTR response tasks. The former involves two liquid-phase reactions while the second involves a gas phase reaction. In [Example -@sec-ch6_ex1] the assignment is to calculate the reactor output, while in [Example -@sec-ch6_ex2] the assignment is to calculate the input needed to achieve a specified output. [Example -@sec-ch6_ex3] presents the solution of an optimization task involving a steady-state CSTR. [Example -@sec-ch6_ex4] features a steady-state CSTR where three steady states are possible. Start-up of a CSTR, a transient process, is analyzed in [Example -@sec-ch6_ex5]. Finally, [Example -@sec-ch6_ex6] follows three transients that result when an operating parameter of a steady-state CSTR is perturbed. In the first analysis the CSTR is operating at an unstable steady state, and when it is perturbed, the transient ends when it reaches a different, stable steady state. The other two analyses illustrate ignition and extinction.
