...
* An unknown initial value or an unknown constant appearing within a set of IVODEs needs to be found.

* Unknown quantities in the ATEs are initial or final values for the IVODEs, but they do not appear within the IVODEs.

## Solving for an Unknown Initial Value or an Unknown Constant in a Set of IVODEs. {#sec-num_impl_unk_init_value}

The presence of an unknown initial value or an unknown constant within a set of IVODEs does not pose a problem when analytical solution is possible. The IVODEs are simply solved, yielding algebraic equations that contain the unknown quantity. The known value of one of the dependent variables is then substituted into the corresponding algebraic equation which then can be solved to find the unknown constant or initial value.

That procedure cannot be used when the IVODEs are solved numerically. As noted in @sec-apndx_solve_ivodes, IVODE solvers must be provided with initial values of *all* of the dependent variables and a derivatives function that uses the IVODEs to calculate the values of all of the derivatives. If the initial value of one of the dependent variables is unknown, it isn't possible to provide the necessary initial values. If an unknown constant appears in the IVODEs, it isn't possible to calculate the values of the derivatives.

Thus, if an initial value is unknown or if an unknown constant appears in the IVODEs, it isn't possible to call an IVODE solver to solve them. In either case, an additional equation is needed in order to calculate the unknown initial value or constant. Whenever this situation arises in *Reaction Engineering Basics*, the final value of the independent variable *and* the final value of one of the dependent variables will be known. As shown below, the final value of the independent variable is used as the stopping criterion for solving the IVODEs, and the known dependent variable final value is used to write an implicit algebraic equation that can be solved to find the unknown initial value or constant.

To illustrate, consider @eq-ivode_w_unk_iv_1 where $f$ is a known function of $x$ and $y$. Let $c$ represent the unknown initial value or constant, $x_0$ the known initial value of the independent variable, $x_f$ its known final value, and $y_f$ the known final value of the dependent variable. The unknown constant, $c$, either appears within the function $f$ or it is the initial value of the dependent variable, $y\big\vert_{x=0}$.

$$
\frac{dy}{dx} = f
$$ {#eq-ivode_w_unk_iv_1}

It is found by noting that it is "the value of $c$ such that the final value of $y$ calculated by solving the IVODEs, $y \big\vert_{x_f}$, equals the known final value, $y_f$," @eq-unk_ivode_init_value. A residual corresponding to the implicit equation for $c$ can be written as shown in @eq-ukn_ivode_init_value_resid. There is only one unknown, $c$, in @eq-ukn_ivode_init_value_resid, so it can be solved for $c$ using an ATE solver.
...
The ATE solver will return the missing value of $c$, assuming it converges. At that point, knowing the value of $c$, the IVODE solver can be used again to find the values of $x$ and $y$ over the range between their initial and final values. While the description here used a single IVODE, this procedure applies equally to sets of IVODEs.
...
* Write a residuals function that receives a guess for $c$, uses that value and an IVODE solver to solve the IVODEs for the corresponding calculated value,  $y \big\vert_{x_f}$, and calculates and returns the residual.
    * If $c$ is an initial value, it will be provided to the IVODE solver as an argument
    * If $c$ is an unknown constant appearing within the IVODEs, the residuals function must somehow make it available to the derivatives function (see @sec-apndx_solve_ivodes) that is passed to the IVODE solver.
* Pass that residuals function and an initial guess for $c$ to an ATE solver to find the value of $c$.

The description presented here was brief and may seem abstract. Hopefully the example presented in @sec-example_K_1 will make it easier to understand how to calculate an unknown initial value or constant appearing in a set of IVODEs when those IVODEs must be solved numerically.

## DAEs where the Unknowns in the ATEs are Initial and Final Values for the IVODEs {#sec-num_impl_daes}
...
Thus, the key to solving the DAEs is to formulate the solution of the ATEs using the *initial* values of the dependent variables in the IVODEs as the unknowns. The IVODEs then become ancillary equations that express the final values in terms of the initial values. The numerical implementation of the solution is relatively straightforward.
...
An example of solving a set of DAEs in this manner is presented in @sec-example_K_2.

## Examples

The first example below illustrates solving a set of IVODEs for an unknown initial value. The second example illustrates solving a set of DAEs where the unknowns in the ATEs are initial and final values of the IVODEs.
...
The IVODEs are given above as equations (1), (2), and (3).

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

I cannot call an IVODE solver at this point because I don't know the initial value of $T$. As described in @sec-num_impl_unk_init_value, I instead need to write an implicit ATE using the known final temperature, and solve that to find the unknown initial value.

:::

In addition, the final temperature equal its known final value as expressed in the residual defined in equation (5).
...
Equation (4) expresses $r$ in terms of known constants, the independent variable, $z$, and the dependent variables, $\dot{n}_A$, $\dot{n}_Z$, and $T$. Thus, given the initial values and stopping criterion shown in @tbl-apndx_K_3_1_init_values, the final values of the dependent variables can be calculated.
...
In this example the initial value of a dependent variable in the IVODEs was calculated. Essentially the same procedure and code could be used if the initial value was known, but a constant appearing in the IVODEs, $\dot{V}$ for example, was missing. The biggest difference in that case would be that in step 3 of the numerical implementation above, the residuals function would receive a guess for that $\dot{V}$, and it would need to make that guess available for use in the derivatives function. The way that is accomplished will depend upon the software and programming language being used.
...
The equations to be solved are DAEs, and numerical solution of DAEs is detailed in @sec-num_impl_daes. The key to solving the DAEs is to mathematically formulated the solution of the ATEs with the initial values of the IVODEs as the unknowns. The solution of the IVODEs will will need to occur within the soluton of the ATEs, and then one more time after the ATEs have bee solved. Therefore, I will indicate the ATES as the model equations and the initial values of the dependent variables in the IVODEs, $\dot{n}_{A,in}$, $\dot{n}_{Z,in}$, and $T_{in}$, as the unknowns.
...
The ATEs include 3 quantities ($\dot{n}_A\big\vert_{z=L}$, $\dot{n}_Z\big\vert_{z=L}$, and $T\big\vert_{z=L}$) in addition to the unknowns to be found by solving them. The former quantities must be expressed in terms of the unknowns. Given the unknowns, the IVODEs, equations (1) - (3) can be solved to find $\dot{n}_A\big\vert_{z=L}$, $\dot{n}_Z\big\vert_{z=L}$, and $T\big\vert_{z=L}$.
...
3. Express $\dot{n}_A\big\vert_{z=L}$, $\dot{n}_Z\big\vert_{z=L}$, and $T\big\vert_{z=L}$ in terms of $\dot{n}_{A,in}$, $\dot{n}_{Z,in}$, and $T_{in}$ by solving equations (1) - (3) using the initial values and stopping criterion in @tbl-apndx_K_3_2_initial_values.
...
@sec-num_impl_daes describes the numerical implementation of the solution of a set of DAEs. It requires the numerical solution of ATEs, as described in @sec-apndx_solve_ates and the numerical solution of IVODEs, as described in @sec-apndx_solve_ivodes. In essence, when the solution of the ATEs is implemented, the IVODEs are solved within the residuals function. Then, after the ATEs have been solved for the initial values, the IVODEs are solved using the results.
...
library(tidyverse)
source("~/Libraries/R/fmt_tibble_col.R")
df <- read.csv("../RE_Basics_Examples/reb_K_3_2/python/results/reb_K_3_2_results.csv")
