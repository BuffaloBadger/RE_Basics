...
The nature of kinetics experiments is relatively straightforward. In each experiment a group of reactor inputs are adjusted to pre-determined values. The inputs that are adjusted by the person performing the experiment can be referred to as set variables or as factors. The person performing the experiment then measures one output from the reactor. The output that is measured can be referred to as the measured response. (The experimenter could measure several different responses. *Reaction Engineering Basics only considers that analysis of data where a single response was measured.) Thus the data from one experiment consists of the values of the factors (set variables) and the corresponding measured response. The experimenter then performs additional experiments resulting in the generation of a kinetics data set. The kinetics data set consists of the values of the factors for each experiment and the corresponding value of the measured response for that experiment.
...
The kinetics experiments entail adjusting each of the inputs to one of its levels, measuring the corresponding response, adjusting one or more of the inputs to a different one of it levels, measuring the response, etc. The final decision in experimental design involves which combinations of levels to use to generate the kinetics data. There are a few ways to do this. A common one, sometimes called a **full factorial design**, simply includes every possible combination of the input levels. [Example -@sec-example_18_6_1] illustrates full factorial design of a kinetics experiment.
...
In the first stage, each data block is processed separately to estimate the value of $k$ at that block temperature. This creates a new data set consisting of pairs of values of the block temperature and the corresponding $k$. In the second stage of analysis, the Arrhenius pre-exponential factor, $k_0$, and activation energy, $E$, are estimated using those data. This can be done using linear least squares parameter estimation as described in @sec-apndx_parameter_est, and illustrated in [Example -@sec-example_4_4].

### Estimation of Rate Expression Parameters

Parameter estimation involves a process known as "fitting" a model to data. To estimate rate expression parameters a reactor model is "fit" to the kinetics data. More specifically, a reactor model is formulated for the reactor used in the kinetics experiments. Just as special solver functions are used to solve ATEs and IVODEs numerically, special fitting functions are used to fit models to data.
...
@fig-param_est_info_flow illustrates the essence of using a fitting function to fit a reactor model to experimental data. Briefly, the fitting function is provided with the experimental factors (variables set by the experimenter) and responses (quantity measured by the experimenter), an initial guess for the parameters, and the name of a "responses function" that the user must write. In essence, the fitting function keeps guessing values for the parameters until it finds the parameter values for which the model-predicted responses are as close as possible to the experimentally measured responses (see @sec-apndx_parameter_est for more details). It then returns those parameter values along with statistical information that indicates how closely the experimental and predicted responses agree. In *Reaction Engineering Basics* the statistical information is assumed to be 95% confidence intervals for each parameter and the coefficient of determinations, $R^2$ (again, see @sec-apndx_parameter_est for more details). Other statistical indicators such as standard errors can be used in place of these.
...
Internally, the fitting function needs to be able to calculate the predicted responses each time it generates a new guess for the parameters. To enable it to do so, the user must write and provide to it a computer function that will be referred to here as the **responses function**. As indicated in @fig-param_est_info_flow, that function will be provided with guesses for the parameters and the factors for all of the experiments in the data set being analyzed. The responses function uses a reactor model just like those used throughout *Reaction Engineering Basics* to calculate the predicted responses and return them to the fitting function.
...
Then, letting $y$ equal either $\ln{k_j}$ or $\ln{K_j}$ and letting $x$ = $\frac{-1}{RT}$, linear least squares can be used to find $k_{0,j}$ and $E_j$ or $K_{0,j}$ and $\Delta H^0_j$, as described in @sec-apndx_parameter_est and illustrated in [Example -@sec-example_4_4].
...
Graphical assessment of model accuracy is also possible. For **linear models with two parameters**, this was discussed in @sec-2_rates_rate_express and illustrated in [Example -@sec-example_4_4]. Spreadsheet programs can perform the parameter estimation and generate a **model plot** as described above. If the rate expression is accurate, the experimental data in the model plot will scatter randomly about the trendline. The deviations from the line will be small and there will not be any systematic deviations about the trendline.
