...
[Chapters -@sec-2_rates_rate_express] and [-@sec-2_mechanisms] show that in most rate expressions the dependence of a rate on the composition appears directly. In contrast, temperature dependence is often introduced by including a parameter (rate coefficient, equilibrium "constant", or power-law reaction order) that in turn depends upon the temperature. Consider the following rate expression. The concentrations appear directly (they could equally well be partial pressures or mole fractions), but as written here, the temperature does not. It appears in the Arrhenius expression for the rate coefficient and the Arrhenius-like expression for the equilibrium constant.

$$
r = kC_AC_B\left( 1 - \frac{C_YC_Z}{KC_AC_B} \right)
$$
