...
The experimental design described above will lead to a data set where every experiment was performed at one of the chosen temperature levels. It is also possible to generate a data set that does not use specific temperature levels, but instead varies them randomly. If specific temperature levels were not used, or if they were used, but one wants to analyze the entire data set at once, then all rate coefficients must be written in terms of the pre-exponential factor and activation energy. For example, a simple rate expression where the reaction is first order in the concentration of reagent A must be written as shown in @eq-first_order_using_arrhenius. The parameters to be estimated in this situation are $k_0$ and $E$.

$$
r = k_0 \exp { \left( \frac{-E}{RT} \right) } C_A
$$ {#eq-first_order_using_arrhenius}

However, when the data are generated using a fixed number of temperature levels, as in the factorial design described above, it is preferrable to break the full data set into "blocks" such that all of the experiments at the first temperature level are placed in one block. All of the experiments at the second temperature level are placed in a second block, and so on. When the data are broken into same-temperature blocks like this, that rate expression should be written without using the Arrhenius expression, as shown in @eq-first_order_not_using_arrhenius. When the rate expression is written in that form the only parameter to be estimated is $k$, but the data analysis must proceed in two stages.

$$
r = k C_A
$$ {#eq-first_order_not_using_arrhenius}

 In the first stage, each data block is processed separately to estimate the value of $k$ at that block temperature. This creates a new data set consisting of a list of the experimental temperatures levels and a corresponding list of the values of $k$ at those temperatures. In the second stage of analysis, linear least squares parameter estimation is used to estimate the Arrhenius pre-exponential factor, $k_0$, and activation energy, $E$, as described in @sec-apndx_parameter_est, and illustrated in [Example -@sec-example_4_4].
