...
A model for the reactor used in the experiments can be generated as described in Chapters [-@sec-3_design_eqns] and [-@sec-3_reactor_model_func]. Assuming that each kinetics experiment was isothermal (and for PFRs assuming negligible pressure drop), only mole balances need to be included in the reactor design equations. The proposed rate expression can be substituted in the reactor design equations. Given values for the rate expression parameters and the values of the experimentally adjusted inputs for any one experiment, The reactor design equations can be solved to find composition of the reacting fluid. Specifically, the molar amounts at the time a batch reactor was sampled or the molar flow rates leaving a CSTR or PFR can be calculated by solving the reactor design equations.
