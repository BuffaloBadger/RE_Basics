...
At this point, I can choose a value for $t_{sf}$ and solve equations (3), (4), (5), (7), and (9) to get corresponding sets of values of $t$, $n_A$, $n_W$, $n_Z$, $T$, and $T_{ex}$ that span the range from their initial values to their final values. From those results I can determine the total processing time and the maximum temperature. I can repeat that process using a range of values for $t_{sf}$ to find the value of $t_{sf}$ where the reaction time is a minimum and the maximum temperature is less than 95 °C.

:::

**Ancillary Equations for Calculating the Quantities of Interest**

With the information provided above, the design equations can be solved using a range of values of $t_{sf}$. Each time, solving the reactor design equations yields sets of corresponding values of $t$, $n_A$, $n_W$, $n_Z$, $T$, and $T_{ex}$. For that value of $t_{sf}$, the total prcessing time is the final value of $t$, and the maximum value of $T$ can be determined. If the maximum value of $T$ is less than 95 °C, the result satisfies the temperature constraint. After doing this for each value of $t_{sf}$ the resulting $t_{sf}$ *vs*. $t_f$ data can be plotted to indentify the $t_{sf}$ that minimizes $t_f$, 
