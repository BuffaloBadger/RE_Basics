...
Numerical implementation involves writing computer code that the instructions at the end of the mathematical formulation. Presentation and discussion of the results is straightforward.
...
{{< include ../RE_Basics_Examples/reb_10_5_1/narrative.qmd >}}

{{< include ../RE_Basics_Examples/reb_10_5_1/equations.qmd >}}
...
**Given and Known Constants**: $C_{B,in}$ = 2 M, $C_{A,0}$ = 10 M, $\Delta H_1$ = -44 kcal mol^-1^, $k_{0,1}$ =  8.11 x 10^12^ L mol^-1^ s^-1^, $E_1$ = 17.7 kcal mol^-1^, $V$ = 25 L, $V_0$ = 4 L, $T_0$ = $T_{ex,0}$ = 20 °C, $T_{ex,in}$ = 20 °C, $\dot{m}_{ex}$ = 0.1 kg min^-1^, $V_{ex}$ = 0.5 L jacket, $A$ = 0.6 ft^2^, $U$ = 1.13 x 10^4^ cal ft^-2^ h^-1^ K^-1^, $\rho$ = $\rho _{ex}$ = 1 g cm^-3^, $\tilde{C}_p$ = $\tilde{C}_{p,ex}$ = 1 cal g^-1^ K^-1^, $P$ =  1 atm, $T_{in}$ = 20 °C, and $\dot{V}_{in}$ = 1.0 L min^-1^.
...
The reactor is cooled using a heat exchange fluid that gains sensible heat, so an energy balance on that exchange fluid, @eq-exchange_energy_bal_sensible is also needed. Again, because the heat capacity is constant it can be taken outside of the integral and the integral can be evaluated.
...
The quantities of interest are the concentration of A and the temperature during the period when B is being added to the reactor. When the reacting fluid volume becomes equal to the specified reactor volume, the feed must stop, so I can use that as the stopping criterion.
...
: Initial values and stopping criterion for solving  equations, (3) through (9). {#tbl-example_10_5_1_initial_values}
...
I'll start by writing the design equations in the form of derivative expressions (see @sec-apndx_solve_ivodes). Looking at equations (3) through (9), I see that all but equation (7) are already in the form of derivative expressions. I can also see that if I substitute equation (9) into equation (7) for $\frac{dV}{dt}$, that will convert equation (7) to the necessary form.

I'll need to evaluate the derivatives at the start of each integration step. At that point, the independent and dependent variables ($t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$) will be known along with the given and known constants. I will need to calculate any other quantities that appear in the derivatives expressions. Examining equations (3) - (6), and (8) through (10), I see that the unknown quantities $r_1$, $\dot{n}_{B,in}$, $\dot{m}_{B,in}$, and $\dot{Q}$ are also present.
...
Substitution of equation (9) for $\frac{dV}{dt}$ in equation (7) converts it to the form of a derivative expression, equation (10).

$$
\rho V \tilde{C}_p \frac{dT}{dt} - P\dot{V}_{in} = \dot Q - \dot{m}_{B,in} \tilde{C}_p\left(T - T_{in}\right)  - V r_1 \Delta H_1 \tag{10}
$$

The rate, $r_1$, appearing in the design equations can be calculated using the rate expression, equation (2). The other unknown quantities appearing in the rate expression and design equations can be calculated using equations (11) through (16).
...
$$
\dot{n}_{B,in} = \dot{V}_{in} C_{B,in} \tag{14}
$$

$$
\dot{m}_{B,in} = \dot{n}_{B,in}M_{NaOH} \tag{15}
$$

$$
\dot{Q} = UA\left(T - T_{ex} \right) \tag{16}
$$

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

When I solve the IVODEs numerically, I'll also need to know the initial and final values in @tbl-example_10_5_1_initial_values. I defined $t_0$ to equal 0, and $T_0$, $T_{ex,0}$, $V_0$, and $V$ are known. The initial molar amount of A can be calculated from the known initial reacting fluid volume and the initial concentration of A. 
...
$$
n_{A,0} = C_{A,0}V_0 \tag{17}
$$

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

At this point the reactor design equations can be solved. Since they are IVODEs, doing so will yield corresponding sets of corresponding values of $t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$ spanning the range from their initial values to the point where the reactor becomes full. I can generate the requested graph showing $T$ *vs*. $t$ directly. Before I can generate the other requested graph, I need to calculate corresponsing values of $C_A$. I can use the definition of concentration for this purpose.

:::

**Ancillary Equations for Calculating the Quantities of Interest**

Solving the reactor design equations yields sets of corresponding values of $t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$ spanning the range from their initial values to the point where the reactor becomes full. Corresponding values of $C_A$ can be calculated using eqation (12).

**Calculations Summary**

1. Substitute given and known constants into all equations.
2. When it is necessary to evaluate the derivatives
	a. $t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$ will be available.
	b. calculate $k_1$, $C_A$, and $C_B$ using equations (11) through (13) and substitute into equation (2).
	c. calculate $r_1$, $\dot{n}_{B,in}$, $\dot{m}_{B,in}$, and $\dot{Q}$ using equations (2) and (14) - (16), and substitute into equations (3) - (6) and (8) - (10).
	d. evaluate the derivatives using equations (3) - (6) and (8) - (10).
3. When it is necessary to calculate the initial and final values in @tbl-example_10_5_1_initial_values
	...
	b. calculate $n_{A,0} using equation (17). 
5. When it is necessary to calculate the quantity of interest, $C_A$
	...
	b. calculate corresponding values of $C_A$ using equation (12).
...
2. Write a derivatives function that
	a. receives the independent and dependent variables, $t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$, as arguments,
...
6. Perform the analysis by
	a. getting corresponding sets of values of $t$, $n_A$, $n_B$, $n_S$, $n_W$, $T$, $T_{ex}$, and $V$, spanning the range from their initial values to their final values by calling the reactor model function from step 3 above,
	b. calculates and plots the quantities of interest as described in step 5 of the calculations summary.
...
{{< include ../RE_Basics_Examples/reb_10_5_2/narrative.qmd >}}

{{< include ../RE_Basics_Examples/reb_10_5_2/equations.qmd >}}
...
This assignment involves the analysis of an isolated SBSTR. I will begin by summarizing the information provided in the assignment narrative. I'll use variable symbols that are appropriate for each given quantity. I'll use subsctipted "0" to denote initial values, subscripted "f" to denote final values and subscripted "in" to denote inlet values. For the situations where the addition of the B solution will stop before the end of processing, I'll denote the elapsed time when the flow stopped as $t_{add}$.

When the solution of B is added over 8 h, the reactor operates as an SBSTR for the entier time. However, when it is added over less than 8 hours, the operational protocol has two stages: semi-batch while reagent B is being added and batch after the feed stops.

:::

#### Assignment Summary

**Given and Known Constants**: $k_{0,1}$ = 1.83 x 10^12^ L mol^-1^ h^-1^, $E_1$ = 18.0 kcal mol^-1^, $k_{0,2}$ = 5.08 x 10^13^ L mol^-1^ h^-1^, $E_2$ = 20.5 kcal mol^-1^, $\Delta H_1$ = -9000, $\Delta H_2$ = -7800 cal mol^-1^, $\breve{C}_p$ = 863 cal L^-1^ K^-1^, $C_{A,0}$ = 2 M, $C_{B,in}$ =  0.5 M, $T_0$ = 40 °C, $P$ = 1 atm, $V_0$ = 2000 L, $V_B$ = 8000 L, $t_f$ = 8 h, and $t_{add}$ = 2, 4, 6 or 8 h.
...
**Quantities of Interest**: $f_B$ and $S_{D/U}$
...
Mole balance design equations for reagents A, B, D, and U are presented in equations (5) through (8). If there is a second stage of BSTR operation, the only change is that $\dot{n}_{B,in}$ is equal to zero. The energy balance on the reacting fluid is given in equation (9). If there is a second stage of BSTR operation, the only change is that the first term on the right side of the equals sign is becomes zero.
...
$$
V \breve{C}_p \frac{dT}{dt} - P\frac{dV}{dt} = -\dot{V}_{in}\breve{C}_p \left(T - T_0 \right)  - r_1V \Delta H_1 - r_2V \Delta H_2 \tag{9}
$$
...
$$
\frac{dT}{dt} = \frac{-\dot{V}_{in}\breve{C}_p \left(T - T_0 \right)  - r_1V \Delta H_1 - r_2V \Delta H_2 + P \dot{V}_{in}}{V \breve{C}_p} \tag{11}
$$

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

The design equations are IVODEs, with two stages of operation possible. I can define $t=0$ to be the time when the flow of reagent B into the reactor begins. At that time only reagent A is present in the reactor. The first stage of operation ends when the flow of reagent B ends.

If the flow of reagent B ends before 8 hours of operation, the second stage begins immediately. Consequently the initial values are equal to the final values from the first stage. I will use a subscript "sb,f" to denote values at the end of the semi-batch stage of operation. The second stage ends after 8 h of operation.
...
I'll start by writing the equations that I'll need in order to evaluate the derivatives appearing in the reactor design equations. The design equations already take the form of derivative expressions (see @sec-apndx_solve_ivodes) so they do not need to be rearranged.
...
Loodking at the design equations I see that they contain the following unknown quantities: $r_1$, $r_2$, and $V$ ($V is the instantaneous volume of the reacting fluid, not the total capacity of the reactor). The rates can be calculated using the rate expressions provided in the assignment, but that introduces the rate coefficients and concentrations of A and B. The Arrhenius expression can be used to calculate the rate coefficients and the defining equations for concentration can be used to calculate the concentations. Noting that the inlet flow rate is constant, equation (10) can be integrated to get an expression for the instantaneous volume during the time that fluid is flowing into the reactor.
...
Solving the reactor design equations will yield corresponding values of $t$, $n_A$, $n_B$, $n_D$, $n_U$, and $T$ spanning the range from $t=0$ to $t=t_f$. The assignment asks for the final selectivity for D over U and the final conversion of B. The former can be calculated using its defining equation. Normally the conversion is calculated as the fraction of the initial amount of a reactant that has been converted. Here, the initial amount of B is zero, so that definition won't work. Instead, the conversion must be calculated as the fraction of the total B added to the system that has been converted.

:::

**Ancillary Equations for Calculating the Quantities of Interest**

Solving the reactor design equations yields $t$, $n_A$, $n_B$, $n_D$, $n_U$, and $T$ spanning the range from $t=0$ to $t=t_f$. The final amounts can be used to calculate the conversion of B and the selectivity using equations (19) and (20).

$$
f_B = \frac{V_BC_{B,in} - n_{B,f}}{V_BC_{B,in}} \tag{19}
$$

$$
S_{D/U} = \frac{n_{D,f}}{n_{U,f}} \tag{20}
$$
...
2. When it is necessary to evaluate the derivatives
	...
	b. calculate $\dot{V}_{in}$, $V$, $k_1$, and $k_2$ using equations (12) through (15).
...
3. When it is necessary to calculate the initial and final values in @tbl-example_10_5_2_stage_1_initial_values and @tbl-example_10_5_2_stage_2_initial_values.
	...
	b. calculated $n_{A,0}$ using equation (18).
...
4. When it is necessary to calculate the quantities of interest, $f_B$ and $S_{D/U}$
	...
	b. calculate $f_B$ and $S_{D/U}$ using equations (19) and (20).
...
I will begin by summarizing the information provided in the assighment narrative. To simplify the notation I well let A represent acetic anhydride, W represent water, and Z represent acetic acid. With this notation, the reaction is $A+W\rightarrow 2Z$. I will use a subscripted "0" to denote initial values, "sf" to denote values at the end of semi-batch processing (stage 1), "f" to denote final values (at the end of stage 2), and "in" to denote inlet values. I will assume that the density and heat capacity of the cooling water are constant and equal to 1 g cm^-3^ and 1 cal g^-1^ K^-1^.

:::

#### Assignment Summary

**Given and Known Constants**: $k_{0,1}$ =  1.192 x 10^15^ min^-1^, $E_1$ =  97,600 J mol^-1^, $\Delta H_1$ = -58,615 J mol^-1^, $V_{ex}$ = ? cm^3^, $T_{ex,in}$ = 60 °C, $\dot{V}_{ex,in}$ = ? cm^3^ min^-1^ $UA$ = 260 cal min^-1^ K^-1^, $P$ = 1 atm, $V_{W,0}$ = 67 cm^3^ $V_{Z,0}$ = 283 cm^3^, $V_{H_2SO_4,0}$ = 0.30 cm^3^, $T_0$ = 60 °C, $\breve{C}_p$ =  2.68 J cm^-3^ K^-1^, $V_A$ = 350 cm^3^, $T_{in}$ =  21 °C, $f_A$ =  0.99, $T_{max}$ = 95 °C, $\rho_A$ = 1.082 g cm^-3^, $\rho_W$ = 1.0 g cm^-3^, $\rho_Z$ = 1.049 g cm^-3^, $M_A$ = 102g mol^-1^, $M_W$ = 18 g mol^-1^, $M_Z$ = 60 g mol^-1^, $\hat{C}_{p,A}$ = 168.2 J mol^-1^ K^-1^, $\rho_{ex} = 1.0 g cm^-3^, and $\tilde{C}_{p,ex}$ = 1 cal g^-1^ K^-1^.
...
This assignment requires the modeling of an isolated SBSTR/BSTR, so I need to generate the reactor design equations for it. Mole balances are always needed when modeling a reactor. The general SBSTR mole balance is given in @eq-sbstr_mol_balance. Only one reaction is taking place, so the summation reduces to a single term, and only reagent A flows into the reactor, so $\dot n_{W,in}$ = $\dot n_{Z,in}$ = 0.
...
The reactor is cooled using water that gains sensible heat, so an energy balance on the exchange fluid, @eq-exchange_energy_bal_sensible, is also needed. Again, because the heat capacity is constant it can be taken outside of the integral and the integral can be evaluated.
...
Mole balance design equations for A, W, and Z are presented in equations (3), (4), and (5). The energy balance on the reacting fluid is given in equation (6). In the second stage of operation, $\dot{n}_{A,in}$ in equations (3) and (6) is equal to zero.  An energy balance on the heat exchange fluid is shown in equation (7).
...
The second stage of operation begins immediately, so the initial values for the second stage are simply equal to the final values from the first stage. The second stage ends when the 99% of the added A has been converted, so the stopping criterion can be defined in terms of the molar amount of A.
...
I'll start by writing the equations that I'll need in order to evaluate the derivatives appearing in the reactor design equations. The design equations are in the form of differential expressions (see @sec-apndx_solve_ivodes), so no manipulation is needed to put them in that form.
...
In this analysis, I will be choosing values of $t_{sf}$ to see which one gives the smallest acceptable processing time. Knowing the volume of A to be added and the time over which it will be added, the volumetric flow rate for the first stage of operation can be calculated. Then, knowing the density and molecular weight of A, its inlet molar flow rate can be calculated. The volumetric and molar flows are equal to zero during the second stage of operation.

$$
\dot{V}_{in} = \frac{V_A}{t_{sf}} \qquad \qquad t \le t_{sf}
$$

$$
\dot{n}_{A,in} = \frac{\dot{V}_{in} \rho_A}{M_A} \qquad \qquad t \le t_{sf}
$$
...
$$
V = \dot{V}_{in} t + V_0 \qquad \qquad t \le t_{sf}
$$
...
The volumetric flow rate of A into the reactor during the first stage of operation is given by equation (10) and its molar flow rate is given by equation (11). The instantaneous reacting fluid volume is given by equation (12). The rate can be calculated using equation (2), with the rate coefficient and concentration of A that appear in it calculated using equations (13) and (14). The rate of heat exchange is given by equation (15) and the mass flow rate of the cooling water by equation (16).

$$
\begin{aligned}
\dot{V}_{in} &= \frac{V_A}{t_{sf}}; \qquad t \le t_{sf}\\
\dot{V}_{in} &= 0; \qquad t > t_{sf}
\end{aligned} \tag{10}
$$

$$
\dot{n}_{A,in} = \frac{\dot{V}_{in} \rho_A}{M_A} \tag{11}
$$

$$
\begin{aligned}
V &= V_0 + \dot{V}_{in}t; \qquad t \le t_{sf}\\
V &= V_0 + V_A; \qquad t > t_{sf}
\end{aligned}\tag{12}
$$
...
$$
\dot{Q} = UA\left(T - T_{ex} \right) \tag{15}
$$

$$
\dot{m}_{ex} = \dot{V}_{ex} \rho_{ex} \tag{16}
$$

:::{.callout-tip collapse="true"}
## Click Here to See What an Expert Might be Thinking at this Point

When I solve the IVODEs numerically, I'll also need to know the initial and final values @tbl-example_10_5_3_stage_1_initial_values and @tbl-example_10_5_3_stage_2_initial_values. For the first stage of operation, the initial molar amounts can be calculated from the initial volumes, densities and molecular weights. The initial temperatures are known. I will be choosing values of $t_{sf}$ to find which one minimizes the processing time, so it, too will be known. The reactor design equations then can be solved for the first stage of processing. The resulting final values then can be used as the initial values for the second stage of operation. The final molar amount of A can be calculated from the spedified conversion and the initial amount.

:::

**Ancillary Equations for Calculating the Initial and Final Values**

The unknown initial molar amounts for the first stage of operation can be found using equations (17) and (18). The initial temperatures will be known and the final time of the first stage, $t_{sf}$ will be chosen. The design equations for the first stage can then be solved. The resulting final values can then be used as the initial values for the second phase of operation. The final molar amount of A can be calculated using equation (19).

$$
n_{W,0} = \frac{V_{W,0}\rho_W}{M_W} \tag{17}
$$ 

$$
n_{Z,0} = \frac{V_{Z,0}\rho_Z}{M_Z} \tag{18}
$$

$$
n_{A,f} = \frac{V_A \rho_A}{M_A}\left(1 - f_A\right) \tag{19}
$$
