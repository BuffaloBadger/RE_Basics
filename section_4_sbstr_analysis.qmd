...
This assignment requires the analysis of an isolated reactor with a two-stage operational protocol. During the first stage it operates as an SBSTR. The first stage ends when all of the acetic anhydride has been added. During the second stage, if there is one, the reactor operates as a BSTR. A second stage is only used if the conversion of acetic anhydride at the end of the first stage is less than 99%.
...
**Given and Known Constants**: $k_{0,1}$ = 1.192 x 10^15^ min^-1^, $E_1$ =  97,600 J mol^-1^, $\Delta H_1$ = -58,615 J mol^-1^, $V_{ex}$ = 300 cm^3^, $T_{ex,in}$ = 60 °C, $\dot{V}_{ex,in}$ = 250 cm^3^ min^-1^ $UA$ = 260 cal min^-1^ K^-1^, $P$ = 1 atm, $V_{W,0}$ = 67 cm^3^ $V_{Z,0}$ = 283 cm^3^, $V_{H_2SO_4,0}$ = 0.30 cm^3^, $T_0$ = 60 °C, $T_{ex,0}$ = 60 °C, $\breve{C}_p$ =  2.68 J cm^-3^ K^-1^, $V_A$ = 350 cm^3^, $T_{in}$ =  21 °C, $f_A$ =  0.99, $T_{max}$ = 95 °C, $\rho_A$ = 1.082 g cm^-3^, $\rho_W$ = 1.0 g cm^-3^, $\rho_Z$ = 1.049 g cm^-3^, $M_A$ = 102 g mol^-1^, $M_W$ = 18 g mol^-1^, $M_Z$ = 60 g mol^-1^, $\hat{C}_{p,A}$ = 168.2 J mol^-1^ K^-1^, $\rho_{ex}$ = 1.0 g cm^-3^, and $\tilde{C}_{p,ex}$ = 1 cal g^-1^ K^-1^.
...
This assignment requires the modeling of an isolated SBSTR/BSTR, so I need to generate the reactor design equations for it. Mole balances are always needed when modeling a reactor. The general SBSTR mole balance is given in @eq-sbstr_mol_balance. Only one reaction is taking place, so the summation reduces to a single term, and only reagent A flows into the reactor, so $\dot n_{W,in}$ = $\dot n_{Z,in}$ = 0. I won't write a mole balance for the acid since it does not react, and its molar amount is constant.
...
If the conversion of A at the end of semi-batch processing is less than 99%, there will be a second, BSTR stage. The second stage of operation begins immediately, so the initial values for the second stage are simply equal to the final values from the first stage. The second stage ends when the 99% of the added A has been converted. I can represent the molar amount of A when the conversion reached 99% by $n_{A,f}$ and define the stopping criterion in terms of the molar amount of A.
...
Having found the optimum volumetric flow rate, I can solve the reactor design equations using it to get corresponding values of $t$, $n_A$, $n_W$, $n_Z$, $T$, and $T_{ex}$. I can calculate corresponding values of $C_A$ using equations (11) and (13).
...
The reactor design equations then can be solved using that optimum volumetric feed rate to get corresponding values of $t$, $n_A$, $n_W$, $n_Z$, $T$, and $T_{ex}$. Corresponding values of $C_A$ can then be calculated using equations (11) and (13).
...
3. When it is necessary to calculate the initial and final values in @tbl-example_10_5_3_stage_1_initial_values and @tbl-example_10_5_3_stage_2_initial_values
	...
	d. calculate the $n_{A,f}$ using equation (19).
4. When it is necessary to calculate the quantity of interest, $C_A$
	...
	b. calculate corresponding values of $V$ using equation (11).
	c. calculate corresponding values of $C_A$ using equation (13).
	d. plot $T$ *vs*. $t$ and $C_A$ *vs*. $t$.
...
The calculations were performed as described above, and a few of the results are shown in @tbl-example_10_5_3_results. The temperature profile is shown in @fig-example_10_5_3_temp, and the acetic anhydride concentration profile is shown in @fig-example_10_5_3_conc. A feed rate of `r d$value[3]` `r d$units[3]` results in the minimum proecssing time, `r d$value[1]` `r d$units[1]`. At the end of semi-batch processing, the conversion of acetic anhydride is greater than 99%, so a second, BSTR stage of operation is not needed. 
...
@fig-example_10_5_1_temperature_profile shows that the reacting fluid temperature just reaches 95 °C at the end of processing. If the acetic anhydride was fed any faster, the temperature would exceed the specified maximum. Lowering the feed rate would increase the processing time. @tbl-example_10_5_2_results shows that the cooling water reaches a maximum temperature of `r d$value[5]` `r d$units[5]`.

**Qualitative Analysis** The shapes of the profile curves are consistent with a qualitative analysis of the process. Initially there isn't any acetic anhydride present so the rate is zero and no heat is being released. As acetic anhydride begins to flow into the reactor, several things start to occur. The concentration of A begins to increase. The acetic anhydride is added at a constant rate, but as the concentration builds, the reaction rate increases while the addition rate remains constant. Thus the rate of concentration increase gets smaller (the curve is concave up).

At first the temperature decreases slightly. This is because the acetic anhydride at 21 °C is being added to the 60 °C reacting fluid. Within less than a minute, however, the temperature begins to rise. This is because the reaction rate is increasing and the exothermic heat of reaction releases more heat than the combined heat used to heat the incoming acetic anhydride and that removed by the cooling water.

The concentration of A, the temperature, and the rate all continue to rise until the concentration reaches a maximum. At that point, the reaction rate has increases to the point of being equal to the rate at which acetic anhydride is being added, so the concentration passes through a maximum and begins to decline. The temperature is still increasing at this point because the heat of reaction is still adding more heat than can be removed.

Beyond that point, the concentration continues to decrease while the temperature is still rising. Shortly after the concentration maximum, the rate of concentration decrease and the rate of temperature increase are large (the curves are steep). Eventually the reaction rate falls because the effect of decreasing concentraton on the rate becomes stronger than the effect of increasing temperature.

The profiles in @fig-example_10_5_3_temp and @fig-example_10_5_3_conc end when all of the acetic anhyride has been added to the system because at that time the conversion is greater than 99%. At that point, heat is still being released as the remaining acetic anhydride reacts, but the cooling associated with adding cold feed stops. Given that the temperature was still rising at the end of semi-batch processing, it is likely that the temperature will continue to rise for a short period of time after the feed flow stops. Eventually the concentration of acetic anhydride (and therefore the rate) will go to zero. At that point no heat is being added by reaction, but the cooling water will continue to remove heat, eventually bringing the temperature back down to 60 °C. 
