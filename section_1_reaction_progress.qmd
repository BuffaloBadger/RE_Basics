...
A mole table for this system is presented in @tbl-mole_table_reb_3_4_1.
...
```{r}
#| echo: false
results <- read.csv("./Calculations/example_3_4_1/reb_3_4_1_results.csv")
```
...
I'll begin by summarizing the information provided in the problem statement. The system described here is the same as in [Example -@sec-example_3_4_3], so I can use the mole table from that example, @tbl-mole_table_reb_3_4_3.
...
There are three unknown quantities in equations (7) through (9), so they can be solved to find the values of $\xi_1$, $\xi_2$, and $\xi_4$. The resulting apparent extents of reaction can then be substituted in the expressions in the third column of @tbl-mole_table_reb_3_4_3 to compute the molar amounts of each of the reagents.
...
Performing the calculations as described yields the final molar amounts shown in @tbl-results_reb_3_4_4.
