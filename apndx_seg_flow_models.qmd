...
## Numerical Cumulative Age Distribution Function

The cumulative age distribution function, $F\left(\lambda\right)$, and its derivative, $\frac{dF}{d\lambda}$, appear in the design equations for the segregated flow reactor models. Those design equations are IVODEs, and they typically must be solved numerically. To do so, a derivatives function must be written. It must accept a value of the independent variable and corresponding values of the dependent variables and return the values of the derivatives that appear in the design equations.

Typically, if the cumulative age distribution function was measured using stimulus-response experiments (see @sec-7_age_function), $F\left(\lambda\right)$ will be available as a table of corresponding values of $F$ and $\lambda$, and not as an analytical function. When the IVODE solver calls the derivatives function, it may pass any value of the independent variable (and corresponding values of the dependent variables). More importantly, it may pass a value for the time (i. e. $\lambda$) that is not in the table of $F$ *vs.* $\lambda$ values. Consequently, when $F\left(\lambda\right)$ and $\frac{dF}{d\lambda}$ are needed to evaluate the derivatives, their values will need to be estimated numerically using the tabular $F$ *vs.* $\lambda$ data.

The value of $F$ at the largest values of $\lambda$ should be equal to 1, within experimental error. If this is not the case, a row can be added to the end of the tabular data with $\lambda$ greater than all of the values in the table and $F$ equal to one. The easiest way to estimate $F$ and $\frac{dF}{d\lambda}$ is using backward differences. The numerical estimates for $F$ and $\frac{dF}{d\lambda}$ should equal zero for values of $\lambda$ less than or equal to zero. For values of $\lambda$ greater than the largest value in the table, the estimate for $F$ should equal 1.0 and that for $\frac{dF}{d\lambda}$ should equal zero. Any other value of $\lambda$ will fall between two table entries, one, $\lambda_{i-1}$, less than $\lambda$ and the other, $\lambda_{i}$, greater than $\lambda$. The value of $F$ can be calculated using @eq-F_estimate, and the value of $\frac{dF}{d\lambda}$ can be estimated using @eq-dFdlambda_estimate.

$$
F\left(\lambda\right) = F\left(\lambda_{i-1}\right) + \frac{F\left(\lambda_{i}\right) - F\left(\lambda_{i-1}\right)}{\lambda_{i} - \lambda_{i-1}} \left( \lambda - \lambda_{i-1} \right)
$${#eq-F_estimate}

$$
\frac{dF}{d\lambda}\Bigg\vert_{\lambda} = \frac{F\left(\lambda_{i}\right) - F\left(\lambda_{i-1}\right)}{\lambda_{i} - \lambda_{i-1}}
$${#eq-dFdlambda_estimate}
